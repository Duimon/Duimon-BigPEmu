...
bool HSM_GetIsInABCompareArea(float2 viewport_coord)
{
    ...
    float test_value = _6093;
    float _6106;
    if (mod(HSM_AB_COMPARE_AREA, 2.0f) == 1.0f)
    {
        _6106 = 1.0f - HSM_AB_COMPARE_SPLIT_POSITION;
    }
    else
    {
        _6106 = HSM_AB_COMPARE_SPLIT_POSITION;
    }
    float position = _6106;
    bool _6120 = (mod(HSM_AB_COMPARE_AREA, 2.0f) == 0.0f) && (test_value < position);
    bool _6131;
    if (!_6120)
    {
        _6131 = (mod(HSM_AB_COMPARE_AREA, 2.0f) == 1.0f) && (test_value > position);
    }
    else
    {
        _6131 = _6120;
...
float2 GetSimpleImageScaledCoord(inout float2 in_viewport_coord, float2 in_viewport_unscaled_coord, float2 in_tube_coord, float2 in_tube_scale, Texture2D<float4> in_sampler, SamplerState _in_sampler_sampler, float2 in_pos, float in_inherit_pos, float2 in_scale, float in_scale_inherit_mode, float in_keep_aspect, float in_mirror_horz, float in_rotate)
{
    ...
    uint _5146_dummy_parameter;
    float2 tex_size = float2(int2(spvTextureSize(in_sampler, uint(0), _5146_dummy_parameter)));
    float _5151;
    if (in_rotate == 1.0f)
    {
    ...
    if (in_rotate > 0.5f)
    {
        _5182 = global_FinalViewportSize.x / global_FinalViewportSize.y;
    }
    else
    {
        _5182 = 1.0f;
...
{
    uint _6137_dummy_parameter;
    float2 tex_size = float2(int2(spvTextureSize(in_sampler, uint(0), _6137_dummy_parameter)));
...
float3 HSM_RGBtoHSV(float3 c)
{
    ...
    if (c.y < c.z)
    {
        _2635 = float4(c.zy, K.wz);
    }
    else
    {
        _2635 = float4(c.yz, K.xy);
    }
    float4 p = _2635;
    float4 _2665;
    if (c.x < p.x)
    {
        _2665 = float4(p.xyw, c.x);
    }
    else
    {
        _2665 = float4(c.x, p.yzx);
    }
    float4 q = _2665;
...
float4 HSM_GetNightLightingMultiplyColor(float2 in_coord, float hue, float saturation, float value, float contrast, float global_ambient_opacity, Texture2D<float4> NightLightingImage_1, SamplerState _NightLightingImage_1_sampler)
{
    ...
    float4 _6266 = lighting_image;
    float3 _6270 = lerp(1.0f.xxx, _6266.xyz, global_ambient_opacity.xxx);
...
bool HSM_Fill_Ambient_Images(float2 in_viewport_coord, float2 in_viewport_unscaled_coord, float2 in_tube_coord, float2 in_tube_scale, float in_swap_images, Texture2D<float4> in_ambient_sampler, SamplerState _in_ambient_sampler_sampler, Texture2D<float4> in_ambient2_sampler, SamplerState _in_ambient2_sampler_sampler, out float4 ambient_lighting_image, out float4 ambient2_lighting_image)
{
    ...
    if (HSM_AMBIENT1_OPACITY > 0.0f)
    {
        ...
        bool _6288 = HSM_AMBIENT1_SCALE_INHERIT_MODE == 1.0f;
        bool _6295;
        if (_6288)
        {
            _6295 = (HSM_AMBIENT1_SCALE * HSM_VIEWPORT_ZOOM) < 1.0f;
        }
        else
        {
            _6295 = _6288;
        ...
        float2 _6329 = GetSimpleImageScaledCoord(param, param_1, param_2, param_3, in_ambient_sampler, _in_ambient_sampler_sampler, param_4, param_5, param_6, param_7, param_8, param_9, param_10);
        float2 lighting_coord = _6329;
    ...
    if (HSM_AMBIENT2_OPACITY > 0.0f)
    {
        ...
        bool _6350 = HSM_AMBIENT2_SCALE_INHERIT_MODE == 1.0f;
        bool _6357;
        if (_6350)
        {
            _6357 = (HSM_AMBIENT2_SCALE * HSM_VIEWPORT_ZOOM) < 1.0f;
        }
        else
        {
            _6357 = _6350;
        ...
        float2 _6391 = GetSimpleImageScaledCoord(param_17, param_18, param_19, param_20, in_ambient2_sampler, _in_ambient2_sampler_sampler, param_21, param_22, param_23, param_24, param_25, param_26, param_27);
        float2 lighting2_coord = _6391;
...
float3 maxscale(float2 sinangle, float2 cosangle, float in_radius, float in_distance, float in_aspect)
{
    ...
    float2 _5508 = fwtrans(param_5, param_6, param_7, param_8, param_9);
    ...
    float2 _5525 = fwtrans(param_10, param_11, param_12, param_13, param_14);
    float2 lo = float2(_5508.x, _5525.y) / aspect_vec2;
    ...
    float2 _5545 = fwtrans(param_15, param_16, param_17, param_18, param_19);
    ...
    float2 _5561 = fwtrans(param_20, param_21, param_22, param_23, param_24);
    float2 hi = float2(_5545.x, _5561.y) / aspect_vec2;
...
float2 HSM_GetGeomCurvedCoord(float2 in_coord, float tilt_x, float tilt_y, float in_radius, inout float in_distance, float in_aspect)
{
    ...
    float2 _5658 = transform(param_5, param_6, param_7, param_8, param_9, param_10, param_11);
    float2 curved_coord = _5658;
    return curved_coord;
}
...
float2 HSM_GetGeomCurvedCoordRetainWidth(float2 in_coord, float tilt_x, float tilt_y, float in_radius, float in_distance, float in_aspect)
{
    ...
    float2 _5675 = HSM_GetGeomCurvedCoord(param, param_1, param_2, param_3, param_4, param_5);
    float2 ctr_curved_coord = _5675 - 0.5f.xx;
    ...
    float2 _5690 = HSM_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11);
    float2 right_edge_curved_ctr_coord = _5690 - 0.5f.xx;
...
float3 hrg_get_ideal_global_eye_pos_for_points(inout float3 eye_pos, float2 output_aspect, float3 global_coords[9], int num_points, float in_geom_radius, float in_geom_view_dist)
{
    ...
    for (int iter = 0; iter < max_centering_iters; iter++)
    {
        ...
        float3 _4087 = eye_pos;
        float2 _4089 = _4087.xy + center_offset;
        eye_pos.x = _4089.x;
        eye_pos.y = _4089.y;
...
float3 hrg_get_ideal_global_eye_pos(float3x3 local_to_global, float2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist)
{
    ...
    if (in_geom_mode < 2.5f)
    {
        _4248 = float3(0.0f, 0.0f, eye_z_spherical);
    }
    else
    {
        _4248 = float3(0.0f, 0.0f, max(in_geom_view_dist, eye_z_spherical));
    }
    float3 eye_pos = _4248;
    int num_points = 9;
    ...
    else
    {
        ...
        float3 _4408 = hrg_get_ideal_global_eye_pos_for_points(param_36, param_37, param_38, param_39, param_40, param_41);
        _4391 = _4408;
    }
    return _4391;
}
...
float2 hrg_get_curved_video_uv_coords_and_tangent_matrix(float2 flat_video_uv, float3 eye_pos_local, float2 output_pixel_size, float2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, float3x3 global_to_local, out float2x2 pixel_to_tangent_video_uv)
{
    ...
    float3 param_5;
    float2 _4633 = hrg_view_vec_to_uv(param, param_1, param_2, param_3, param_4, param_5);
    float3 pos = param_5;
    float2 centered_uv = _4633;
    ...
    if (geom_force_correct_tangent_matrix)
    {
        ...
        {
            _4645 = float3(pos.x, 0.0f, pos.z);
        }
        float3 normal_base = _4645;
...
float2 HRG_GetGeomCurvedCoord(float2 in_coord, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, float in_geom_tilt_angle_x, float in_geom_tilt_angle_y, float in_screen_aspect, float pin_inner_edge, float2 in_source_size, float2 in_output_size, out float2x2 pixel_to_video_uv)
{
    ...
    if (in_geom_mode > 0.5f)
    {
        ...
        float2x2 param_14;
        float2 _4813 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14);
        pixel_to_video_uv = param_14;
        float2 ctr_curved_coord = _4813 - 0.5f.xx;
        if ((in_geom_tilt_angle_x == 0.0f) && (in_geom_tilt_angle_y == 0.0f))
        {
            ...
            float2x2 param_23;
            float2 _4842 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_15, param_16, param_17, param_18, param_19, param_20, param_21, param_22, param_23);
            pixel_to_video_uv = param_23;
            float2 right_edge_curved_ctr_coord = _4842 - 0.5f.xx;
            ...
            float2x2 param_32;
            float2 _4864 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_24, param_25, param_26, param_27, param_28, param_29, param_30, param_31, param_32);
            pixel_to_video_uv = param_32;
            float2 bottom_edge_curved_ctr_coord = _4864 - 0.5f.xx;
            ctr_curved_coord.x = (ctr_curved_coord.x * 0.5f) / right_edge_curved_ctr_coord.x;
        ...
        if (pin_inner_edge == 1.0f)
        {
            if (in_geom_tilt_angle_y != 0.0f)
            {
                ...
                float2x2 param_41;
                float2 _4907 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_33, param_34, param_35, param_36, param_37, param_38, param_39, param_40, param_41);
                pixel_to_video_uv = param_41;
                float2 top_edge_curved_ctr_coord = _4907;
            ...
            if (in_geom_tilt_angle_x != 0.0f)
            {
                ...
                float2x2 param_50;
                float2 _4936 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_42, param_43, param_44, param_45, param_46, param_47, param_48, param_49, param_50);
                pixel_to_video_uv = param_50;
                float2 left_edge_curved_ctr_coord = _4936;
...
float2 HSM_GetCurvatureValues(float screen_aspect)
{
    ...
    {
        _5220 = float2((((2.0f * global_HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f, ((global_HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f);
    }
    else
    {
        _5220 = float2(((global_HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f, (((2.0f * global_HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f);
    }
    float2 curvature_values = _5220;
...
float2 HSM_GetCrtPiCurvedCoord(inout float2 in_coord, inout float2 in_curvature)
{
    ...
    float rsq = (in_coord.x * in_coord.x) + _5723;
...
float2 HSM_Get2DCurvedCoord(float2 in_coord, float2 curvature_values)
{
    ...
    float2 _5754 = HSM_GetCrtPiCurvedCoord(param, param_1);
    ctr_curved_coord = _5754 - 0.5f.xx;
    float2 param_2 = float2(1.0f, 0.5f);
    float2 param_3 = curvature_values;
    float2 _5761 = HSM_GetCrtPiCurvedCoord(param_2, param_3);
    float2 right_edge_curved_ctr_coord = _5761 - 0.5f.xx;
    ctr_curved_coord.x = (ctr_curved_coord.x * 0.5f) / right_edge_curved_ctr_coord.x;
    float2 param_4 = float2(0.5f, 1.0f);
    float2 param_5 = curvature_values;
    float2 _5775 = HSM_GetCrtPiCurvedCoord(param_4, param_5);
    float2 bottom_edge_curved_ctr_coord = _5775 - 0.5f.xx;
    ctr_curved_coord.y = (ctr_curved_coord.y * 0.5f) / bottom_edge_curved_ctr_coord.y;
    return ctr_curved_coord + 0.5f.xx;
}
...
float2 HSM_GetCurvedCoord(float2 in_coord, float curvature_multiplier, float screen_aspect)
{
    ...
    if (HSM_CURVATURE_MODE > CURVATURE_MODE_2D_CYLINDER)
    {
        ...
        else
        {
            ...
            float2x2 param_16;
            float2 _5883 = HRG_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14, param_15, param_16);
...
float2 HSM_GetTubeCurvedCoord(float2 screen_coord, float curvature_scale, float2 screen_scale, float2 tube_scale, float screen_aspect, float apply_black_edge_offset)
{
    ...
    if (global_HSM_BZL_USE_INDEPENDENT_CURVATURE == 1.0f)
    {
        ...
        if (screen_aspect < 1.0f)
        {
            _6022 = float2((((2.0f * global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f, ((global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f);
        }
        else
        {
            _6022 = float2(((global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f, (((2.0f * global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f);
        }
        float2 curvature_values = _6022;
...
float HSM_GetBezelCoords(float2 tube_diffuse_coord, float2 tube_diffuse_scale, float2 tube_scale, float screen_aspect, bool curve_coords_on, out float2 bezel_outside_scale, inout float2 bezel_outside_coord, inout float2 bezel_outside_curved_coord, inout float2 frame_outside_curved_coord)
{
    ...
    if (curve_coords_on)
    {
        bool _6551 = HSM_CURVATURE_MODE > 0.0f;
        bool _6559;
        if (_6551)
        {
            _6559 = (HSM_CURVATURE_3D_TILT_ANGLE_X != 0.0f) || (HSM_CURVATURE_3D_TILT_ANGLE_Y != 0.0f);
        }
        else
        {
            _6559 = _6551;
...
float4 HSM_BlendModeLayerMix(float4 color_under, inout float4 color_over, float blend_mode, float layer_opacity)
{
    ...
    {
        float _2930 = color_over.w;
        float4 _2931 = color_over;
        float3 _2933 = _2931.xyz * _2930;
    ...
    else
    {
        ...
        {
            float3 _2957 = color_under.xyz + color_over.xyz;
        ...
        {
            float3 _2973 = color_under.xyz * color_over.xyz;
...
float3 ApplyAmbientImages(float3 base_image, inout float3 ambient_image, inout float3 ambient2_image, float blend_ambient, float blend_ambient2, float apply_in_add_mode, float layer_blend_mode, float swap_images)
{
    ...
    bool _6431 = (HSM_AMBIENT1_OPACITY > 0.0f) || (HSM_AMBIENT2_OPACITY > 0.0f);
    bool _6439;
    if (_6431)
    {
        _6439 = (blend_ambient > 0.0f) || (blend_ambient2 > 0.0f);
    }
    else
    {
        _6439 = _6431;
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    _6743 = (mask_mode == 8.0f) ? OUTSIDE_FRAME_MASK : 0.5f;
                                }
                                _6735 = _6743;
                            }
                            _6728 = _6735;
...
{
    bool _6764 = blend_mode == 0.0f;
    bool _6774;
    if (!_6764)
    {
        _6774 = (dualscreen_mode != 0.0f) && (dualscreen_mode != SCREEN_INDEX);
    }
    else
    {
        _6774 = _6764;
    ...
    if (blend_mode == BLEND_MODE_NORMAL)
    {
        float _6816 = color_over.w;
        float4 _6817 = color_over;
        float3 _6819 = _6817.xyz * _6816;
    ...
    else
    {
        ...
        if (blend_mode == BLEND_MODE_ADD)
        {
            float3 _6843 = color_under.xyz + color_over.xyz;
        ...
        if (blend_mode == BLEND_MODE_MULTIPLY)
        {
            float3 _6859 = color_under.xyz * color_over.xyz;
...
float4 HSM_GetPreMultipliedColorLinear(float4 in_color, float matte_type, float encoded_gamma)
{
    ...
    {
        float4 _2841 = out_color;
        float _2844 = out_color.w;
        float3 _2850 = clamp(_2841.xyz - (1.0f - _2844).xxx, 0.0f.xxx, 1.0f.xxx);
        out_color.x = _2850.x;
        out_color.y = _2850.y;
        out_color.z = _2850.z;
    }
    ...
    {
        float _2868 = out_color.w;
        float4 _2869 = out_color;
        float3 _2871 = _2869.xyz * _2868;
        out_color.x = _2871.x;
        out_color.y = _2871.y;
        out_color.z = _2871.z;
    }
...
void frag_main()
{
    ...
    bool _7239 = HSM_AB_COMPARE_FREEZE_GRAPHICS == 1.0f;
    bool _7247;
    if (_7239)
    {
    ...
    float4 _7266 = HSM_UpdateGlobalScreenValuesFromCache(InfoCachePass, _InfoCachePass_sampler, param_3);
    float4 feedback_color_test = BR_LayersUnderCRTPassFeedback.Sample(_BR_LayersUnderCRTPassFeedback_sampler, 0.0f.xx);
    bool _7271 = HSM_CACHE_GRAPHICS_ON > 0.5f;
    bool _7277;
    if (_7271)
    {
        _7277 = feedback_color_test.w < 0.0f;
    }
    else
    {
        _7277 = _7271;
    ...
    bool _7306 = HSM_Fill_Ambient_Images(param_4, param_5, param_6, param_7, param_8, NightLightingImage, _NightLightingImage_sampler, NightLighting2Image, _NightLighting2Image_sampler, param_9, param_10);
    ...
    float _7458 = HSM_GetBezelCoords(param_32, param_33, param_34, param_35, param_36, param_37, param_38, param_39, param_40);
    ...
    float corner_rotation_offset = _7573;
    ...
    float _7644 = HHLP_QuadraticBezier(param_62, param_63);
    float fade_out_to_corner = _7644;
    ...
    {
        _7740 = 1.0f - smoothstep(0.0f, corner_fade_width / 2.0f, distance_from_crease_shifted);
    }
    else
    {
        _7740 = 1.0f;
    }
    float top_bottom_vs_sides_mask = _7740;
    ...
    else
    {
        ...
        _7779 = 1.0f - HSM_GetCornerMask(param_69, param_70, param_71, param_72);
    }
    float frame_inner_edge_mask = _7779;
    ...
    float hmbz_bezel_brightness_frame_inner_edge = 0.01400000043213367462158203125f;
    ...
    float _7996 = HHLP_QuadraticBezier(param_87, param_88);
    float top_center_highlight_mask = (hmbz_bezel_highlight_top * top_mask) * _7996;
    ...
    float _8011 = HHLP_QuadraticBezier(param_89, param_90);
    float bottom_center_highlight_mask = (hmbz_bezel_highlight_bottom * bottom_mask) * _8011;
    ...
    float _8026 = HHLP_QuadraticBezier(param_91, param_92);
    float sides_highlight_mask = (hmbz_bezel_highlight_sides * sides_mask) * _8026;
    ...
    float _8040 = HHLP_QuadraticBezier(param_93, param_94);
    float edge_top_center_highlight_mask = (hmbz_bezel_highlight_top * top_mask) * _8040;
    ...
    float _8053 = HHLP_QuadraticBezier(param_95, param_96);
    float edge_bottom_center_highlight_mask = (hmbz_bezel_highlight_bottom * bottom_mask) * _8053;
    ...
    float _8066 = HHLP_QuadraticBezier(param_97, param_98);
    float edge_sides_highlight_mask = (hmbz_bezel_highlight_sides * sides_mask) * _8066;
    ...
    float inner_edge_highlight_multiplier = hmbz_bezel_brightness_frame_inner_edge + ((HSM_BZL_HIGHLIGHT * 10.0f) * hmbz_bezel_brightness_frame_inner_edge);
    ...
    if (HSM_FRM_TEXTURE_OPACITY > 0.0f)
    {
        ...
        float4 _8181 = HSM_BlendModeLayerMix(param_101, param_102, param_103, param_104);
    ...
    if (HSM_FRM_OPACITY > 0.001000000047497451305389404296875f)
    {
        ...
        float _8217 = HHLP_QuadraticBezier(param_105, param_106);
    ...
    if ((HSM_BZL_OPACITY > 0.0f) || (HSM_FRM_OPACITY > 0.0f))
    {
        ...
        float3 _8262 = ApplyAmbientImages(param_109, param_110, param_111, param_112, param_113, param_114, param_115, param_116);
        ...
        if (HSM_FRM_SHADOW_OPACITY > 0.0f)
        {
            ...
            float4 _8320 = BlendModeMaskLayerMix(param_121, param_122, param_123, param_124, param_125, param_126, param_127);
            bezel_layer = _8320;
        }
    ...
    bool _8359 = SCREEN_ASPECT < 1.0f;
    bool _8368;
    if (_8359)
    {
        uint _8365_dummy_parameter;
        _8368 = int2(spvTextureSize(BackgroundVertImage, uint(0), _8365_dummy_parameter)).y > 16;
    }
    else
    {
        _8368 = _8359;
    ...
    {
        uint _8378_dummy_parameter;
        _8373 = int2(spvTextureSize(BackgroundVertImage, uint(0), _8378_dummy_parameter));
    }
    else
    {
        uint _8383_dummy_parameter;
        _8373 = int2(spvTextureSize(BackgroundImage, uint(0), _8383_dummy_parameter));
    }
    float2 bg_size = float2(_8373);
    ...
    float2 _8424 = HSM_GetScaledCoord(param_132, param_133, param_134, param_135, param_136, param_137, param_138, param_139, param_140, param_141, param_142, param_143, param_144, param_145, param_146);
    ...
    bool _8443 = HSM_BG_OPACITY > 0.0f;
    bool _8450;
    if (_8443)
    {
        _8450 = bg_size.y > 16.0f;
    }
    else
    {
        _8450 = _8443;
    ...
    if (_8450)
    {
        ...
        float3 _8490 = HSM_ApplyHSVAdjustment(param_157, param_158, param_159, param_160, param_161, param_162);
        bg_image.x = _8490.x;
        bg_image.y = _8490.y;
        bg_image.z = _8490.z;
        if (HSM_STATIC_LAYERS_GAMMA != 1.0f)
        ...
        float3 _8525 = ApplyAmbientImages(param_165, param_166, param_167, param_168, param_169, param_170, param_171, param_172);
        bg_image.x = _8525.x;
        bg_image.y = _8525.y;
        bg_image.z = _8525.z;
    }
    bool _8534 = SCREEN_ASPECT < 1.0f;
    bool _8543;
    if (_8534)
    {
        uint _8540_dummy_parameter;
        _8543 = int2(spvTextureSize(DeviceVertImage, uint(0), _8540_dummy_parameter)).y > 16;
    }
    else
    {
        _8543 = _8534;
    ...
    {
        uint _8552_dummy_parameter;
        _8547 = int2(spvTextureSize(DeviceVertImage, uint(0), _8552_dummy_parameter));
    }
    else
    {
        uint _8557_dummy_parameter;
        _8547 = int2(spvTextureSize(DeviceImage, uint(0), _8557_dummy_parameter));
    }
    float2 device_size = float2(_8547);
    ...
    float2 _8598 = HSM_GetScaledCoord(param_173, param_174, param_175, param_176, param_177, param_178, param_179, param_180, param_181, param_182, param_183, param_184, param_185, param_186, param_187);
    DEVICE_COORD = param_186;
    DEVICE_SCALE = param_187;
    DEVICE_CURVED_COORD = _8598;
    float4 device_image = 0.0f.xxxx;
    bool _8603 = HSM_DEVICE_OPACITY > 0.0f;
    bool _8609;
    if (_8603)
    {
        _8609 = device_size.y > 16.0f;
    }
    else
    {
        _8609 = _8603;
    ...
    if (_8609)
    {
        ...
        if (device_use_vert_image)
        {
            ...
            _8613 = HSM_GetMipmappedTexSample(DeviceVertImage, _DeviceVertImage_sampler, param_188, param_189, param_190);
        }
        else
        {
            ...
            _8613 = HSM_GetMipmappedTexSample(DeviceImage, _DeviceImage_sampler, param_191, param_192, param_193);
        }
        device_image = _8613;
        ...
        float3 _8651 = HSM_ApplyHSVAdjustment(param_197, param_198, param_199, param_200, param_201, param_202);
        ...
        float3 _8686 = ApplyAmbientImages(param_205, param_206, param_207, param_208, param_209, param_210, param_211, param_212);
        device_image.x = _8686.x;
        device_image.y = _8686.y;
        device_image.z = _8686.z;
    }
    uint _8697_dummy_parameter;
    float2 deviceled_size = float2(int2(spvTextureSize(DeviceLEDImage, uint(0), _8697_dummy_parameter)));
    ...
    float2 _8737 = HSM_GetScaledCoord(param_213, param_214, param_215, param_216, param_217, param_218, param_219, param_220, param_221, param_222, param_223, param_224, param_225, param_226, param_227);
    DEVICELED_COORD = param_226;
    DEVICELED_SCALE = param_227;
    DEVICELED_CURVED_COORD = _8737;
    float4 deviceled_image = 0.0f.xxxx;
    bool _8742 = HSM_DEVICELED_OPACITY > 0.0f;
    bool _8748;
    if (_8742)
    {
        _8748 = deviceled_size.y > 16.0f;
    }
    else
    {
        _8748 = _8742;
    ...
    if (_8748)
    {
        ...
        float3 _8777 = HSM_ApplyHSVAdjustment(param_234, param_235, param_236, param_237, param_238, param_239);
        ...
        float3 _8812 = ApplyAmbientImages(param_242, param_243, param_244, param_245, param_246, param_247, param_248, param_249);
    ...
    {
        uint _8828_dummy_parameter;
        float2 led_size = float2(int2(spvTextureSize(LEDImage, uint(0), _8828_dummy_parameter)));
        ...
        float2 _8868 = HSM_GetScaledCoord(param_250, param_251, param_252, param_253, param_254, param_255, param_256, param_257, param_258, param_259, param_260, param_261, param_262, param_263, param_264);
        LED_COORD = param_263;
        LED_SCALE = param_264;
        LED_CURVED_COORD = _8868;
        bool _8872 = HSM_LED_OPACITY > 0.0f;
        bool _8878;
        if (_8872)
        {
            _8878 = led_size.y > 16.0f;
        }
        else
        {
            _8878 = _8872;
        ...
        if (_8878)
        {
            ...
            float3 _8907 = HSM_ApplyHSVAdjustment(param_271, param_272, param_273, param_274, param_275, param_276);
            led_image.x = _8907.x;
            led_image.y = _8907.y;
            led_image.z = _8907.z;
            if (HSM_STATIC_LAYERS_GAMMA != 1.0f)
            ...
            float3 _8942 = ApplyAmbientImages(param_279, param_280, param_281, param_282, param_283, param_284, param_285, param_286);
            led_image.x = _8942.x;
            led_image.y = _8942.y;
            led_image.z = _8942.z;
        }
    }
    uint _8953_dummy_parameter;
    float2 decal_size = float2(int2(spvTextureSize(DecalImage, uint(0), _8953_dummy_parameter)));
    ...
    float2 _8993 = HSM_GetScaledCoord(param_287, param_288, param_289, param_290, param_291, param_292, param_293, param_294, param_295, param_296, param_297, param_298, param_299, param_300, param_301);
    DECAL_COORD = param_300;
    DECAL_SCALE = param_301;
    DECAL_CURVED_COORD = _8993;
    float4 decal_image = 0.0f.xxxx;
    bool _8998 = HSM_DECAL_OPACITY > 0.0f;
    bool _9004;
    if (_8998)
    {
        _9004 = decal_size.y > 16.0f;
    }
    else
    {
        _9004 = _8998;
    ...
    if (_9004)
    {
        ...
        float3 _9033 = HSM_ApplyHSVAdjustment(param_308, param_309, param_310, param_311, param_312, param_313);
        ...
        float3 _9068 = ApplyAmbientImages(param_316, param_317, param_318, param_319, param_320, param_321, param_322, param_323);
        decal_image.x = _9068.x;
        decal_image.y = _9068.y;
        decal_image.z = _9068.z;
    }
    uint _9079_dummy_parameter;
    float2 top_size = float2(int2(spvTextureSize(TopLayerImage, uint(0), _9079_dummy_parameter)));
    ...
    float2 _9119 = HSM_GetScaledCoord(param_324, param_325, param_326, param_327, param_328, param_329, param_330, param_331, param_332, param_333, param_334, param_335, param_336, param_337, param_338);
    ...
    bool _9131 = HSM_TOP_OPACITY > 0.0f;
    bool _9137;
    if (_9131)
    {
        _9137 = top_size.y > 16.0f;
    }
    else
    {
        _9137 = _9131;
    ...
    if (_9137)
    {
        ...
        float3 _9166 = HSM_ApplyHSVAdjustment(param_346, param_347, param_348, param_349, param_350, param_351);
        top_image.x = _9166.x;
        top_image.y = _9166.y;
        top_image.z = _9166.z;
        if (HSM_STATIC_LAYERS_GAMMA != 1.0f)
        ...
        float3 _9201 = ApplyAmbientImages(param_354, param_355, param_356, param_357, param_358, param_359, param_360, param_361);
        top_image.x = _9201.x;
        top_image.y = _9201.y;
        top_image.z = _9201.z;
    }
    ...
    {
        uint _9217_dummy_parameter;
        float2 cab_glass_size = float2(int2(spvTextureSize(CabinetGlassImage, uint(0), _9217_dummy_parameter)));
        ...
        float2 _9257 = HSM_GetScaledCoord(param_362, param_363, param_364, param_365, param_366, param_367, param_368, param_369, param_370, param_371, param_372, param_373, param_374, param_375, param_376);
        CAB_GLASS_COORD = param_375;
        CAB_GLASS_SCALE = param_376;
        CAB_GLASS_CURVED_COORD = _9257;
        bool _9261 = HSM_CAB_GLASS_OPACITY > 0.0f;
        bool _9267;
        if (_9261)
        {
            _9267 = cab_glass_size.y > 16.0f;
        }
        else
        {
            _9267 = _9261;
        ...
        if (_9267)
        {
            ...
            float3 _9296 = HSM_ApplyHSVAdjustment(param_383, param_384, param_385, param_386, param_387, param_388);
            ...
            float3 _9331 = ApplyAmbientImages(param_391, param_392, param_393, param_394, param_395, param_396, param_397, param_398);
    ...
    float2 _9393 = HSM_GetScaledCoord(param_399, param_400, param_401, param_402, param_403, param_404, param_405, param_406, param_407, param_408, param_409, param_410, param_411, param_412, param_413);
    temp_coord = param_412;
    temp_scale = param_413;
    float2 cutout_coord = _9393;
    ...
    if (HSM_VIEWPORT_VIGNETTE_OPACITY > 0.0f)
    {
        ...
        float2 _9444 = HSM_GetScaledCoord(param_418, param_419, param_420, param_421, param_422, param_423, param_424, param_425, param_426, param_427, param_428, param_429, param_430, param_431, param_432);
        temp_coord = param_431;
        temp_scale = param_432;
        float2 vignette_coord = _9444;
        ...
        float _9456 = HHLP_QuadraticBezier(param_436, param_437);
        vignette_layer.w += (0.75f * _9456);
    }
    ...
    for (int i = start_layer; i <= end_layer; i++)
    {
        if (HSM_BG_LAYER_ORDER == float(i))
        {
            ...
            float4 _9513 = BlendModeMaskLayerMix(param_442, param_443, param_444, param_445, param_446, param_447, param_448);
        ...
        if (HSM_VIEWPORT_VIGNETTE_LAYER_ORDER == float(i))
        {
            ...
            float4 _9533 = BlendModeMaskLayerMix(param_449, param_450, param_451, param_452, param_453, param_454, param_455);
        ...
        if ((HSM_LED_LAYER_ORDER == float(i)) && (HSM_LED_OPACITY > 0.0f))
        {
            ...
            float4 _9557 = BlendModeMaskLayerMix(param_456, param_457, param_458, param_459, param_460, param_461, param_462);
        ...
        if ((HSM_DEVICE_LAYER_ORDER == float(i)) && (HSM_DEVICE_OPACITY > 0.0f))
        {
            ...
            float4 _9581 = BlendModeMaskLayerMix(param_463, param_464, param_465, param_466, param_467, param_468, param_469);
        ...
        if ((HSM_DEVICELED_LAYER_ORDER == float(i)) && (HSM_DEVICELED_OPACITY > 0.0f))
        {
            ...
            float4 _9605 = BlendModeMaskLayerMix(param_470, param_471, param_472, param_473, param_474, param_475, param_476);
        ...
        if ((HSM_DECAL_LAYER_ORDER == float(i)) && (HSM_DECAL_OPACITY > 0.0f))
        {
            ...
            float4 _9629 = BlendModeMaskLayerMix(param_477, param_478, param_479, param_480, param_481, param_482, param_483);
        ...
        if ((HSM_CAB_GLASS_LAYER_ORDER == float(i)) && (HSM_CAB_GLASS_OPACITY > 0.0f))
        {
            ...
            float4 _9653 = BlendModeMaskLayerMix(param_484, param_485, param_486, param_487, param_488, param_489, param_490);
        ...
        if ((HSM_TOP_LAYER_ORDER == float(i)) && (HSM_TOP_OPACITY > 0.0f))
        {
            ...
            float4 _9677 = BlendModeMaskLayerMix(param_491, param_492, param_493, param_494, param_495, param_496, param_497);
    ...
    if ((HSM_BZL_OPACITY > 0.0f) || (HSM_FRM_OPACITY > 0.0f))
    {
        ...
        float4 _9702 = BlendModeMaskLayerMix(param_500, param_501, param_502, param_503, param_504, param_505, param_506);
    ...
    bool _9808 = UNFLIPPED_VIEWPORT_COORD.x < (2.0f / global_OutputSize.x);
    bool _9817;
    if (_9808)
    {
        _9817 = UNFLIPPED_VIEWPORT_COORD.y < (2.0f / global_OutputSize.y);
    }
    else
    {
        _9817 = _9808;
