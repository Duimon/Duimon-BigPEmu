...
float4 HSM_GetBlackOnlyCropInPixels(Texture2D<float4> in_sampler_2D, SamplerState _in_sampler_2D_sampler, float2 sample_start_pixel_coord, float2 window_size, float num_samples, float4 max_crop)
{
    ...
    for (int i = 0; float(i) < crop_left_px; i++)
    {
        ...
        float4 _3944 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler_2D, _in_sampler_2D_sampler, param, param_1, param_2);
        tex_sample = _3944;
    ...
    for (int i_1 = 0; float(i_1) < crop_top_px; i_1++)
    {
        ...
        float4 _3991 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler_2D, _in_sampler_2D_sampler, param_3, param_4, param_5);
        tex_sample = _3991;
        brightness = (tex_sample.x + tex_sample.y) + tex_sample.z;
        if (brightness > HSM_CROP_BLACK_THRESHOLD)
        {
    ...
    for (int i_2 = 0; float(i_2) < crop_right_px; i_2++)
    {
        ...
        float4 _4039 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler_2D, _in_sampler_2D_sampler, param_6, param_7, param_8);
        tex_sample = _4039;
        brightness = (tex_sample.x + tex_sample.y) + tex_sample.z;
        if (brightness > HSM_CROP_BLACK_THRESHOLD)
        {
    ...
    for (int i_3 = 0; float(i_3) < crop_bottom_px; i_3++)
    {
        ...
        float4 _4087 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler_2D, _in_sampler_2D_sampler, param_9, param_10, param_11);
        tex_sample = _4087;
        brightness = (tex_sample.x + tex_sample.y) + tex_sample.z;
        if (brightness > 0.0f)
        {
...
void HSM_GetCroppedRotatedSizeAndPixelSampleAreaStart(inout float screen_index, Texture2D<float4> original_pass, SamplerState _original_pass_sampler, out float2 cropped_rotated_size, out float2 cropped_sample_area_start)
{
    ...
    float2 rotated_negative_crop_added_size = HSM_GetRotatedNegativeCropAddedSize();
...
{
    float2 core_prepped_size = global_NegativeCropAddedPassSize.xy;
    if (HSM_DUALSCREEN_MODE > 0.0f)
    {
        if (HSM_GetCoreImageSplitDirection() == 1.0f)
        {
...
float2 HSM_GetResMult(float use_vert_scanlines)
{
    ...
    bool _3501 = (use_vert_scanlines == 1.0f) && (HSM_ROTATE_CORE_IMAGE == 0.0f);
    bool _3513;
    if (!_3501)
    {
        bool _3506 = use_vert_scanlines == 0.0f;
        bool _3512;
        if (_3506)
        {
            _3512 = abs(HSM_ROTATE_CORE_IMAGE) == 1.0f;
        }
        else
        {
            _3512 = _3506;
    ...
    else
    {
        ...
        if (global_HSM_INT_SCALE_MODE == 0.0f)
        {
            if (HSM_USE_PHYSICAL_SIZE_FOR_NON_INTEGER == 1.0f)
            {
                ...
                {
                    _3173 = 1.0f / HSM_PHYSICAL_MONITOR_ASPECT_RATIO;
                }
                else
                {
            ...
            if (HSM_SNAP_TO_CLOSEST_INT_SCALE_TOLERANCE > 0.0f)
            {
                ...
                {
                    _3228 = 1.0f - int_scale_remainder;
                }
        ...
        if (global_HSM_INT_SCALE_MODE > 0.5f)
        {
            ...
            bool _3382 = HSM_GetUseVerticalScanlines(param) == 1.0f;
            bool _3389;
            if (!_3382)
            {
                _3389 = global_HSM_INT_SCALE_MODE == 2.0f;
            }
            else
            {
                _3389 = _3382;
...
float2 HSM_GetScreenPositionOffset(inout float2 placement_image_pos, float2 screen_scale, float screen_index)
{
    ...
    if (screen_index == 1.0f)
    {
        _2612 = float2(HSM_SCREEN_POSITION_X / output_aspect, HSM_SCREEN_POSITION_Y) + (placement_image_pos - 0.5f.xx);
    }
    else
    {
        _2612 = float2(HSM_2ND_SCREEN_POS_X / output_aspect, HSM_2ND_SCREEN_POS_Y);
    }
    float2 pos_offset = _2612;
...
{
    NEGATIVE_CROP_EXPAND_MULTIPLIER = global_NegativeCropAddedPassSize.y / global_DerezedPassSize.y;
    ...
    float2 _5542 = HSM_GetCroppedRotatedSizeWithResMult(param_7, param_8);
    float2 cropped_rotated_size_with_res_mult = _5542;
    float param_9 = 60.0f;
    ...
    {
        _5567 = (0.60000002384185791015625f * HSM_SINDEN_BORDER_THICKNESS) * 60.0f;
    }
    else
    {
        _5567 = 0.0f;
    ...
    float2 _5601 = HSM_GetTubeScale(param_14, param_15, param_16);
    float2 tube_diffuse_scale = _5601;
    ...
    if (global_HSM_TUBE_DIFFUSE_FORCE_ASPECT > 0.0f)
    {
        ...
        if (global_HSM_TUBE_DIFFUSE_FORCE_ASPECT == 2.0f)
        {
            ...
            {
                _5628 = global_HSM_TUBE_EXPLICIT_ASPECT;
            }
            else
            {
                _5628 = 1.0f / global_HSM_TUBE_EXPLICIT_ASPECT;
            }
    ...
    float2 _5667 = HSM_GetTubeScale(param_17, param_18, param_19);
    float2 tube_scale = _5667;
    float2 param_20 = screen_pos_and_height.xy;
    float2 param_21 = screen_scale;
    float param_22 = 1.0f;
    float2 _5675 = HSM_GetScreenPositionOffset(param_20, param_21, param_22);
    float2 pos_offset = _5675;
    float param_23 = 1.0f;
    ...
    float2 _5718 = HSM_GetCroppedRotatedSizeWithResMult(param_30, param_31);
    float2 cropped_size_with_res_mult_2nd_screen = _5718;
    ...
    float2 _5737 = HSM_GetTubeScale(param_32, param_33, param_34);
    float2 tube_diffuse_scale_2nd_screen = _5737;
    ...
    float2 _5756 = HSM_GetTubeScale(param_35, param_36, param_37);
    float2 tube_scale_2nd_screen = _5756;
    ...
    float param_40 = 2.0f;
    float2 _5770 = HSM_GetScreenPositionOffset(param_38, param_39, param_40);
    float2 pos_offset_2nd_screen = _5770;
    ...
    if (HSM_IsCoordIn2DRange(param_43, param_44) == 1)
    {
        ...
        float2 param_45 = global_NegativeCropAddedPassSize.xy;
    ...
    if (HSM_IsCoordIn2DRange(param_56, param_57) == 1)
    {
        ...
        float2 _5851 = HSM_GetRotatedNegativeCropAddedSize();
        out_color.z = _5851.x;
        out_color.w = _5851.y;
    }
    float param_58 = 1.0f;
    ...
    {
        out_color.x = global_CorePassSize.xy.x;
        out_color.y = global_CorePassSize.xy.y;
        float2 _5972 = HSM_GetRotatedCoreOriginalSize();
        out_color.z = _5972.x;
        out_color.w = _5972.y;
    }
    float param_82 = 3.0f;
    ...
    {
        float2 _5990 = float2(HSM_VIEWPORT_ZOOM, HSM_VIEWPORT_ZOOM);
        out_color.x = _5990.x;
        out_color.y = _5990.y;
        float2 _5997 = float2(HSM_VIEWPORT_POSITION_X, HSM_VIEWPORT_POSITION_Y);
        out_color.z = _5997.x;
        out_color.w = _5997.y;
    }
    float param_86 = 4.0f;
    ...
    if (!_6179)
    {
        _6189 = abs(parameter_sum - parameter_sum_feedback) > 1.0000000116860974230803549289703e-07f;
    }
    else
    {
        _6189 = _6179;
    ...
    {
        _6190 = false || (HSM_CACHE_GRAPHICS_ON < 0.5f);
    }
...
void frag_main()
{
    ...
    float4 _6226 = HSM_GetColorForScreenInfoCache(param, InfoCachePassFeedback, _InfoCachePassFeedback_sampler, Source, _Source_sampler, ScreenPlacementImage, _ScreenPlacementImage_sampler);
    FragColor = _6226;
}
