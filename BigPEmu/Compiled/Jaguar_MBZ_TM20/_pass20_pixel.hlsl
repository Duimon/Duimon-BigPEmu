...
float3 maxscale(float2 sinangle, float2 cosangle, float in_radius, float in_distance, float in_aspect)
{
    ...
    float2 _3920 = fwtrans(param_5, param_6, param_7, param_8, param_9);
    ...
    float2 _3937 = fwtrans(param_10, param_11, param_12, param_13, param_14);
    float2 lo = float2(_3920.x, _3937.y) / aspect_vec2;
    ...
    float2 _3957 = fwtrans(param_15, param_16, param_17, param_18, param_19);
    ...
    float2 _3973 = fwtrans(param_20, param_21, param_22, param_23, param_24);
    float2 hi = float2(_3957.x, _3973.y) / aspect_vec2;
...
float2 HSM_GetGeomCurvedCoord(float2 in_coord, float tilt_x, float tilt_y, float in_radius, inout float in_distance, float in_aspect)
{
    ...
    float2 _4070 = transform(param_5, param_6, param_7, param_8, param_9, param_10, param_11);
    float2 curved_coord = _4070;
    return curved_coord;
}
...
float2 HSM_GetGeomCurvedCoordRetainWidth(float2 in_coord, float tilt_x, float tilt_y, float in_radius, float in_distance, float in_aspect)
{
    ...
    float2 _4087 = HSM_GetGeomCurvedCoord(param, param_1, param_2, param_3, param_4, param_5);
    float2 ctr_curved_coord = _4087 - 0.5f.xx;
    ...
    float2 _4102 = HSM_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11);
    float2 right_edge_curved_ctr_coord = _4102 - 0.5f.xx;
...
float3 hrg_uv_to_xyz(float2 uv, float2 output_aspect, float in_geom_mode, float in_geom_radius)
{
    ...
    if (in_geom_mode < 1.5f)
    {
        ...
        _2470 = hrg_sphere_uv_to_xyz(param, param_1, param_2);
    }
    else
    {
        float3 _2483;
        if (in_geom_mode < 2.5f)
        {
            ...
            _2483 = hrg_sphere_alt_uv_to_xyz(param_3, param_4, param_5);
        }
        else
        {
            float2 param_6 = uv;
            ...
            _2483 = hrg_cylinder_uv_to_xyz(param_6, param_7, param_8);
        }
        _2470 = _2483;
    }
    return _2470;
...
float3 hrg_get_ideal_global_eye_pos_for_points(inout float3 eye_pos, float2 output_aspect, float3 global_coords[9], int num_points, float in_geom_radius, float in_geom_view_dist)
{
    ...
    for (int iter = 0; iter < max_centering_iters; iter++)
    {
        ...
        float3 _2649 = eye_pos;
        float2 _2651 = _2649.xy + center_offset;
        eye_pos.x = _2651.x;
        eye_pos.y = _2651.y;
        ...
        for (int i_3 = 0; i_3 < num_points; i_3++)
        {
            ...
            {
                _2715 = max(offset_z_max, offset_zzzz.x);
            }
            else
            {
                _2715 = offset_z_max;
            ...
            {
                _2728 = max(offset_z_max, offset_zzzz.y);
            }
            else
            {
                _2728 = offset_z_max;
            ...
            if (eyespace_xyz_flipy.x > 0.0f)
            {
                _2741 = max(offset_z_max, offset_zzzz.z);
            }
            else
            {
                _2741 = offset_z_max;
            ...
            if (eyespace_xyz_flipy.y > 0.0f)
            {
                _2754 = max(offset_z_max, offset_zzzz.w);
            }
            else
            {
                _2754 = offset_z_max;
...
float3 hrg_get_ideal_global_eye_pos(float3x3 local_to_global, float2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist)
{
    ...
    if (in_geom_mode < 2.5f)
    {
        _2810 = float3(0.0f, 0.0f, eye_z_spherical);
    }
    else
    {
        _2810 = float3(0.0f, 0.0f, max(in_geom_view_dist, eye_z_spherical));
    }
    float3 eye_pos = _2810;
    int num_points = 9;
    ...
    else
    {
        ...
        float3 _2970 = hrg_get_ideal_global_eye_pos_for_points(param_36, param_37, param_38, param_39, param_40, param_41);
        _2953 = _2970;
    }
    return _2953;
}
...
float2 hrg_intersect(float3 view_vec_local, float3 eye_pos_local, float in_geom_mode, float in_geom_radius)
{
    ...
    if (in_geom_mode < 2.5f)
    {
        ...
        float param_2 = in_geom_radius;
        _2409 = hrg_intersect_sphere(param, param_1, param_2);
    }
    else
    {
        ...
        float param_5 = in_geom_radius;
        _2409 = hrg_intersect_cylinder(param_3, param_4, param_5);
    }
    return _2409;
}
...
float2 hrg_xyz_to_uv(float3 intersection_pos_local, float2 output_aspect, float in_geom_mode, float in_geom_radius)
{
    ...
    if (in_geom_mode < 1.5f)
    {
        ...
        float2 param_1 = output_aspect;
        float param_2 = in_geom_radius;
        _2433 = hrg_sphere_xyz_to_uv(param, param_1, param_2);
    }
    else
    {
        float2 _2446;
        if (in_geom_mode < 2.5f)
        {
            ...
            float2 param_4 = output_aspect;
            float param_5 = in_geom_radius;
            _2446 = hrg_sphere_alt_xyz_to_uv(param_3, param_4, param_5);
        }
        else
        {
            ...
            float2 param_7 = output_aspect;
            float param_8 = in_geom_radius;
            _2446 = hrg_cylinder_xyz_to_uv(param_6, param_7, param_8);
        }
        _2433 = _2446;
    }
    return _2433;
...
float2 hrg_view_vec_to_uv(float3 view_vec_local, float3 eye_pos_local, float2 output_aspect, float in_geom_mode, float in_geom_radius, out float3 intersection_pos)
{
    ...
    if (intersect_dist_and_discriminant.y > 0.004999999888241291046142578125f)
    {
        ...
        float param_6 = in_geom_mode;
        float param_7 = in_geom_radius;
        _2527 = hrg_xyz_to_uv(param_4, param_5, param_6, param_7);
    }
    else
    {
        _2527 = 1.0f.xx;
...
float2 hrg_get_curved_video_uv_coords_and_tangent_matrix(float2 flat_video_uv, float3 eye_pos_local, float2 output_pixel_size, float2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, float3x3 global_to_local, out float2x2 pixel_to_tangent_video_uv)
{
    ...
    float3 param_5;
    float2 _3195 = hrg_view_vec_to_uv(param, param_1, param_2, param_3, param_4, param_5);
    float3 pos = param_5;
    float2 centered_uv = _3195;
    ...
    if (geom_force_correct_tangent_matrix)
    {
        ...
        {
            _3207 = float3(pos.x, 0.0f, pos.z);
        }
        float3 normal_base = _3207;
...
float2 HRG_GetGeomCurvedCoord(float2 in_coord, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, float in_geom_tilt_angle_x, float in_geom_tilt_angle_y, float in_screen_aspect, float pin_inner_edge, float2 in_source_size, float2 in_output_size, out float2x2 pixel_to_video_uv)
{
    ...
    if (in_geom_mode > 0.5f)
    {
        ...
        float2x2 param_14;
        float2 _3375 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14);
        pixel_to_video_uv = param_14;
        float2 ctr_curved_coord = _3375 - 0.5f.xx;
        if ((in_geom_tilt_angle_x == 0.0f) && (in_geom_tilt_angle_y == 0.0f))
        {
            ...
            float2x2 param_23;
            float2 _3404 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_15, param_16, param_17, param_18, param_19, param_20, param_21, param_22, param_23);
            pixel_to_video_uv = param_23;
            float2 right_edge_curved_ctr_coord = _3404 - 0.5f.xx;
            ...
            float2x2 param_32;
            float2 _3426 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_24, param_25, param_26, param_27, param_28, param_29, param_30, param_31, param_32);
            pixel_to_video_uv = param_32;
            float2 bottom_edge_curved_ctr_coord = _3426 - 0.5f.xx;
            ctr_curved_coord.x = (ctr_curved_coord.x * 0.5f) / right_edge_curved_ctr_coord.x;
        ...
        if (pin_inner_edge == 1.0f)
        {
            if (in_geom_tilt_angle_y != 0.0f)
            {
                ...
                float2x2 param_41;
                float2 _3469 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_33, param_34, param_35, param_36, param_37, param_38, param_39, param_40, param_41);
                pixel_to_video_uv = param_41;
                float2 top_edge_curved_ctr_coord = _3469;
            ...
            if (in_geom_tilt_angle_x != 0.0f)
            {
                ...
                float2x2 param_50;
                float2 _3498 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_42, param_43, param_44, param_45, param_46, param_47, param_48, param_49, param_50);
                pixel_to_video_uv = param_50;
                float2 left_edge_curved_ctr_coord = _3498;
...
float2 HSM_GetCurvatureValues(float screen_aspect)
{
    ...
    {
        _3631 = float2((((2.0f * global_HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f, ((global_HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f);
    }
    else
    {
        _3631 = float2(((global_HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f, (((2.0f * global_HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f);
    }
    float2 curvature_values = _3631;
...
float2 HSM_GetCrtPiCurvedCoord(inout float2 in_coord, inout float2 in_curvature)
{
    ...
    {
        _4135 = in_coord.y * in_coord.y;
    }
    float rsq = (in_coord.x * in_coord.x) + _4135;
...
float2 HSM_Get2DCurvedCoord(float2 in_coord, float2 curvature_values)
{
    ...
    float2 _4166 = HSM_GetCrtPiCurvedCoord(param, param_1);
    ctr_curved_coord = _4166 - 0.5f.xx;
    float2 param_2 = float2(1.0f, 0.5f);
    float2 param_3 = curvature_values;
    float2 _4173 = HSM_GetCrtPiCurvedCoord(param_2, param_3);
    float2 right_edge_curved_ctr_coord = _4173 - 0.5f.xx;
    ctr_curved_coord.x = (ctr_curved_coord.x * 0.5f) / right_edge_curved_ctr_coord.x;
    float2 param_4 = float2(0.5f, 1.0f);
    float2 param_5 = curvature_values;
    float2 _4187 = HSM_GetCrtPiCurvedCoord(param_4, param_5);
    float2 bottom_edge_curved_ctr_coord = _4187 - 0.5f.xx;
    ctr_curved_coord.y = (ctr_curved_coord.y * 0.5f) / bottom_edge_curved_ctr_coord.y;
    return ctr_curved_coord + 0.5f.xx;
}
...
float2 HSM_GetCurvedCoord(float2 in_coord, float curvature_multiplier, float screen_aspect)
{
    ...
    if (HSM_CURVATURE_MODE > CURVATURE_MODE_2D_CYLINDER)
    {
        ...
        else
        {
            ...
            float2x2 param_16;
            float2 _4294 = HRG_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14, param_15, param_16);
...
float2 HSM_GetTubeCurvedCoord(float2 screen_coord, float curvature_scale, float2 screen_scale, float2 tube_scale, float screen_aspect, float apply_black_edge_offset)
{
    ...
    if (global_HSM_BZL_USE_INDEPENDENT_CURVATURE == 1.0f)
    {
        ...
        if (screen_aspect < 1.0f)
        {
            _4489 = float2((((2.0f * global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f, ((global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f);
        }
        else
        {
            _4489 = float2(((global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f, (((2.0f * global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f);
        }
        float2 curvature_values = _4489;
...
float HSM_GetBezelCoords(float2 tube_diffuse_coord, float2 tube_diffuse_scale, float2 tube_scale, float screen_aspect, bool curve_coords_on, out float2 bezel_outside_scale, inout float2 bezel_outside_coord, inout float2 bezel_outside_curved_coord, inout float2 frame_outside_curved_coord)
{
    ...
    if (curve_coords_on)
    {
        bool _4692 = HSM_CURVATURE_MODE > 0.0f;
        bool _4700;
        if (_4692)
        {
            _4700 = (HSM_CURVATURE_3D_TILT_ANGLE_X != 0.0f) || (HSM_CURVATURE_3D_TILT_ANGLE_Y != 0.0f);
        }
        else
        {
            _4700 = _4692;
...
{
    bool _1628 = abs(in_coord.x - 0.5f) > 0.500999987125396728515625f;
    bool _1638;
    if (!_1628)
    {
        _1638 = abs(in_coord.y - 0.5f) > 0.500999987125396728515625f;
    }
    else
    {
        _1638 = _1628;
...
float2 HSM_GetMirrorWrappedCoord(inout float2 in_coord)
{
    ...
    bool _4356 = abs(ctr_coord.x) > 0.5f;
    bool _4364;
    if (!_4356)
    {
        _4364 = abs(ctr_coord.y) > 0.5f;
    }
    else
    {
        _4364 = _4356;
...
float4 HSM_GetCroppedTexSample(Texture2D<float4> in_sampler, SamplerState _in_sampler_sampler, float2 in_screen_coord)
{
    ...
    float4 _4563 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler, _in_sampler_sampler, param, param_1, param_2);
    return _4563;
}
...
float SlotMask(inout float2 pos, float m)
{
    ...
    else
    {
        ...
        {
            bool _6098 = py == global_double_slot;
            bool _6105;
            if (_6098)
            {
                _6105 = px >= global_slotwidth;
            }
            else
            {
                _6105 = _6098;
...
float3 Mask(inout float2 pos, float mx, float mb, out float mask_average_luma)
{
    ...
    bool _4887 = frac(abs(global_mshift)) > 0.25f;
    bool _4895;
    if (_4887)
    {
        _4895 = abs(global_mshift) > 1.25f;
    }
    else
    {
        _4895 = _4887;
    ...
    {
        _4909 = pos0.x + (next_line * global_mshift);
    }
    else
    {
        _4909 = pos0.x + (pos.y * global_mshift);
    }
    pos0.x = _4909;
    ...
    bool _4959 = global_shadowMask == 8.0f;
    bool _4967;
    if (!_4959)
    {
        _4967 = global_shadowMask == 11.0f;
    }
    else
    {
        _4967 = _4959;
...
void frag_main()
{
    ...
    {
        _711 = global_HSM_SNAP_TO_CLOSEST_INT_SCALE_TOLERANCE / 100.0f;
    }
    else
    {
        _711 = 0.0f;
    }
    HSM_SNAP_TO_CLOSEST_INT_SCALE_TOLERANCE = _711;
    ...
    float4 _6543 = HSM_UpdateGlobalScreenValuesFromCache(InfoCachePass, _InfoCachePass_sampler, param_1);
    ...
    float param_5 = SCREEN_ASPECT;
    bool param_6 = false;
    float2 param_7 = BEZEL_OUTSIDE_SCALE;
    float2 param_8 = BEZEL_OUTSIDE_COORD;
    float2 param_9 = BEZEL_OUTSIDE_CURVED_COORD;
    float2 param_10 = FRAME_OUTSIDE_CURVED_COORD;
    float _6567 = HSM_GetBezelCoords(param_2, param_3, param_4, param_5, param_6, param_7, param_8, param_9, param_10);
    ...
    cache_bounds_coord = ((BEZEL_OUTSIDE_COORD - 0.5f.xx) * 0.89999997615814208984375f) + 0.5f.xx;
    float2 param_11 = cache_bounds_coord;
    if (HHLP_IsOutsideCoordSpace(param_11))
    {
        FragColor = 0.0f.xxxx;
        return;
    }
    float lum = AvgLumPass.Sample(_AvgLumPass_sampler, 0.5f.xx).w;
    ...
    float2 _6642 = HSM_GetMirrorWrappedCoord(param_16);
    float2 pos = _6642;
    float2 param_17 = pos;
    ...
    if (global_GDV_RASTER_BLOOM_ON > 0.5f)
    {
        ...
        float2 param_35 = screen_curved_coord_with_overscan;
        if (HHLP_IsOutsideCoordSpace(param_35))
        {
            FragColor = 0.0f.xxxx;
            return;
        }
    ...
    float2 param_36 = pos;
    float3 Bloom = HSM_GetCroppedTexSample(BloomPass, _BloomPass_sampler, param_36).xyz;
    float2 param_37 = ((screen_curved_coord - 0.5f.xx) * 0.999000012874603271484375f) + 0.5f.xx;
    float param_38 = SCREEN_ASPECT;
    float param_39 = global_HSM_GLOBAL_CORNER_RADIUS * (global_HSM_SCREEN_CORNER_RADIUS_SCALE / 100.0f);
    float param_40 = 0.89999997615814208984375f;
    float screen_mask = HSM_GetCornerMask(param_37, param_38, param_39, param_40);
    bool is_inside_screen = screen_mask > 0.001000000047497451305389404296875f;
    bool _6772;
    if (is_inside_screen)
    {
        _6772 = global_GDV_DECONVERGENCE_ON > 0.5f;
    }
    else
    {
        _6772 = is_inside_screen;
    }
    bool _6799;
    if (_6772)
    {
        _6799 = (((((abs(global_deconrr) + abs(global_deconrry)) + abs(global_deconrg)) + abs(global_deconrgy)) + abs(global_deconrb)) + abs(global_deconrby)) > 0.20000000298023223876953125f;
    }
    else
    {
        _6799 = _6772;
    ...
    if (_6799)
    {
        float3 param_41 = color;
        float3 param_42 = Bloom;
        float2 param_43 = pos1;
        float2 param_44 = pos;
        fetch_pixel(param_41, param_42, param_43, param_44);
        color = param_41;
        Bloom = param_42;
    }
    float param_45 = max(max(color.x, color.y), color.z);
    float cm = igc(param_45);
    ...
    float2 param_46 = maskcoord;
    float param_47 = mx;
    float _6937 = SlotMask(param_46, param_47);
    float smask = _6937;
    ...
    float2 param_48 = maskcoord;
    float param_49 = mx;
    float param_50 = mb;
    float param_51 = mask_average_luma;
    float3 _6965 = Mask(param_48, param_49, param_50, param_51);
    mask_average_luma = param_51;
    cmask *= _6965;
    if (!is_inside_screen)
    {
        float2 param_52 = 0.5f.xx;
        float param_53 = mx;
        smask = HSM_SlotMaskAverage(param_52, param_53);
        smask = clamp(smask + lerp(global_smask_mit, 0.0f, min(w3, sqrt(max(max(orig1.x, orig1.y), orig1.z)))), 0.0f, 1.0f);
    ...
    if (interb)
    {
        ...
        {
            _7089 = pow(0.800000011920928955078125f * bb, 0.64999997615814208984375f);
        }
        else
        {
            _7089 = pow(bb, 0.699999988079071044921875f);
        }
        bb = _7089;
    }
    color *= bb;
    float2 param_54 = pos;
    float3 Ref = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_54).xyz;
    float2 param_55 = pos;
    float3 Glow = HSM_GetCroppedTexSample(GlowPass, _GlowPass_sampler, param_55).xyz;
    float2 param_56 = pos;
    float maxb = HSM_GetCroppedTexSample(BloomPass, _BloomPass_sampler, param_56).w;
    float3 Bloom1 = Bloom;
    if (abs(global_bloom) > 0.02500000037252902984619140625f)
    {
        if (global_bloom < (-0.00999999977648258209228515625f))
        {
            float3 param_57 = Bloom;
            float param_58 = maxb;
            Bloom1 = plant(param_57, param_58);
        }
    ...
    if (!interb)
    {
        ...
        float param_60 = lerp(1.0f, w3, 0.60000002384185791015625f);
        float3 _7220 = declip(param_59, param_60);
        color = _7220;
    }
    else
    {
    ...
    else
    {
        if (global_halation < (-0.00999999977648258209228515625f))
        {
            ...
            float3 param_61 = ((Bloom + Ref) + orig1) + ((Bloom * Bloom) * Bloom);
            float param_62 = min(mbl * mbl, 0.75f);
            Bloom = plant(param_61, param_62);
    ...
    else
    {
        ...
        float3 param_63 = orig1 + (Ref * 0.001000000047497451305389404296875f);
        float param_64 = 1.0f;
        orig1 = plant(param_63, param_64);
        float3 param_65 = Glow;
        float param_66 = 1.0f;
        Bloom = plant(param_65, param_66);
        Ref = abs(orig1 - Bloom);
    ...
    bool _7493 = global_glow >= 0.0f;
    bool _7499;
    if (_7493)
    {
        _7499 = global_m_glow < 0.5f;
    }
    else
    {
        _7499 = _7493;
    ...
    if (global_GDV_NOISE_ON > 0.5f)
    {
        ...
        {
            _7562 = global_noiseresd * global_noiseresd4kmult;
        }
        else
        {
            _7562 = global_noiseresd;
        }
        float noise_res = _7562;
        bool _7578 = global_noiseresd4kmult > 1.0f;
        bool _7585;
        if (_7578)
        {
            _7585 = global_FinalViewportSize.y > 4200.0f;
        }
        else
        {
            _7585 = _7578;
        }
        noise_res *= float(_7585 ? 2 : 1);
        float3 param_67 = float3(floor((global_OutputSize.xy * vTexCoord) / noise_res.xx), float(global_FrameCount));
        float3 _7607 = _noise(param_67);
        float3 noise0 = _7607;
    ...
    float adjusted_bardir = _7642;
    float param_68 = lerp(pos.y, pos.x, adjusted_bardir);
    float _7657 = humbar(param_68);
    color *= _7657;
    FragColor = float4(color, 1.0f);
    float4 param_69 = FragColor;
    float param_70 = 2.2000000476837158203125f;
    FragColor = HSM_Linearize(param_69, param_70);
}
