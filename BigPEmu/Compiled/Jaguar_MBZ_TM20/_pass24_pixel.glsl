...
layout(binding = 2) uniform sampler2D CRTPass;
layout(binding = 3) uniform sampler2D PostCRTPass;
...
void main()
{
    ...
    {
        _305 = global.HSM_SNAP_TO_CLOSEST_INT_SCALE_TOLERANCE / 100.0;
    }
    else
    {
        _305 = 0.0;
    ...
    vec4 crt_rgba = textureLod(CRTPass, vTexCoord, 2.0);
    vec4 post_crt_rgba = textureLod(PostCRTPass, vTexCoord, 2.0);
    FragColor = mix(crt_rgba, post_crt_rgba, vec4(HSM_REFLECT_SHOW_TUBE_FX_AMOUNT));
    vec4 param = FragColor;
    float param_1 = 0.86666667461395263671875;
