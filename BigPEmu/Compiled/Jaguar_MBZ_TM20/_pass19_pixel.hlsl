...
{
    bool _1593 = abs(in_coord.x - 0.5f) > 0.500999987125396728515625f;
    bool _1603;
    if (!_1593)
    {
        _1603 = abs(in_coord.y - 0.5f) > 0.500999987125396728515625f;
    }
    else
    {
        _1603 = _1593;
...
float4 HSM_GetCroppedTexSample(Texture2D<float4> in_sampler, SamplerState _in_sampler_sampler, float2 in_screen_coord)
{
    ...
    float4 _4530 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler, _in_sampler_sampler, param, param_1, param_2);
    return _4530;
}
...
float3 maxscale(float2 sinangle, float2 cosangle, float in_radius, float in_distance, float in_aspect)
{
    ...
    float2 _3887 = fwtrans(param_5, param_6, param_7, param_8, param_9);
    ...
    float2 _3904 = fwtrans(param_10, param_11, param_12, param_13, param_14);
    float2 lo = float2(_3887.x, _3904.y) / aspect_vec2;
    ...
    float2 _3924 = fwtrans(param_15, param_16, param_17, param_18, param_19);
    ...
    float2 _3940 = fwtrans(param_20, param_21, param_22, param_23, param_24);
    float2 hi = float2(_3924.x, _3940.y) / aspect_vec2;
...
float2 HSM_GetGeomCurvedCoord(float2 in_coord, float tilt_x, float tilt_y, float in_radius, inout float in_distance, float in_aspect)
{
    ...
    float2 _4037 = transform(param_5, param_6, param_7, param_8, param_9, param_10, param_11);
    float2 curved_coord = _4037;
    return curved_coord;
}
...
float2 HSM_GetGeomCurvedCoordRetainWidth(float2 in_coord, float tilt_x, float tilt_y, float in_radius, float in_distance, float in_aspect)
{
    ...
    float2 _4054 = HSM_GetGeomCurvedCoord(param, param_1, param_2, param_3, param_4, param_5);
    float2 ctr_curved_coord = _4054 - 0.5f.xx;
    ...
    float2 _4069 = HSM_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11);
    float2 right_edge_curved_ctr_coord = _4069 - 0.5f.xx;
...
float3 hrg_get_ideal_global_eye_pos_for_points(inout float3 eye_pos, float2 output_aspect, float3 global_coords[9], int num_points, float in_geom_radius, float in_geom_view_dist)
{
    ...
    for (int iter = 0; iter < max_centering_iters; iter++)
    {
        ...
        float3 _2615 = eye_pos;
        float2 _2617 = _2615.xy + center_offset;
        eye_pos.x = _2617.x;
        eye_pos.y = _2617.y;
...
float3 hrg_get_ideal_global_eye_pos(float3x3 local_to_global, float2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist)
{
    ...
    if (in_geom_mode < 2.5f)
    {
        _2776 = float3(0.0f, 0.0f, eye_z_spherical);
    }
    else
    {
        _2776 = float3(0.0f, 0.0f, max(in_geom_view_dist, eye_z_spherical));
    }
    float3 eye_pos = _2776;
    int num_points = 9;
    ...
    else
    {
        ...
        float3 _2936 = hrg_get_ideal_global_eye_pos_for_points(param_36, param_37, param_38, param_39, param_40, param_41);
        _2919 = _2936;
    }
    return _2919;
}
...
float2 hrg_get_curved_video_uv_coords_and_tangent_matrix(float2 flat_video_uv, float3 eye_pos_local, float2 output_pixel_size, float2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, float3x3 global_to_local, out float2x2 pixel_to_tangent_video_uv)
{
    ...
    float3 param_5;
    float2 _3161 = hrg_view_vec_to_uv(param, param_1, param_2, param_3, param_4, param_5);
    float3 pos = param_5;
    float2 centered_uv = _3161;
    ...
    if (geom_force_correct_tangent_matrix)
    {
        ...
        {
            _3174 = float3(pos.x, 0.0f, pos.z);
        }
        float3 normal_base = _3174;
...
float2 HRG_GetGeomCurvedCoord(float2 in_coord, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, float in_geom_tilt_angle_x, float in_geom_tilt_angle_y, float in_screen_aspect, float pin_inner_edge, float2 in_source_size, float2 in_output_size, out float2x2 pixel_to_video_uv)
{
    ...
    if (in_geom_mode > 0.5f)
    {
        ...
        float2x2 param_14;
        float2 _3342 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14);
        pixel_to_video_uv = param_14;
        float2 ctr_curved_coord = _3342 - 0.5f.xx;
        if ((in_geom_tilt_angle_x == 0.0f) && (in_geom_tilt_angle_y == 0.0f))
        {
            ...
            float2x2 param_23;
            float2 _3371 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_15, param_16, param_17, param_18, param_19, param_20, param_21, param_22, param_23);
            pixel_to_video_uv = param_23;
            float2 right_edge_curved_ctr_coord = _3371 - 0.5f.xx;
            ...
            float2x2 param_32;
            float2 _3393 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_24, param_25, param_26, param_27, param_28, param_29, param_30, param_31, param_32);
            pixel_to_video_uv = param_32;
            float2 bottom_edge_curved_ctr_coord = _3393 - 0.5f.xx;
            ctr_curved_coord.x = (ctr_curved_coord.x * 0.5f) / right_edge_curved_ctr_coord.x;
        ...
        if (pin_inner_edge == 1.0f)
        {
            if (in_geom_tilt_angle_y != 0.0f)
            {
                ...
                float2x2 param_41;
                float2 _3436 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_33, param_34, param_35, param_36, param_37, param_38, param_39, param_40, param_41);
                pixel_to_video_uv = param_41;
                float2 top_edge_curved_ctr_coord = _3436;
            ...
            if (in_geom_tilt_angle_x != 0.0f)
            {
                ...
                float2x2 param_50;
                float2 _3465 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_42, param_43, param_44, param_45, param_46, param_47, param_48, param_49, param_50);
                pixel_to_video_uv = param_50;
                float2 left_edge_curved_ctr_coord = _3465;
...
float2 HSM_GetCurvatureValues(float screen_aspect)
{
    ...
    {
        _3598 = float2((((2.0f * global_HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f, ((global_HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f);
    }
    else
    {
        _3598 = float2(((global_HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f, (((2.0f * global_HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f);
    }
    float2 curvature_values = _3598;
...
float2 HSM_Get2DCurvedCoord(float2 in_coord, float2 curvature_values)
{
    ...
    float2 _4133 = HSM_GetCrtPiCurvedCoord(param, param_1);
    ctr_curved_coord = _4133 - 0.5f.xx;
    float2 param_2 = float2(1.0f, 0.5f);
    float2 param_3 = curvature_values;
    float2 _4140 = HSM_GetCrtPiCurvedCoord(param_2, param_3);
    float2 right_edge_curved_ctr_coord = _4140 - 0.5f.xx;
    ctr_curved_coord.x = (ctr_curved_coord.x * 0.5f) / right_edge_curved_ctr_coord.x;
    float2 param_4 = float2(0.5f, 1.0f);
    float2 param_5 = curvature_values;
    float2 _4154 = HSM_GetCrtPiCurvedCoord(param_4, param_5);
    float2 bottom_edge_curved_ctr_coord = _4154 - 0.5f.xx;
    ctr_curved_coord.y = (ctr_curved_coord.y * 0.5f) / bottom_edge_curved_ctr_coord.y;
    return ctr_curved_coord + 0.5f.xx;
}
...
float2 HSM_GetCurvedCoord(float2 in_coord, float curvature_multiplier, float screen_aspect)
{
    ...
    if (HSM_CURVATURE_MODE > CURVATURE_MODE_2D_CYLINDER)
    {
        ...
        else
        {
            ...
            float2x2 param_16;
            float2 _4261 = HRG_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14, param_15, param_16);
...
{
    float2 ctr_coord = in_coord - 0.5f.xx;
    bool _4323 = abs(ctr_coord.x) > 0.5f;
    bool _4331;
    if (!_4323)
    {
        _4331 = abs(ctr_coord.y) > 0.5f;
    }
    else
    {
        _4331 = _4323;
...
float sw0(float x, float color, float scanline)
{
    ...
    {
        _4940 = lerp(ex * ex, (ex * ex) * ex, 0.4000000059604644775390625f);
    }
    ex = _4940;
...
bool HSM_GetUseFakeScanlines()
{
    ...
    bool _4549 = (global_HSM_FAKE_SCANLINE_OPACITY / 100.0f) > 0.001000000047497451305389404296875f;
    bool _4571;
    if (_4549)
    {
        bool _4555 = global_HSM_FAKE_SCANLINE_MODE == 1.0f;
        bool _4570;
        if (!_4555)
        {
            bool _4561 = global_HSM_FAKE_SCANLINE_MODE == 2.0f;
            bool _4569;
            if (_4561)
            {
                _4569 = scane_axis_core_res > global_HSM_INTERLACE_TRIGGER_RES;
            }
            else
            {
                _4569 = _4561;
...
float2 HSM_GetTubeCurvedCoord(float2 screen_coord, float curvature_scale, float2 screen_scale, float2 tube_scale, float screen_aspect, float apply_black_edge_offset)
{
    ...
    if (global_HSM_BZL_USE_INDEPENDENT_CURVATURE == 1.0f)
    {
        ...
        if (screen_aspect < 1.0f)
        {
            _4456 = float2((((2.0f * global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f, ((global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f);
        }
        else
        {
            _4456 = float2(((global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f, (((2.0f * global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f);
        }
        float2 curvature_values = _4456;
...
float4 HSM_ApplyScanlineMask(float4 in_color, float2 screen_scale, inout float2 in_coord, float2 in_screen_curved_coord, float2 in_tube_curved_coord, float in_scanline_opacity)
{
    ...
    {
        _4646 = global_HSM_FAKE_SCANLINE_RES;
    }
    else
    {
...
void frag_main()
{
    ...
    {
        _691 = global_HSM_SNAP_TO_CLOSEST_INT_SCALE_TOLERANCE / 100.0f;
    }
    else
    {
        _691 = 0.0f;
    ...
    float4 _5065 = HSM_UpdateGlobalScreenValuesFromCache(InfoCachePass, _InfoCachePass_sampler, param_1);
    float2 cache_bounds_coord = SCREEN_COORD;
    float2 param_2 = TUBE_DIFFUSE_COORD;
    float2 param_3 = TUBE_DIFFUSE_SCALE;
    float2 param_4 = TUBE_SCALE;
    float param_5 = SCREEN_ASPECT;
    float2 bezel_outside_flat_coord;
    float2 param_6 = bezel_outside_flat_coord;
    float2 frame_outside_flat_coord;
    float2 param_7 = frame_outside_flat_coord;
    ...
    float2 param_8 = cache_bounds_coord;
    ...
    float2 param_9 = 0.5f.xx;
    float lum = HSM_GetCroppedTexSample(AvgLumPass, _AvgLumPass_sampler, param_9).w;
    float2 param_10 = 0.25f.xx;
    float gamma_in = 1.0f / HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_10).w;
    float2 param_11 = float2(0.75f, 0.25f);
    float intera = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_11).w;
    ...
    float2 param_12 = SCREEN_COORD;
    float2 screen_curved_coord = HSM_GetCRTShaderCurvedCoord(param_12);
    float2 param_13 = screen_curved_coord;
    float param_14 = (global_GDV_OVERSCAN / 100.0f) + (global_GDV_OVERSCAN_X / 100.0f);
    float param_15 = (global_GDV_OVERSCAN / 100.0f) + (global_GDV_OVERSCAN_Y / 100.0f);
    float2 screen_curved_coord_with_overscan = HSM_ApplyOverscan(param_13, param_14, param_15);
    float2 param_16 = screen_curved_coord;
    float2 _5172 = HSM_GetMirrorWrappedCoord(param_16);
    float2 pos = _5172;
    float2 param_17 = pos;
    float param_18 = (global_GDV_OVERSCAN / 100.0f) + (global_GDV_OVERSCAN_X / 100.0f);
    float param_19 = (global_GDV_OVERSCAN / 100.0f) + (global_GDV_OVERSCAN_Y / 100.0f);
    pos = HSM_ApplyOverscan(param_17, param_18, param_19);
    float2 screen_scale_with_overscan = SCREEN_SCALE * (float2((global_GDV_OVERSCAN / 100.0f) + (global_GDV_OVERSCAN_X / 100.0f), (global_GDV_OVERSCAN / 100.0f) + (global_GDV_OVERSCAN_Y / 100.0f)) + 1.0f.xx);
    float2 param_20 = SCREEN_COORD;
    float param_21 = (global_GDV_OVERSCAN / 100.0f) + (global_GDV_OVERSCAN_X / 100.0f);
    float param_22 = (global_GDV_OVERSCAN / 100.0f) + (global_GDV_OVERSCAN_Y / 100.0f);
    float2 screen_coord_with_overscan = HSM_ApplyOverscan(param_20, param_21, param_22);
    if (global_GDV_RASTER_BLOOM_ON > 0.5f)
    {
        float2 param_23 = pos;
        float param_24 = global_OS;
        float param_25 = global_BLOOM;
        float param_26 = lum;
        pos = HSM_ApplyRasterBloomOverscan(param_23, param_24, param_25, param_26);
        float2 param_27 = screen_curved_coord_with_overscan;
        float param_28 = global_OS;
        float param_29 = global_BLOOM;
        float param_30 = lum;
        screen_curved_coord_with_overscan = HSM_ApplyRasterBloomOverscan(param_27, param_28, param_29, param_30);
        float2 param_31 = screen_coord_with_overscan;
        float param_32 = global_OS;
        float param_33 = global_BLOOM;
        float param_34 = lum;
        screen_coord_with_overscan = HSM_ApplyRasterBloomOverscan(param_31, param_32, param_33, param_34);
        float param_35 = global_OS;
        float param_36 = global_BLOOM;
        float param_37 = lum;
        screen_scale_with_overscan *= HSM_GetRasterBloomScale(param_35, param_36, param_37);
        float2 param_38 = screen_curved_coord_with_overscan;
    ...
    if (smarte)
    {
        ...
        twr3 = 0.0f;
        float2 param_39 = pC4;
        c1 = HSM_GetCroppedTexSample(AvgLumPass, _AvgLumPass_sampler, param_39).xyz;
        float2 param_40 = pC4 + offy;
        c2 = HSM_GetCroppedTexSample(AvgLumPass, _AvgLumPass_sampler, param_40).xyz;
        c1 = max(c1 - global_sth.xxx, 0.0f.xxx);
        c2 = max(c2 - global_sth.xxx, 0.0f.xxx);
    }
    float2 param_41 = pC4 - off2;
    float3 l3 = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_41).xyz;
    float2 param_42 = pC4 - offx;
    float3 l2 = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_42).xyz;
    float2 param_43 = pC4;
    float3 l1 = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_43).xyz;
    float2 param_44 = pC4 + offx;
    float3 r1 = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_44).xyz;
    float2 param_45 = pC4 + off2;
    float3 r2 = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_45).xyz;
    float2 param_46 = (pC4 + offx) + off2;
    float3 r3 = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_46).xyz;
    ...
    {
        pC4 += offy;
        float2 param_47 = pC4 - off2;
        l3 = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_47).xyz;
        float2 param_48 = pC4 - offx;
        l2 = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_48).xyz;
        float2 param_49 = pC4;
        l1 = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_49).xyz;
        float2 param_50 = pC4 + offx;
        r1 = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_50).xyz;
        float2 param_51 = pC4 + off2;
        r2 = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_51).xyz;
        float2 param_52 = (pC4 + offx) + off2;
        r3 = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_52).xyz;
    ...
    if (!interb)
    {
        ...
        bool _6536 = global_HSM_INTERLACE_MODE < 0.0f;
        bool _6543;
        if (_6536)
        {
            _6543 = global_HSM_INTERLACE_TRIGGER_RES <= scan_res;
        }
        else
        {
            _6543 = _6536;
        ...
        {
            _6544 = global_no_scanlines;
        }
        float hsm_no_scanlines = _6544;
        float2 param_73 = ((screen_curved_coord - 0.5f.xx) * 0.999000012874603271484375f) + 0.5f.xx;
        float param_74 = SCREEN_ASPECT;
        float param_75 = global_HSM_GLOBAL_CORNER_RADIUS * (global_HSM_SCREEN_CORNER_RADIUS_SCALE / 100.0f);
        ...
        float3 param_77 = color1;
        float3 param_78 = color2;
        color = (((gc(param_77) * w1) * cd1) + ((gc(param_78) * w2) * cd2)) / lerp(1.0f.xxx, w1 + w2, hsm_no_scanlines.xxx);
        if (abs(global_rolling_scan) > 0.004999999888241291046142578125f)
        {
            float param_79 = f;
            wt1 = st1(param_79);
            float param_80 = 1.0f - f;
            wt2 = st1(param_80);
            color00 = ((color1 * wt1) + (color2 * wt2)) / (wt1 + wt2).xxx;
            float3 param_81 = color00;
            color = gc(param_81) * lerp(w1 + w2, w3.xxx, max(wf1, wf2).xxx);
        }
        color = min(color, 1.0f.xxx);
    }
    if (interb)
    {
        float3 param_82 = color1;
        color = gc(param_82);
    }
    ...
    {
        float2 param_83 = TUBE_DIFFUSE_COORD;
        float param_84 = 1.0f;
        float2 param_85 = TUBE_DIFFUSE_SCALE;
        float2 param_86 = TUBE_SCALE;
        float param_87 = TUBE_DIFFUSE_ASPECT;
        float param_88 = 1.0f;
        float2 tube_curved_coord = HSM_GetTubeCurvedCoord(param_83, param_84, param_85, param_86, param_87, param_88);
        float4 param_89 = FragColor;
        float2 param_90 = screen_scale_with_overscan;
        float2 param_91 = screen_coord_with_overscan;
        float2 param_92 = screen_curved_coord_with_overscan;
        float2 param_93 = tube_curved_coord;
        float param_94 = global_HSM_FAKE_SCANLINE_OPACITY / 100.0f;
        float4 _6701 = HSM_ApplyScanlineMask(param_89, param_90, param_91, param_92, param_93, param_94);
        FragColor = _6701;
    }
}
