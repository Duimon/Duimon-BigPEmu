...
bool HSM_GetIsInABCompareArea(vec2 viewport_coord)
{
    ...
    {
        _5966 = 1.0 - HSM_AB_COMPARE_SPLIT_POSITION;
    }
    else
    {
    ...
    bool _5980 = (mod(HSM_AB_COMPARE_AREA, 2.0) == 0.0) && (test_value < position);
    bool _5991;
    if (!_5980)
    {
        _5991 = (mod(HSM_AB_COMPARE_AREA, 2.0) == 1.0) && (test_value > position);
    }
    else
    {
        _5991 = _5980;
...
vec2 GetSimpleImageScaledCoord(inout vec2 in_viewport_coord, vec2 in_viewport_unscaled_coord, vec2 in_tube_coord, vec2 in_tube_scale, sampler2D in_sampler, vec2 in_pos, float in_inherit_pos, vec2 in_scale, float in_scale_inherit_mode, float in_keep_aspect, float in_mirror_horz, float in_rotate)
{
    ...
    if (in_rotate > 0.5)
    {
        _5042 = global.FinalViewportSize.x / global.FinalViewportSize.y;
    }
    else
    {
        _5042 = 1.0;
    }
    coord_ctr /= vec2(_5042);
    if (in_rotate > 0.5)
    {
...
vec3 HSM_RGBtoHSV(vec3 c)
{
    ...
    if (c.y < c.z)
    {
        _2613 = vec4(c.zy, K.wz);
    }
    else
    {
        _2613 = vec4(c.yz, K.xy);
    }
    vec4 p = _2613;
    vec4 _2643;
    if (c.x < p.x)
    {
        _2643 = vec4(p.xyw, c.x);
    }
    else
    {
        _2643 = vec4(c.x, p.yzx);
    }
    vec4 q = _2643;
...
vec4 HSM_GetNightLightingMultiplyColor(vec2 in_coord, float hue, float saturation, float value, float contrast, float global_ambient_opacity, sampler2D NightLightingImage_1)
{
    ...
    vec4 _6126 = lighting_image;
    vec3 _6130 = mix(vec3(1.0), _6126.xyz, vec3(global_ambient_opacity));
...
bool HSM_Fill_Ambient_Images(vec2 in_viewport_coord, vec2 in_viewport_unscaled_coord, vec2 in_tube_coord, vec2 in_tube_scale, float in_swap_images, sampler2D in_ambient_sampler, sampler2D in_ambient2_sampler, out vec4 ambient_lighting_image, out vec4 ambient2_lighting_image)
{
    ...
    if (HSM_AMBIENT1_OPACITY > 0.0)
    {
        ...
        bool _6148 = HSM_AMBIENT1_SCALE_INHERIT_MODE == 1.0;
        bool _6155;
        if (_6148)
        {
            _6155 = (HSM_AMBIENT1_SCALE * HSM_VIEWPORT_ZOOM) < 1.0;
        }
        else
        {
            _6155 = _6148;
        ...
        vec2 _6189 = GetSimpleImageScaledCoord(param, param_1, param_2, param_3, in_ambient_sampler, param_4, param_5, param_6, param_7, param_8, param_9, param_10);
        vec2 lighting_coord = _6189;
    ...
    if (HSM_AMBIENT2_OPACITY > 0.0)
    {
        ...
        bool _6210 = HSM_AMBIENT2_SCALE_INHERIT_MODE == 1.0;
        bool _6217;
        if (_6210)
        {
            _6217 = (HSM_AMBIENT2_SCALE * HSM_VIEWPORT_ZOOM) < 1.0;
        }
        else
        {
            _6217 = _6210;
        ...
        vec2 _6251 = GetSimpleImageScaledCoord(param_17, param_18, param_19, param_20, in_ambient2_sampler, param_21, param_22, param_23, param_24, param_25, param_26, param_27);
        vec2 lighting2_coord = _6251;
...
vec2 HSM_GetGeomCurvedCoord(vec2 in_coord, float tilt_x, float tilt_y, float in_radius, inout float in_distance, float in_aspect)
{
    ...
    vec2 _5518 = transform(param_5, param_6, param_7, param_8, param_9, param_10, param_11);
    vec2 curved_coord = _5518;
    return curved_coord;
}
...
vec2 HSM_GetGeomCurvedCoordRetainWidth(vec2 in_coord, float tilt_x, float tilt_y, float in_radius, float in_distance, float in_aspect)
{
    ...
    vec2 _5535 = HSM_GetGeomCurvedCoord(param, param_1, param_2, param_3, param_4, param_5);
    vec2 ctr_curved_coord = _5535 - vec2(0.5);
    ...
    vec2 _5550 = HSM_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11);
    vec2 right_edge_curved_ctr_coord = _5550 - vec2(0.5);
...
vec3 hrg_get_ideal_global_eye_pos_for_points(inout vec3 eye_pos, vec2 output_aspect, vec3 global_coords[9], int num_points, float in_geom_radius, float in_geom_view_dist)
{
    ...
    for (int iter = 0; iter < max_centering_iters; iter++)
    {
        ...
        vec3 _3947 = eye_pos;
        vec2 _3949 = _3947.xy + center_offset;
        eye_pos.x = _3949.x;
        eye_pos.y = _3949.y;
...
vec3 hrg_get_ideal_global_eye_pos(mat3 local_to_global, vec2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist)
{
    ...
    if (in_geom_mode < 2.5)
    {
        _4108 = vec3(0.0, 0.0, eye_z_spherical);
    }
    else
    {
        _4108 = vec3(0.0, 0.0, max(in_geom_view_dist, eye_z_spherical));
    }
    vec3 eye_pos = _4108;
    int num_points = 9;
    vec2 param = vec2(0.0);
    ...
    else
    {
        ...
        vec3 _4268 = hrg_get_ideal_global_eye_pos_for_points(param_36, param_37, param_38, param_39, param_40, param_41);
        _4251 = _4268;
    }
    return _4251;
}
...
vec2 hrg_get_curved_video_uv_coords_and_tangent_matrix(vec2 flat_video_uv, vec3 eye_pos_local, vec2 output_pixel_size, vec2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, mat3 global_to_local, out mat2 pixel_to_tangent_video_uv)
{
    ...
    vec3 param_5;
    vec2 _4493 = hrg_view_vec_to_uv(param, param_1, param_2, param_3, param_4, param_5);
    vec3 pos = param_5;
    vec2 centered_uv = _4493;
    ...
    if (geom_force_correct_tangent_matrix)
    {
        ...
        {
            _4505 = vec3(pos.x, 0.0, pos.z);
        }
        vec3 normal_base = _4505;
...
vec2 HRG_GetGeomCurvedCoord(vec2 in_coord, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, float in_geom_tilt_angle_x, float in_geom_tilt_angle_y, float in_screen_aspect, float pin_inner_edge, vec2 in_source_size, vec2 in_output_size, out mat2 pixel_to_video_uv)
{
    ...
    if (in_geom_mode > 0.5)
    {
        ...
        mat2 param_14;
        vec2 _4673 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14);
        pixel_to_video_uv = param_14;
        vec2 ctr_curved_coord = _4673 - vec2(0.5);
        if ((in_geom_tilt_angle_x == 0.0) && (in_geom_tilt_angle_y == 0.0))
        {
            ...
            mat2 param_23;
            vec2 _4702 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_15, param_16, param_17, param_18, param_19, param_20, param_21, param_22, param_23);
            pixel_to_video_uv = param_23;
            vec2 right_edge_curved_ctr_coord = _4702 - vec2(0.5);
            ...
            mat2 param_32;
            vec2 _4724 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_24, param_25, param_26, param_27, param_28, param_29, param_30, param_31, param_32);
            pixel_to_video_uv = param_32;
            vec2 bottom_edge_curved_ctr_coord = _4724 - vec2(0.5);
            ctr_curved_coord.x = (ctr_curved_coord.x * 0.5) / right_edge_curved_ctr_coord.x;
        ...
        if (pin_inner_edge == 1.0)
        {
            if (in_geom_tilt_angle_y != 0.0)
            {
                ...
                mat2 param_41;
                vec2 _4767 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_33, param_34, param_35, param_36, param_37, param_38, param_39, param_40, param_41);
                pixel_to_video_uv = param_41;
                vec2 top_edge_curved_ctr_coord = _4767;
            ...
            if (in_geom_tilt_angle_x != 0.0)
            {
                ...
                mat2 param_50;
                vec2 _4796 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_42, param_43, param_44, param_45, param_46, param_47, param_48, param_49, param_50);
                pixel_to_video_uv = param_50;
                vec2 left_edge_curved_ctr_coord = _4796;
...
vec2 HSM_GetCurvatureValues(float screen_aspect)
{
    ...
    {
        _5080 = vec2((((2.0 * global.HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0) * 2.0) / 100.0, ((global.HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0) * 3.0) / 100.0);
    }
    else
    {
        _5080 = vec2(((global.HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0) * 3.0) / 100.0, (((2.0 * global.HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0) * 2.0) / 100.0);
    }
    vec2 curvature_values = _5080;
...
vec2 HSM_Get2DCurvedCoord(vec2 in_coord, vec2 curvature_values)
{
    ...
    vec2 _5614 = HSM_GetCrtPiCurvedCoord(param, param_1);
    ctr_curved_coord = _5614 - vec2(0.5);
    vec2 param_2 = vec2(1.0, 0.5);
    vec2 param_3 = curvature_values;
    vec2 _5621 = HSM_GetCrtPiCurvedCoord(param_2, param_3);
    vec2 right_edge_curved_ctr_coord = _5621 - vec2(0.5);
    ctr_curved_coord.x = (ctr_curved_coord.x * 0.5) / right_edge_curved_ctr_coord.x;
    vec2 param_4 = vec2(0.5, 1.0);
    vec2 param_5 = curvature_values;
    vec2 _5635 = HSM_GetCrtPiCurvedCoord(param_4, param_5);
    vec2 bottom_edge_curved_ctr_coord = _5635 - vec2(0.5);
    ctr_curved_coord.y = (ctr_curved_coord.y * 0.5) / bottom_edge_curved_ctr_coord.y;
    return ctr_curved_coord + vec2(0.5);
}
...
vec2 HSM_GetCurvedCoord(vec2 in_coord, float curvature_multiplier, float screen_aspect)
{
    ...
    if (HSM_CURVATURE_MODE > CURVATURE_MODE_2D_CYLINDER)
    {
        ...
        else
        {
            ...
            mat2 param_16;
            vec2 _5743 = HRG_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14, param_15, param_16);
...
vec2 HSM_GetTubeCurvedCoord(vec2 screen_coord, float curvature_scale, vec2 screen_scale, vec2 tube_scale, float screen_aspect, float apply_black_edge_offset)
{
    ...
    if (global.HSM_BZL_USE_INDEPENDENT_CURVATURE == 1.0)
    {
        ...
        if (screen_aspect < 1.0)
        {
            _5882 = vec2((((2.0 * global.HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0) * 2.0) / 100.0, ((global.HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0) * 3.0) / 100.0);
        }
        else
        {
            _5882 = vec2(((global.HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0) * 3.0) / 100.0, (((2.0 * global.HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0) * 2.0) / 100.0);
        }
        vec2 curvature_values = _5882;
...
float HSM_GetBezelCoords(vec2 tube_diffuse_coord, vec2 tube_diffuse_scale, vec2 tube_scale, float screen_aspect, bool curve_coords_on, out vec2 bezel_outside_scale, inout vec2 bezel_outside_coord, inout vec2 bezel_outside_curved_coord, inout vec2 frame_outside_curved_coord)
{
    ...
    if (curve_coords_on)
    {
        bool _6411 = HSM_CURVATURE_MODE > 0.0;
        bool _6419;
        if (_6411)
        {
            _6419 = (HSM_CURVATURE_3D_TILT_ANGLE_X != 0.0) || (HSM_CURVATURE_3D_TILT_ANGLE_Y != 0.0);
        }
        else
        {
            _6419 = _6411;
...
vec4 HSM_GetPreMultipliedColorLinear(vec4 in_color, float matte_type, float encoded_gamma)
{
    ...
    {
        vec4 _2819 = out_color;
        float _2822 = out_color.w;
        vec3 _2828 = clamp(_2819.xyz - vec3(1.0 - _2822), vec3(0.0), vec3(1.0));
        out_color.x = _2828.x;
        out_color.y = _2828.y;
        out_color.z = _2828.z;
    }
    ...
    if (matte_type == SOURCE_MATTE_NONE)
    {
        ...
        vec4 _2847 = out_color;
        vec3 _2849 = _2847.xyz * _2846;
        out_color.x = _2849.x;
        out_color.y = _2849.y;
        out_color.z = _2849.z;
    }
...
vec3 ApplyAmbientImages(vec3 base_image, inout vec3 ambient_image, inout vec3 ambient2_image, float blend_ambient, float blend_ambient2, float apply_in_add_mode, float layer_blend_mode, float swap_images)
{
    ...
    bool _6291 = (HSM_AMBIENT1_OPACITY > 0.0) || (HSM_AMBIENT2_OPACITY > 0.0);
    bool _6299;
    if (_6291)
    {
        _6299 = (blend_ambient > 0.0) || (blend_ambient2 > 0.0);
    }
    else
    {
        _6299 = _6291;
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    _6603 = (mask_mode == 8.0) ? OUTSIDE_FRAME_MASK : 0.5;
                                }
                                _6595 = _6603;
                            }
                            _6588 = _6595;
...
{
    bool _6624 = blend_mode == 0.0;
    bool _6634;
    if (!_6624)
    {
        _6634 = (dualscreen_mode != 0.0) && (dualscreen_mode != SCREEN_INDEX);
    }
    else
    {
        _6634 = _6624;
    ...
    if (blend_mode == BLEND_MODE_NORMAL)
    {
        ...
        vec4 _6677 = color_over;
        vec3 _6679 = _6677.xyz * _6676;
    ...
    else
    {
        ...
        {
            vec3 _6703 = color_under.xyz + color_over.xyz;
        ...
        {
            vec3 _6719 = color_under.xyz * color_over.xyz;
...
void main()
{
    ...
    bool _7032 = HSM_AB_COMPARE_FREEZE_GRAPHICS == 1.0;
    bool _7040;
    if (_7032)
    {
    ...
    vec4 _7059 = HSM_UpdateGlobalScreenValuesFromCache(InfoCachePass, param_3);
    vec4 feedback_color_test = texture(BR_LayersOverCRTPassFeedback, vec2(0.0));
    bool _7064 = HSM_CACHE_GRAPHICS_ON > 0.5;
    bool _7070;
    if (_7064)
    {
        _7070 = feedback_color_test.w < 0.0;
    }
    else
    {
        _7070 = _7064;
    ...
    bool _7099 = HSM_Fill_Ambient_Images(param_4, param_5, param_6, param_7, param_8, NightLightingImage, NightLighting2Image, param_9, param_10);
    ...
    bool _7322 = SCREEN_ASPECT < 1.0;
    bool _7331;
    if (_7322)
    {
        _7331 = textureSize(BackgroundVertImage, 0).y > 16;
    }
    else
    {
        _7331 = _7322;
    ...
    {
        _7336 = textureSize(BackgroundVertImage, 0);
    }
    else
    {
        _7336 = textureSize(BackgroundImage, 0);
    }
    vec2 bg_size = vec2(_7336);
    ...
    vec2 _7387 = HSM_GetScaledCoord(param_57, param_58, param_59, param_60, param_61, param_62, param_63, param_64, param_65, param_66, param_67, param_68, param_69, param_70, param_71);
    ...
    bool _7406 = HSM_BG_OPACITY > 0.0;
    bool _7413;
    if (_7406)
    {
        _7413 = bg_size.y > 16.0;
    }
    else
    {
        _7413 = _7406;
    ...
    if (_7413)
    {
        ...
        vec3 _7453 = HSM_ApplyHSVAdjustment(param_82, param_83, param_84, param_85, param_86, param_87);
        bg_image.x = _7453.x;
        bg_image.y = _7453.y;
        bg_image.z = _7453.z;
        ...
        vec3 _7488 = ApplyAmbientImages(param_90, param_91, param_92, param_93, param_94, param_95, param_96, param_97);
        bg_image.x = _7488.x;
        bg_image.y = _7488.y;
        bg_image.z = _7488.z;
    }
    bool _7497 = SCREEN_ASPECT < 1.0;
    bool _7506;
    if (_7497)
    {
        _7506 = textureSize(DeviceVertImage, 0).y > 16;
    }
    else
    {
        _7506 = _7497;
    ...
    {
        _7510 = textureSize(DeviceVertImage, 0);
    }
    else
    {
        _7510 = textureSize(DeviceImage, 0);
    }
    vec2 device_size = vec2(_7510);
    ...
    vec2 _7561 = HSM_GetScaledCoord(param_98, param_99, param_100, param_101, param_102, param_103, param_104, param_105, param_106, param_107, param_108, param_109, param_110, param_111, param_112);
    DEVICE_COORD = param_111;
    DEVICE_SCALE = param_112;
    DEVICE_CURVED_COORD = _7561;
    vec4 device_image = vec4(0.0);
    bool _7566 = HSM_DEVICE_OPACITY > 0.0;
    bool _7572;
    if (_7566)
    {
        _7572 = device_size.y > 16.0;
    }
    else
    {
        _7572 = _7566;
    ...
    if (_7572)
    {
        ...
        if (device_use_vert_image)
        {
            ...
            _7576 = HSM_GetMipmappedTexSample(DeviceVertImage, param_113, param_114, param_115);
        }
        else
        {
            ...
            _7576 = HSM_GetMipmappedTexSample(DeviceImage, param_116, param_117, param_118);
        }
        device_image = _7576;
        ...
        vec3 _7614 = HSM_ApplyHSVAdjustment(param_122, param_123, param_124, param_125, param_126, param_127);
        ...
        vec3 _7649 = ApplyAmbientImages(param_130, param_131, param_132, param_133, param_134, param_135, param_136, param_137);
    ...
    vec2 _7700 = HSM_GetScaledCoord(param_138, param_139, param_140, param_141, param_142, param_143, param_144, param_145, param_146, param_147, param_148, param_149, param_150, param_151, param_152);
    DEVICELED_COORD = param_151;
    DEVICELED_SCALE = param_152;
    DEVICELED_CURVED_COORD = _7700;
    vec4 deviceled_image = vec4(0.0);
    bool _7705 = HSM_DEVICELED_OPACITY > 0.0;
    bool _7711;
    if (_7705)
    {
        _7711 = deviceled_size.y > 16.0;
    }
    else
    {
        _7711 = _7705;
    ...
    if (_7711)
    {
        ...
        vec3 _7740 = HSM_ApplyHSVAdjustment(param_159, param_160, param_161, param_162, param_163, param_164);
        ...
        vec3 _7775 = ApplyAmbientImages(param_167, param_168, param_169, param_170, param_171, param_172, param_173, param_174);
    ...
    if (HSM_LED_OPACITY > 0.0)
    {
        ...
        vec2 _7831 = HSM_GetScaledCoord(param_175, param_176, param_177, param_178, param_179, param_180, param_181, param_182, param_183, param_184, param_185, param_186, param_187, param_188, param_189);
        LED_COORD = param_188;
        LED_SCALE = param_189;
        LED_CURVED_COORD = _7831;
        bool _7835 = HSM_LED_OPACITY > 0.0;
        bool _7841;
        if (_7835)
        {
            _7841 = led_size.y > 16.0;
        }
        else
        {
            _7841 = _7835;
        ...
        if (_7841)
        {
            ...
            vec3 _7870 = HSM_ApplyHSVAdjustment(param_196, param_197, param_198, param_199, param_200, param_201);
            led_image.x = _7870.x;
            led_image.y = _7870.y;
            led_image.z = _7870.z;
            if (HSM_STATIC_LAYERS_GAMMA != 1.0)
            ...
            vec3 _7905 = ApplyAmbientImages(param_204, param_205, param_206, param_207, param_208, param_209, param_210, param_211);
            led_image.x = _7905.x;
            led_image.y = _7905.y;
            led_image.z = _7905.z;
        }
    ...
    vec2 _7956 = HSM_GetScaledCoord(param_212, param_213, param_214, param_215, param_216, param_217, param_218, param_219, param_220, param_221, param_222, param_223, param_224, param_225, param_226);
    DECAL_COORD = param_225;
    DECAL_SCALE = param_226;
    DECAL_CURVED_COORD = _7956;
    vec4 decal_image = vec4(0.0);
    bool _7961 = HSM_DECAL_OPACITY > 0.0;
    bool _7967;
    if (_7961)
    {
        _7967 = decal_size.y > 16.0;
    }
    else
    {
        _7967 = _7961;
    ...
    if (_7967)
    {
        ...
        vec3 _7996 = HSM_ApplyHSVAdjustment(param_233, param_234, param_235, param_236, param_237, param_238);
        ...
        vec3 _8031 = ApplyAmbientImages(param_241, param_242, param_243, param_244, param_245, param_246, param_247, param_248);
    ...
    vec2 _8082 = HSM_GetScaledCoord(param_249, param_250, param_251, param_252, param_253, param_254, param_255, param_256, param_257, param_258, param_259, param_260, param_261, param_262, param_263);
    ...
    bool _8094 = HSM_TOP_OPACITY > 0.0;
    bool _8100;
    if (_8094)
    {
        _8100 = top_size.y > 16.0;
    }
    else
    {
        _8100 = _8094;
    ...
    if (_8100)
    {
        ...
        vec3 _8129 = HSM_ApplyHSVAdjustment(param_271, param_272, param_273, param_274, param_275, param_276);
        top_image.x = _8129.x;
        top_image.y = _8129.y;
        top_image.z = _8129.z;
        if (HSM_STATIC_LAYERS_GAMMA != 1.0)
        ...
        vec3 _8164 = ApplyAmbientImages(param_279, param_280, param_281, param_282, param_283, param_284, param_285, param_286);
        top_image.x = _8164.x;
        top_image.y = _8164.y;
        top_image.z = _8164.z;
    }
    ...
    if (HSM_CAB_GLASS_OPACITY > 0.0)
    {
        ...
        vec2 _8220 = HSM_GetScaledCoord(param_287, param_288, param_289, param_290, param_291, param_292, param_293, param_294, param_295, param_296, param_297, param_298, param_299, param_300, param_301);
        CAB_GLASS_COORD = param_300;
        CAB_GLASS_SCALE = param_301;
        CAB_GLASS_CURVED_COORD = _8220;
        bool _8224 = HSM_CAB_GLASS_OPACITY > 0.0;
        bool _8230;
        if (_8224)
        {
            _8230 = cab_glass_size.y > 16.0;
        }
        else
        {
            _8230 = _8224;
        ...
        if (_8230)
        {
            ...
            vec3 _8259 = HSM_ApplyHSVAdjustment(param_308, param_309, param_310, param_311, param_312, param_313);
            ...
            vec3 _8294 = ApplyAmbientImages(param_316, param_317, param_318, param_319, param_320, param_321, param_322, param_323);
    ...
    vec2 _8356 = HSM_GetScaledCoord(param_324, param_325, param_326, param_327, param_328, param_329, param_330, param_331, param_332, param_333, param_334, param_335, param_336, param_337, param_338);
    temp_coord = param_337;
    temp_scale = param_338;
    vec2 cutout_coord = _8356;
    ...
    if (HSM_VIEWPORT_VIGNETTE_OPACITY > 0.0)
    {
        ...
        vec2 _8408 = HSM_GetScaledCoord(param_343, param_344, param_345, param_346, param_347, param_348, param_349, param_350, param_351, param_352, param_353, param_354, param_355, param_356, param_357);
        temp_coord = param_356;
        temp_scale = param_357;
        vec2 vignette_coord = _8408;
    ...
    for (int i = start_layer; i <= end_layer; i++)
    {
        if (HSM_BG_LAYER_ORDER == float(i))
        {
            ...
            vec4 _8478 = BlendModeMaskLayerMix(param_367, param_368, param_369, param_370, param_371, param_372, param_373);
        ...
        if (HSM_VIEWPORT_VIGNETTE_LAYER_ORDER == float(i))
        {
            ...
            vec4 _8498 = BlendModeMaskLayerMix(param_374, param_375, param_376, param_377, param_378, param_379, param_380);
        ...
        if ((HSM_LED_LAYER_ORDER == float(i)) && (HSM_LED_OPACITY > 0.0))
        {
            ...
            vec4 _8522 = BlendModeMaskLayerMix(param_381, param_382, param_383, param_384, param_385, param_386, param_387);
        ...
        if ((HSM_DEVICE_LAYER_ORDER == float(i)) && (HSM_DEVICE_OPACITY > 0.0))
        {
            ...
            vec4 _8546 = BlendModeMaskLayerMix(param_388, param_389, param_390, param_391, param_392, param_393, param_394);
        ...
        if ((HSM_DEVICELED_LAYER_ORDER == float(i)) && (HSM_DEVICELED_OPACITY > 0.0))
        {
            ...
            vec4 _8570 = BlendModeMaskLayerMix(param_395, param_396, param_397, param_398, param_399, param_400, param_401);
        ...
        if ((HSM_DECAL_LAYER_ORDER == float(i)) && (HSM_DECAL_OPACITY > 0.0))
        {
            ...
            vec4 _8594 = BlendModeMaskLayerMix(param_402, param_403, param_404, param_405, param_406, param_407, param_408);
        ...
        if ((HSM_CAB_GLASS_LAYER_ORDER == float(i)) && (HSM_CAB_GLASS_OPACITY > 0.0))
        {
            ...
            vec4 _8618 = BlendModeMaskLayerMix(param_409, param_410, param_411, param_412, param_413, param_414, param_415);
        ...
        if ((HSM_TOP_LAYER_ORDER == float(i)) && (HSM_TOP_OPACITY > 0.0))
        {
            ...
            vec4 _8642 = BlendModeMaskLayerMix(param_416, param_417, param_418, param_419, param_420, param_421, param_422);
    ...
    bool _8686 = UNFLIPPED_VIEWPORT_COORD.x < (2.0 / global.OutputSize.x);
    bool _8695;
    if (_8686)
    {
        _8695 = UNFLIPPED_VIEWPORT_COORD.y < (2.0 / global.OutputSize.y);
    }
    else
    {
        _8695 = _8686;
