...
vec4 HSM_GetBlackOnlyCropInPixels(sampler2D in_sampler_2D, vec2 sample_start_pixel_coord, vec2 window_size, float num_samples, vec4 max_crop)
{
    ...
    for (int i = 0; float(i) < crop_left_px; i++)
    {
        ...
        vec4 _3827 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler_2D, param, param_1, param_2);
        tex_sample = _3827;
    ...
    for (int i_1 = 0; float(i_1) < crop_top_px; i_1++)
    {
        ...
        vec4 _3874 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler_2D, param_3, param_4, param_5);
        tex_sample = _3874;
        brightness = (tex_sample.x + tex_sample.y) + tex_sample.z;
        if (brightness > HSM_CROP_BLACK_THRESHOLD)
        {
    ...
    for (int i_2 = 0; float(i_2) < crop_right_px; i_2++)
    {
        ...
        vec4 _3922 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler_2D, param_6, param_7, param_8);
        tex_sample = _3922;
        brightness = (tex_sample.x + tex_sample.y) + tex_sample.z;
        if (brightness > HSM_CROP_BLACK_THRESHOLD)
        {
    ...
    for (int i_3 = 0; float(i_3) < crop_bottom_px; i_3++)
    {
        ...
        vec4 _3970 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler_2D, param_9, param_10, param_11);
        tex_sample = _3970;
        brightness = (tex_sample.x + tex_sample.y) + tex_sample.z;
        if (brightness > 0.0)
        {
...
vec2 HSM_GetResMult(float use_vert_scanlines)
{
    ...
    bool _3384 = (use_vert_scanlines == 1.0) && (HSM_ROTATE_CORE_IMAGE == 0.0);
    bool _3396;
    if (!_3384)
    {
        bool _3389 = use_vert_scanlines == 0.0;
        bool _3395;
        if (_3389)
        {
            _3395 = abs(HSM_ROTATE_CORE_IMAGE) == 1.0;
        }
        else
        {
            _3395 = _3389;
    ...
    else
    {
        ...
        if (global.HSM_INT_SCALE_MODE == 0.0)
        {
            if (HSM_USE_PHYSICAL_SIZE_FOR_NON_INTEGER == 1.0)
            {
                ...
                {
                    _3056 = 1.0 / HSM_PHYSICAL_MONITOR_ASPECT_RATIO;
                }
                else
                {
        ...
        if (global.HSM_INT_SCALE_MODE > 0.5)
        {
            ...
            bool _3265 = HSM_GetUseVerticalScanlines(param) == 1.0;
            bool _3272;
            if (!_3265)
            {
                _3272 = global.HSM_INT_SCALE_MODE == 2.0;
            }
            else
            {
                _3272 = _3265;
...
vec2 HSM_GetScreenPositionOffset(inout vec2 placement_image_pos, vec2 screen_scale, float screen_index)
{
    ...
    if (screen_index == 1.0)
    {
        _2496 = vec2(HSM_SCREEN_POSITION_X / output_aspect, HSM_SCREEN_POSITION_Y) + (placement_image_pos - vec2(0.5));
    }
    else
    {
        _2496 = vec2(HSM_2ND_SCREEN_POS_X / output_aspect, HSM_2ND_SCREEN_POS_Y);
    }
    vec2 pos_offset = _2496;
...
vec4 HSM_GetColorForScreenInfoCache(vec2 viewport_coord, sampler2D feedback_pass, sampler2D original_pass, sampler2D screen_placement_image)
{
    ...
    vec2 _5431 = HSM_GetCroppedRotatedSizeWithResMult(param_7, param_8);
    vec2 cropped_rotated_size_with_res_mult = _5431;
    float param_9 = 60.0;
    ...
    {
        _5456 = (0.60000002384185791015625 * HSM_SINDEN_BORDER_THICKNESS) * 60.0;
    }
    else
    {
        _5456 = 0.0;
    ...
    vec2 _5490 = HSM_GetTubeScale(param_14, param_15, param_16);
    vec2 tube_diffuse_scale = _5490;
    ...
    if (global.HSM_TUBE_DIFFUSE_FORCE_ASPECT > 0.0)
    {
        ...
        if (global.HSM_TUBE_DIFFUSE_FORCE_ASPECT == 2.0)
        {
            ...
            {
                _5517 = global.HSM_TUBE_EXPLICIT_ASPECT;
            }
            else
            {
                _5517 = 1.0 / global.HSM_TUBE_EXPLICIT_ASPECT;
            }
    ...
    vec2 _5556 = HSM_GetTubeScale(param_17, param_18, param_19);
    vec2 tube_scale = _5556;
    vec2 param_20 = screen_pos_and_height.xy;
    vec2 param_21 = screen_scale;
    float param_22 = 1.0;
    vec2 _5564 = HSM_GetScreenPositionOffset(param_20, param_21, param_22);
    vec2 pos_offset = _5564;
    float param_23 = 1.0;
    ...
    vec2 _5607 = HSM_GetCroppedRotatedSizeWithResMult(param_30, param_31);
    vec2 cropped_size_with_res_mult_2nd_screen = _5607;
    ...
    vec2 _5626 = HSM_GetTubeScale(param_32, param_33, param_34);
    vec2 tube_diffuse_scale_2nd_screen = _5626;
    ...
    vec2 _5645 = HSM_GetTubeScale(param_35, param_36, param_37);
    vec2 tube_scale_2nd_screen = _5645;
    ...
    float param_40 = 2.0;
    vec2 _5659 = HSM_GetScreenPositionOffset(param_38, param_39, param_40);
    vec2 pos_offset_2nd_screen = _5659;
    ...
    if (HSM_IsCoordIn2DRange(param_56, param_57) == 1)
    {
        ...
        vec2 _5740 = HSM_GetRotatedDerezedSize();
        out_color.z = _5740.x;
        out_color.w = _5740.y;
    }
    float param_58 = 1.0;
    ...
    if (HSM_IsCoordIn2DRange(param_80, param_81) == 1)
    {
        ...
        vec2 _5860 = HSM_GetRotatedCoreOriginalSize();
        out_color.z = _5860.x;
        out_color.w = _5860.y;
    }
    float param_82 = 3.0;
    ...
    {
        vec2 _5878 = vec2(HSM_VIEWPORT_ZOOM, HSM_VIEWPORT_ZOOM);
        out_color.x = _5878.x;
        out_color.y = _5878.y;
        vec2 _5885 = vec2(HSM_VIEWPORT_POSITION_X, HSM_VIEWPORT_POSITION_Y);
        out_color.z = _5885.x;
        out_color.w = _5885.y;
    }
    float param_86 = 4.0;
    ...
    if (!_6067)
    {
        _6077 = abs(parameter_sum - parameter_sum_feedback) > 1.0000000116860974230803549289703e-07;
    }
    else
    {
        _6077 = _6067;
    ...
    {
        _6078 = false || (HSM_CACHE_GRAPHICS_ON < 0.5);
    }
...
void main()
{
    ...
    vec4 _6114 = HSM_GetColorForScreenInfoCache(param, InfoCachePassFeedback, Source, ScreenPlacementImage);
    FragColor = _6114;
}
