...
layout(binding = 8) uniform sampler2D PostCRTPass;
layout(binding = 7) uniform sampler2D CRTPass;
layout(binding = 6) uniform sampler2D LinearizePass;
layout(binding = 5) uniform sampler2D ColorCorrectPass;
layout(binding = 4) uniform sampler2D DeditherPass;
layout(binding = 3) uniform sampler2D DerezedPass;
layout(binding = 2) uniform sampler2D Original;
layout(binding = 11) uniform sampler2D BR_LayersOverCRTPass;
layout(binding = 10) uniform sampler2D BR_LayersUnderCRTPass;
layout(binding = 12) uniform sampler2D CombinePassFeedback;
...
vec2 UNFLIPPED_VIEWPORT_COORD;
vec2 FLIPPED_VIEWPORT_COORD;
...
vec2 HSM_GetGeomCurvedCoord(vec2 in_coord, float tilt_x, float tilt_y, float in_radius, inout float in_distance, float in_aspect)
{
    ...
    vec2 _4841 = transform(param_5, param_6, param_7, param_8, param_9, param_10, param_11);
    vec2 curved_coord = _4841;
    return curved_coord;
}
...
vec2 HSM_GetGeomCurvedCoordRetainWidth(vec2 in_coord, float tilt_x, float tilt_y, float in_radius, float in_distance, float in_aspect)
{
    ...
    vec2 _4858 = HSM_GetGeomCurvedCoord(param, param_1, param_2, param_3, param_4, param_5);
    vec2 ctr_curved_coord = _4858 - vec2(0.5);
    ...
    vec2 _4873 = HSM_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11);
    vec2 right_edge_curved_ctr_coord = _4873 - vec2(0.5);
...
vec3 hrg_get_ideal_global_eye_pos_for_points(inout vec3 eye_pos, vec2 output_aspect, vec3 global_coords[9], int num_points, float in_geom_radius, float in_geom_view_dist)
{
    ...
    for (int iter = 0; iter < max_centering_iters; iter++)
    {
        ...
        vec3 _3527 = eye_pos;
        vec2 _3529 = _3527.xy + center_offset;
        eye_pos.x = _3529.x;
        eye_pos.y = _3529.y;
...
vec3 hrg_get_ideal_global_eye_pos(mat3 local_to_global, vec2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist)
{
    ...
    if (in_geom_mode < 2.5)
    {
        _3688 = vec3(0.0, 0.0, eye_z_spherical);
    }
    else
    {
        _3688 = vec3(0.0, 0.0, max(in_geom_view_dist, eye_z_spherical));
    }
    vec3 eye_pos = _3688;
    int num_points = 9;
    vec2 param = vec2(0.0);
    ...
    else
    {
        ...
        vec3 _3847 = hrg_get_ideal_global_eye_pos_for_points(param_36, param_37, param_38, param_39, param_40, param_41);
        _3830 = _3847;
    }
    return _3830;
}
...
vec2 hrg_get_curved_video_uv_coords_and_tangent_matrix(vec2 flat_video_uv, vec3 eye_pos_local, vec2 output_pixel_size, vec2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, mat3 global_to_local, out mat2 pixel_to_tangent_video_uv)
{
    ...
    vec3 param_5;
    vec2 _4072 = hrg_view_vec_to_uv(param, param_1, param_2, param_3, param_4, param_5);
    vec3 pos = param_5;
    vec2 centered_uv = _4072;
    ...
    if (geom_force_correct_tangent_matrix)
    {
        ...
        {
            _4084 = vec3(pos.x, 0.0, pos.z);
        }
        vec3 normal_base = _4084;
...
vec2 HRG_GetGeomCurvedCoord(vec2 in_coord, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, float in_geom_tilt_angle_x, float in_geom_tilt_angle_y, float in_screen_aspect, float pin_inner_edge, vec2 in_source_size, vec2 in_output_size, out mat2 pixel_to_video_uv)
{
    ...
    if (in_geom_mode > 0.5)
    {
        ...
        mat2 param_14;
        vec2 _4252 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14);
        pixel_to_video_uv = param_14;
        vec2 ctr_curved_coord = _4252 - vec2(0.5);
        if ((in_geom_tilt_angle_x == 0.0) && (in_geom_tilt_angle_y == 0.0))
        {
            ...
            mat2 param_23;
            vec2 _4281 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_15, param_16, param_17, param_18, param_19, param_20, param_21, param_22, param_23);
            pixel_to_video_uv = param_23;
            vec2 right_edge_curved_ctr_coord = _4281 - vec2(0.5);
            ...
            mat2 param_32;
            vec2 _4303 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_24, param_25, param_26, param_27, param_28, param_29, param_30, param_31, param_32);
            pixel_to_video_uv = param_32;
            vec2 bottom_edge_curved_ctr_coord = _4303 - vec2(0.5);
            ctr_curved_coord.x = (ctr_curved_coord.x * 0.5) / right_edge_curved_ctr_coord.x;
        ...
        if (pin_inner_edge == 1.0)
        {
            if (in_geom_tilt_angle_y != 0.0)
            {
                ...
                mat2 param_41;
                vec2 _4346 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_33, param_34, param_35, param_36, param_37, param_38, param_39, param_40, param_41);
                pixel_to_video_uv = param_41;
                vec2 top_edge_curved_ctr_coord = _4346;
            ...
            if (in_geom_tilt_angle_x != 0.0)
            {
                ...
                mat2 param_50;
                vec2 _4375 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_42, param_43, param_44, param_45, param_46, param_47, param_48, param_49, param_50);
                pixel_to_video_uv = param_50;
                vec2 left_edge_curved_ctr_coord = _4375;
...
vec2 HSM_GetCurvatureValues(float screen_aspect)
{
    ...
    {
        _4402 = vec2((((2.0 * global.HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0) * 2.0) / 100.0, ((global.HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0) * 3.0) / 100.0);
    }
    else
    {
        _4402 = vec2(((global.HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0) * 3.0) / 100.0, (((2.0 * global.HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0) * 2.0) / 100.0);
    }
    vec2 curvature_values = _4402;
...
vec2 HSM_Get2DCurvedCoord(vec2 in_coord, vec2 curvature_values)
{
    ...
    vec2 _4937 = HSM_GetCrtPiCurvedCoord(param, param_1);
    ctr_curved_coord = _4937 - vec2(0.5);
    vec2 param_2 = vec2(1.0, 0.5);
    vec2 param_3 = curvature_values;
    vec2 _4944 = HSM_GetCrtPiCurvedCoord(param_2, param_3);
    vec2 right_edge_curved_ctr_coord = _4944 - vec2(0.5);
    ctr_curved_coord.x = (ctr_curved_coord.x * 0.5) / right_edge_curved_ctr_coord.x;
    vec2 param_4 = vec2(0.5, 1.0);
    vec2 param_5 = curvature_values;
    vec2 _4958 = HSM_GetCrtPiCurvedCoord(param_4, param_5);
    vec2 bottom_edge_curved_ctr_coord = _4958 - vec2(0.5);
    ctr_curved_coord.y = (ctr_curved_coord.y * 0.5) / bottom_edge_curved_ctr_coord.y;
    return ctr_curved_coord + vec2(0.5);
}
...
vec2 HSM_GetCurvedCoord(vec2 in_coord, float curvature_multiplier, float screen_aspect)
{
    ...
    if (HSM_CURVATURE_MODE > CURVATURE_MODE_2D_CYLINDER)
    {
        ...
        else
        {
            ...
            mat2 param_16;
            vec2 _5066 = HRG_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14, param_15, param_16);
...
{
    bool _2506 = abs(in_coord.x - 0.5) > 0.500999987125396728515625;
    bool _2516;
    if (!_2506)
    {
        _2516 = abs(in_coord.y - 0.5) > 0.500999987125396728515625;
    }
    else
    {
        _2516 = _2506;
...
vec2 HSM_GetTubeCurvedCoord(vec2 screen_coord, float curvature_scale, vec2 screen_scale, vec2 tube_scale, float screen_aspect, float apply_black_edge_offset)
{
    ...
    if (global.HSM_BZL_USE_INDEPENDENT_CURVATURE == 1.0)
    {
        ...
        if (screen_aspect < 1.0)
        {
            _5174 = vec2((((2.0 * global.HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0) * 2.0) / 100.0, ((global.HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0) * 3.0) / 100.0);
        }
        else
        {
            _5174 = vec2(((global.HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0) * 3.0) / 100.0, (((2.0 * global.HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0) * 2.0) / 100.0);
        }
        vec2 curvature_values = _5174;
...
float HSM_GetBezelCoords(vec2 tube_diffuse_coord, vec2 tube_diffuse_scale, vec2 tube_scale, float screen_aspect, bool curve_coords_on, out vec2 bezel_outside_scale, inout vec2 bezel_outside_coord, inout vec2 bezel_outside_curved_coord, inout vec2 frame_outside_curved_coord)
{
    ...
    if (curve_coords_on)
    {
        bool _5312 = HSM_CURVATURE_MODE > 0.0;
        bool _5320;
        if (_5312)
        {
            _5320 = (HSM_CURVATURE_3D_TILT_ANGLE_X != 0.0) || (HSM_CURVATURE_3D_TILT_ANGLE_Y != 0.0);
        }
        else
        {
            _5320 = _5312;
...
void main()
{
    ...
    vec2 VIEWPORT_COORD = HSM_GetViewportCoordWithZoomAndPan(param);
    vec2 param_1 = vTexCoord;
    vec4 _5470 = HSM_UpdateGlobalScreenValuesFromCache(InfoCachePass, param_1);
    if (HSM_SHOW_PASS_INDEX > 0.5)
    {
        ...
        vec2 coord_for_raw_passes = _5480;
    ...
    vec2 param_14 = BEZEL_OUTSIDE_COORD;
    if (HHLP_IsOutsideCoordSpace(param_14))
    {
        vec4 feedback_color_test = texture(CombinePassFeedback, vec2(0.0));
        bool _5657 = HSM_CACHE_GRAPHICS_ON > 0.5;
        bool _5663;
        if (_5657)
        {
            _5663 = feedback_color_test.w < 0.0;
        }
        else
        {
            _5663 = _5657;
        }
        if ((_5663 && (!CACHE_INFO_CHANGED)) && (HSM_RENDER_SIMPLE_MODE < 0.5))
        {
    ...
    vec3 param_15 = under_layer.xyz;
    vec3 _5683 = PrepGraphicsForHDR(param_15);
    under_layer.x = _5683.x;
    under_layer.y = _5683.y;
    under_layer.z = _5683.z;
    vec4 out_color = under_layer;
    vec4 crt_layer = texture(PostCRTPass, vTexCoord);
    ...
    if (HSM_RENDER_SIMPLE_MODE < 0.5)
    {
        vec4 param_16 = out_color;
        vec4 param_17 = crt_layer;
        out_color = HSM_PreMultAlphaBlend(param_16, param_17);
    }
    vec4 over_layer = texture(BR_LayersOverCRTPass, vTexCoord);
    vec3 param_18 = over_layer.xyz;
    vec3 _5736 = PrepGraphicsForHDR(param_18);
    over_layer.x = _5736.x;
    over_layer.y = _5736.y;
    over_layer.z = _5736.z;
    vec4 param_19 = out_color;
    vec4 param_20 = over_layer;
    out_color = HSM_PreMultAlphaBlend(param_19, param_20);
    FragColor = out_color;
    if (HSM_RENDER_SIMPLE_MODE > 0.5)
    {
        vec2 param_21 = TUBE_DIFFUSE_COORD;
        float param_22 = 1.0;
        vec2 param_23 = TUBE_DIFFUSE_SCALE;
        vec2 param_24 = TUBE_SCALE;
        float param_25 = TUBE_DIFFUSE_ASPECT;
        float param_26 = 1.0;
        vec2 tube_curved_coord = HSM_GetTubeCurvedCoord(param_21, param_22, param_23, param_24, param_25, param_26);
        vec2 param_27 = TUBE_DIFFUSE_COORD;
        float param_28 = global.HSM_TUBE_BLACK_EDGE_CURVATURE_SCALE / 100.0;
        float param_29 = TUBE_DIFFUSE_ASPECT;
        TUBE_DIFFUSE_CURVED_COORD = HSM_GetCurvedCoord(param_27, param_28, param_29);
        vec2 param_30 = ((TUBE_DIFFUSE_CURVED_COORD - vec2(0.5)) * 1.00100004673004150390625) + vec2(0.5);
        float param_31 = TUBE_DIFFUSE_ASPECT;
        float param_32 = (global.HSM_GLOBAL_CORNER_RADIUS * global.HSM_TUBE_BLACK_EDGE_CORNER_RADIUS_SCALE) / 100.0;
        float param_33 = global.HSM_TUBE_BLACK_EDGE_SHARPNESS / 100.0;
        TUBE_DIFFUSE_MASK = HSM_GetCornerMask(param_30, param_31, param_32, param_33);
        vec2 param_34 = tube_curved_coord;
        float param_35 = TUBE_DIFFUSE_ASPECT;
        float param_36 = bezel_corner_radius;
        float param_37 = 0.9900000095367431640625;
        TUBE_MASK = HSM_GetCornerMask(param_34, param_35, param_36, param_37);
        vec2 param_38 = BEZEL_OUTSIDE_CURVED_COORD;
        float param_39 = TUBE_DIFFUSE_ASPECT;
        float param_40 = global.HSM_GLOBAL_CORNER_RADIUS * HSM_BZL_OUTER_CORNER_RADIUS_SCALE;
        float param_41 = 0.89999997615814208984375;
        ...
        vec2 param_42 = FRAME_OUTSIDE_CURVED_COORD;
        float param_43 = TUBE_DIFFUSE_ASPECT;
        float param_44 = HSM_FRM_OUTER_CORNER_RADIUS;
        float param_45 = 1.0;
        OUTSIDE_FRAME_MASK = 1.0 - HSM_GetCornerMask(param_42, param_43, param_44, param_45);
        FRAME_MASK = OUTSIDE_BEZEL_MASK * (1.0 - OUTSIDE_FRAME_MASK);
        if (HSM_RENDER_SIMPLE_MODE == 1.0)
        {
            vec4 param_46 = FragColor;
            vec4 param_47 = vec4(0.0, 0.0, 0.0, 1.0) * TUBE_MASK;
            FragColor = HSM_PreMultAlphaBlend(param_46, param_47);
        }
        if (HSM_RENDER_SIMPLE_MODE == 2.0)
        {
            vec4 param_48 = FragColor;
            vec4 param_49 = vec4(0.0, 0.0, 0.0, 1.0) * (TUBE_MASK + OUTSIDE_FRAME_MASK);
    ...
    bool _5965 = UNFLIPPED_VIEWPORT_COORD.x < (2.0 / global.OutputSize.x);
    bool _5974;
    if (_5965)
    {
        _5974 = UNFLIPPED_VIEWPORT_COORD.y < (2.0 / global.OutputSize.y);
    }
    else
    {
        _5974 = _5965;
    ...
    vec4 param_60 = FragColor;
    float param_61 = 2.2000000476837158203125;
