...
{
    bool _1381 = abs(in_coord.x - 0.5) > 0.500999987125396728515625;
    bool _1391;
    if (!_1381)
    {
        _1391 = abs(in_coord.y - 0.5) > 0.500999987125396728515625;
    }
    else
    {
        _1391 = _1381;
...
vec2 HSM_GetGeomCurvedCoord(vec2 in_coord, float tilt_x, float tilt_y, float in_radius, inout float in_distance, float in_aspect)
{
    ...
    vec2 _3823 = transform(param_5, param_6, param_7, param_8, param_9, param_10, param_11);
    vec2 curved_coord = _3823;
    return curved_coord;
}
...
vec2 HSM_GetGeomCurvedCoordRetainWidth(vec2 in_coord, float tilt_x, float tilt_y, float in_radius, float in_distance, float in_aspect)
{
    ...
    vec2 _3840 = HSM_GetGeomCurvedCoord(param, param_1, param_2, param_3, param_4, param_5);
    vec2 ctr_curved_coord = _3840 - vec2(0.5);
    ...
    vec2 _3855 = HSM_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11);
    vec2 right_edge_curved_ctr_coord = _3855 - vec2(0.5);
...
vec3 hrg_get_ideal_global_eye_pos_for_points(inout vec3 eye_pos, vec2 output_aspect, vec3 global_coords[9], int num_points, float in_geom_radius, float in_geom_view_dist)
{
    ...
    for (int iter = 0; iter < max_centering_iters; iter++)
    {
        ...
        vec3 _2402 = eye_pos;
        vec2 _2404 = _2402.xy + center_offset;
        eye_pos.x = _2404.x;
        eye_pos.y = _2404.y;
...
vec3 hrg_get_ideal_global_eye_pos(mat3 local_to_global, vec2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist)
{
    ...
    if (in_geom_mode < 2.5)
    {
        _2563 = vec3(0.0, 0.0, eye_z_spherical);
    }
    else
    {
        _2563 = vec3(0.0, 0.0, max(in_geom_view_dist, eye_z_spherical));
    }
    vec3 eye_pos = _2563;
    int num_points = 9;
    vec2 param = vec2(0.0);
    ...
    else
    {
        ...
        vec3 _2722 = hrg_get_ideal_global_eye_pos_for_points(param_36, param_37, param_38, param_39, param_40, param_41);
        _2705 = _2722;
    }
    return _2705;
}
...
vec2 hrg_get_curved_video_uv_coords_and_tangent_matrix(vec2 flat_video_uv, vec3 eye_pos_local, vec2 output_pixel_size, vec2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, mat3 global_to_local, out mat2 pixel_to_tangent_video_uv)
{
    ...
    vec3 param_5;
    vec2 _2947 = hrg_view_vec_to_uv(param, param_1, param_2, param_3, param_4, param_5);
    vec3 pos = param_5;
    vec2 centered_uv = _2947;
    ...
    if (geom_force_correct_tangent_matrix)
    {
        ...
        {
            _2960 = vec3(pos.x, 0.0, pos.z);
        }
        vec3 normal_base = _2960;
...
vec2 HRG_GetGeomCurvedCoord(vec2 in_coord, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, float in_geom_tilt_angle_x, float in_geom_tilt_angle_y, float in_screen_aspect, float pin_inner_edge, vec2 in_source_size, vec2 in_output_size, out mat2 pixel_to_video_uv)
{
    ...
    if (in_geom_mode > 0.5)
    {
        ...
        mat2 param_14;
        vec2 _3128 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14);
        pixel_to_video_uv = param_14;
        vec2 ctr_curved_coord = _3128 - vec2(0.5);
        if ((in_geom_tilt_angle_x == 0.0) && (in_geom_tilt_angle_y == 0.0))
        {
            ...
            mat2 param_23;
            vec2 _3157 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_15, param_16, param_17, param_18, param_19, param_20, param_21, param_22, param_23);
            pixel_to_video_uv = param_23;
            vec2 right_edge_curved_ctr_coord = _3157 - vec2(0.5);
            ...
            mat2 param_32;
            vec2 _3179 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_24, param_25, param_26, param_27, param_28, param_29, param_30, param_31, param_32);
            pixel_to_video_uv = param_32;
            vec2 bottom_edge_curved_ctr_coord = _3179 - vec2(0.5);
            ctr_curved_coord.x = (ctr_curved_coord.x * 0.5) / right_edge_curved_ctr_coord.x;
        ...
        if (pin_inner_edge == 1.0)
        {
            if (in_geom_tilt_angle_y != 0.0)
            {
                ...
                mat2 param_41;
                vec2 _3222 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_33, param_34, param_35, param_36, param_37, param_38, param_39, param_40, param_41);
                pixel_to_video_uv = param_41;
                vec2 top_edge_curved_ctr_coord = _3222;
            ...
            if (in_geom_tilt_angle_x != 0.0)
            {
                ...
                mat2 param_50;
                vec2 _3251 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_42, param_43, param_44, param_45, param_46, param_47, param_48, param_49, param_50);
                pixel_to_video_uv = param_50;
                vec2 left_edge_curved_ctr_coord = _3251;
...
vec2 HSM_GetCurvatureValues(float screen_aspect)
{
    ...
    {
        _3384 = vec2((((2.0 * global.HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0) * 2.0) / 100.0, ((global.HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0) * 3.0) / 100.0);
    }
    else
    {
        _3384 = vec2(((global.HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0) * 3.0) / 100.0, (((2.0 * global.HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0) * 2.0) / 100.0);
    }
    vec2 curvature_values = _3384;
...
vec2 HSM_Get2DCurvedCoord(vec2 in_coord, vec2 curvature_values)
{
    ...
    vec2 _3919 = HSM_GetCrtPiCurvedCoord(param, param_1);
    ctr_curved_coord = _3919 - vec2(0.5);
    vec2 param_2 = vec2(1.0, 0.5);
    vec2 param_3 = curvature_values;
    vec2 _3926 = HSM_GetCrtPiCurvedCoord(param_2, param_3);
    vec2 right_edge_curved_ctr_coord = _3926 - vec2(0.5);
    ctr_curved_coord.x = (ctr_curved_coord.x * 0.5) / right_edge_curved_ctr_coord.x;
    vec2 param_4 = vec2(0.5, 1.0);
    vec2 param_5 = curvature_values;
    vec2 _3940 = HSM_GetCrtPiCurvedCoord(param_4, param_5);
    vec2 bottom_edge_curved_ctr_coord = _3940 - vec2(0.5);
    ctr_curved_coord.y = (ctr_curved_coord.y * 0.5) / bottom_edge_curved_ctr_coord.y;
    return ctr_curved_coord + vec2(0.5);
}
...
vec2 HSM_GetCurvedCoord(vec2 in_coord, float curvature_multiplier, float screen_aspect)
{
    ...
    if (HSM_CURVATURE_MODE > CURVATURE_MODE_2D_CYLINDER)
    {
        ...
        else
        {
            ...
            mat2 param_16;
            vec2 _4047 = HRG_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14, param_15, param_16);
...
vec2 HSM_GetMirrorWrappedCoord(inout vec2 in_coord)
{
    ...
    bool _4109 = abs(ctr_coord.x) > 0.5;
    bool _4117;
    if (!_4109)
    {
        _4117 = abs(ctr_coord.y) > 0.5;
    }
    else
    {
        _4117 = _4109;
...
vec2 HSM_GetCrtShaderFinalCoord(vec2 screen_coord, vec2 screen_scale, float raster_bloom_avg_lum, inout vec2 screen_curved_coord)
{
    ...
    vec2 _4299 = HSM_GetMirrorWrappedCoord(param_1);
    vec2 screen_curved_coord_with_overscan_and_mirror = _4299;
...
vec4 HSM_GetCroppedTexSample(sampler2D in_sampler, vec2 in_screen_coord)
{
    ...
    vec4 _4234 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler, param, param_1, param_2);
    return _4234;
}
...
float SlotMask(inout vec2 pos, float m)
{
    ...
    else
    {
        ...
        {
            bool _5611 = py == global.double_slot;
            bool _5618;
            if (_5611)
            {
                _5618 = px >= global.slotwidth;
            }
            else
            {
                _5618 = _5611;
...
vec3 Mask(inout vec2 pos, float mx, float mb, out float mask_average_luma)
{
    ...
    pos.y = floor(pos.y / final_mask_size);
    float stagg_lvl = 1.0;
    bool _4399 = fract(abs(global.mshift)) > 0.25;
    bool _4407;
    if (_4399)
    {
        _4407 = abs(global.mshift) > 1.25;
    }
    else
    {
        _4407 = _4399;
    ...
    {
        _4421 = pos0.x + (next_line * global.mshift);
    }
    else
    {
        _4421 = pos0.x + (pos.y * global.mshift);
    }
    pos0.x = _4421;
    ...
    bool _4471 = global.shadowMask == 8.0;
    bool _4479;
    if (!_4471)
    {
        _4479 = global.shadowMask == 11.0;
    }
    else
    {
        _4479 = _4471;
...
void main()
{
    ...
    vec4 _6055 = HSM_UpdateGlobalScreenValuesFromCache(InfoCachePass, param_1);
    ...
    vec2 _6106 = HSM_GetCrtShaderFinalCoord(param_3, param_4, param_5, param_6);
    screen_curved_coord = param_6;
    vec2 screen_curved_coord_with_overscan_and_mirror = _6106;
    ...
    if (is_inside_screen)
    {
        _6154 = global.GDV_DECONVERGENCE_ON > 0.5;
    }
    else
    {
    ...
    if (_6154)
    {
        _6181 = (((((abs(global.deconrr) + abs(global.deconrry)) + abs(global.deconrg)) + abs(global.deconrgy)) + abs(global.deconrb)) + abs(global.deconrby)) > 0.20000000298023223876953125;
    }
    else
    {
        _6181 = _6154;
    ...
    vec3 _6347 = Mask(param_19, param_20, param_21, param_22);
    ...
    if (interb)
    {
        ...
        {
            _6471 = pow(0.800000011920928955078125 * bb, 0.64999997615814208984375);
        }
        else
        {
            _6471 = pow(bb, 0.699999988079071044921875);
        }
        bb = _6471;
    }
    color *= bb;
    ...
    if (!interb)
    {
        ...
        vec3 _6602 = declip(param_30, param_31);
        color = _6602;
    }
    else
    {
    ...
    bool _6875 = global.glow >= 0.0;
    bool _6881;
    if (_6875)
    {
        _6881 = global.m_glow < 0.5;
    }
    else
    {
        _6881 = _6875;
    ...
    if (global.GDV_NOISE_ON > 0.5)
    {
        ...
        {
            _6945 = global.noiseresd * global.noiseresd4kmult;
        }
        else
        {
            _6945 = global.noiseresd;
        }
        float noise_res = _6945;
        bool _6961 = global.noiseresd4kmult > 1.0;
        bool _6968;
        if (_6961)
        {
            _6968 = global.FinalViewportSize.y > 4200.0;
        }
        else
        {
            _6968 = _6961;
