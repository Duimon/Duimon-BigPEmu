...
Texture2D<float4> CorePass : register(t2);
SamplerState _CorePass_sampler : register(s2);
...
float3 maxscale(float2 sinangle, float2 cosangle, float in_radius, float in_distance, float in_aspect)
{
    ...
    float2 _4681 = fwtrans(param_5, param_6, param_7, param_8, param_9);
    ...
    float2 _4698 = fwtrans(param_10, param_11, param_12, param_13, param_14);
    float2 lo = float2(_4681.x, _4698.y) / aspect_vec2;
    ...
    float2 _4718 = fwtrans(param_15, param_16, param_17, param_18, param_19);
    ...
    float2 _4734 = fwtrans(param_20, param_21, param_22, param_23, param_24);
    float2 hi = float2(_4718.x, _4734.y) / aspect_vec2;
...
float2 HSM_GetGeomCurvedCoord(float2 in_coord, float tilt_x, float tilt_y, float in_radius, inout float in_distance, float in_aspect)
{
    ...
    float2 _4831 = transform(param_5, param_6, param_7, param_8, param_9, param_10, param_11);
    float2 curved_coord = _4831;
    return curved_coord;
}
...
float2 HSM_GetGeomCurvedCoordRetainWidth(float2 in_coord, float tilt_x, float tilt_y, float in_radius, float in_distance, float in_aspect)
{
    ...
    float2 _4848 = HSM_GetGeomCurvedCoord(param, param_1, param_2, param_3, param_4, param_5);
    float2 ctr_curved_coord = _4848 - 0.5f.xx;
    ...
    float2 _4863 = HSM_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11);
    float2 right_edge_curved_ctr_coord = _4863 - 0.5f.xx;
...
float3 hrg_get_ideal_global_eye_pos_for_points(inout float3 eye_pos, float2 output_aspect, float3 global_coords[9], int num_points, float in_geom_radius, float in_geom_view_dist)
{
    ...
    for (int iter = 0; iter < max_centering_iters; iter++)
    {
        ...
        float3 _3516 = eye_pos;
        float2 _3518 = _3516.xy + center_offset;
        eye_pos.x = _3518.x;
        eye_pos.y = _3518.y;
...
float3 hrg_get_ideal_global_eye_pos(float3x3 local_to_global, float2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist)
{
    ...
    if (in_geom_mode < 2.5f)
    {
        _3677 = float3(0.0f, 0.0f, eye_z_spherical);
    }
    else
    {
        _3677 = float3(0.0f, 0.0f, max(in_geom_view_dist, eye_z_spherical));
    }
    float3 eye_pos = _3677;
    int num_points = 9;
    ...
    else
    {
        ...
        float3 _3837 = hrg_get_ideal_global_eye_pos_for_points(param_36, param_37, param_38, param_39, param_40, param_41);
        _3820 = _3837;
    }
    return _3820;
}
...
float2 hrg_get_curved_video_uv_coords_and_tangent_matrix(float2 flat_video_uv, float3 eye_pos_local, float2 output_pixel_size, float2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, float3x3 global_to_local, out float2x2 pixel_to_tangent_video_uv)
{
    ...
    float3 param_5;
    float2 _4062 = hrg_view_vec_to_uv(param, param_1, param_2, param_3, param_4, param_5);
    float3 pos = param_5;
    float2 centered_uv = _4062;
    ...
    if (geom_force_correct_tangent_matrix)
    {
        ...
        {
            _4074 = float3(pos.x, 0.0f, pos.z);
        }
        float3 normal_base = _4074;
...
float2 HRG_GetGeomCurvedCoord(float2 in_coord, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, float in_geom_tilt_angle_x, float in_geom_tilt_angle_y, float in_screen_aspect, float pin_inner_edge, float2 in_source_size, float2 in_output_size, out float2x2 pixel_to_video_uv)
{
    ...
    if (in_geom_mode > 0.5f)
    {
        ...
        float2x2 param_14;
        float2 _4242 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14);
        pixel_to_video_uv = param_14;
        float2 ctr_curved_coord = _4242 - 0.5f.xx;
        if ((in_geom_tilt_angle_x == 0.0f) && (in_geom_tilt_angle_y == 0.0f))
        {
            ...
            float2x2 param_23;
            float2 _4271 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_15, param_16, param_17, param_18, param_19, param_20, param_21, param_22, param_23);
            pixel_to_video_uv = param_23;
            float2 right_edge_curved_ctr_coord = _4271 - 0.5f.xx;
            ...
            float2x2 param_32;
            float2 _4293 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_24, param_25, param_26, param_27, param_28, param_29, param_30, param_31, param_32);
            pixel_to_video_uv = param_32;
            float2 bottom_edge_curved_ctr_coord = _4293 - 0.5f.xx;
            ctr_curved_coord.x = (ctr_curved_coord.x * 0.5f) / right_edge_curved_ctr_coord.x;
        ...
        if (pin_inner_edge == 1.0f)
        {
            if (in_geom_tilt_angle_y != 0.0f)
            {
                ...
                float2x2 param_41;
                float2 _4336 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_33, param_34, param_35, param_36, param_37, param_38, param_39, param_40, param_41);
                pixel_to_video_uv = param_41;
                float2 top_edge_curved_ctr_coord = _4336;
            ...
            if (in_geom_tilt_angle_x != 0.0f)
            {
                ...
                float2x2 param_50;
                float2 _4365 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_42, param_43, param_44, param_45, param_46, param_47, param_48, param_49, param_50);
                pixel_to_video_uv = param_50;
                float2 left_edge_curved_ctr_coord = _4365;
...
float2 HSM_GetCurvatureValues(float screen_aspect)
{
    ...
    {
        _4392 = float2((((2.0f * global_HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f, ((global_HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f);
    }
    else
    {
        _4392 = float2(((global_HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f, (((2.0f * global_HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f);
    }
    float2 curvature_values = _4392;
...
float2 HSM_GetCrtPiCurvedCoord(inout float2 in_coord, inout float2 in_curvature)
{
    ...
    float rsq = (in_coord.x * in_coord.x) + _4896;
...
float2 HSM_Get2DCurvedCoord(float2 in_coord, float2 curvature_values)
{
    ...
    float2 _4927 = HSM_GetCrtPiCurvedCoord(param, param_1);
    ctr_curved_coord = _4927 - 0.5f.xx;
    float2 param_2 = float2(1.0f, 0.5f);
    float2 param_3 = curvature_values;
    float2 _4934 = HSM_GetCrtPiCurvedCoord(param_2, param_3);
    float2 right_edge_curved_ctr_coord = _4934 - 0.5f.xx;
    ctr_curved_coord.x = (ctr_curved_coord.x * 0.5f) / right_edge_curved_ctr_coord.x;
    float2 param_4 = float2(0.5f, 1.0f);
    float2 param_5 = curvature_values;
    float2 _4948 = HSM_GetCrtPiCurvedCoord(param_4, param_5);
    float2 bottom_edge_curved_ctr_coord = _4948 - 0.5f.xx;
    ctr_curved_coord.y = (ctr_curved_coord.y * 0.5f) / bottom_edge_curved_ctr_coord.y;
    return ctr_curved_coord + 0.5f.xx;
}
...
float2 HSM_GetCurvedCoord(float2 in_coord, float curvature_multiplier, float screen_aspect)
{
    ...
    if (HSM_CURVATURE_MODE > CURVATURE_MODE_2D_CYLINDER)
    {
        ...
        else
        {
            ...
            float2x2 param_16;
            float2 _5056 = HRG_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14, param_15, param_16);
...
{
    bool _2495 = abs(in_coord.x - 0.5f) > 0.500999987125396728515625f;
    bool _2505;
    if (!_2495)
    {
        _2505 = abs(in_coord.y - 0.5f) > 0.500999987125396728515625f;
    }
    else
    {
        _2505 = _2495;
...
float2 HSM_GetTubeCurvedCoord(float2 screen_coord, float curvature_scale, float2 screen_scale, float2 tube_scale, float screen_aspect, float apply_black_edge_offset)
{
    ...
    if (global_HSM_BZL_USE_INDEPENDENT_CURVATURE == 1.0f)
    {
        ...
        if (screen_aspect < 1.0f)
        {
            _5164 = float2((((2.0f * global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f, ((global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f);
        }
        else
        {
            _5164 = float2(((global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f, (((2.0f * global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f);
        }
        float2 curvature_values = _5164;
...
float HSM_GetBezelCoords(float2 tube_diffuse_coord, float2 tube_diffuse_scale, float2 tube_scale, float screen_aspect, bool curve_coords_on, out float2 bezel_outside_scale, inout float2 bezel_outside_coord, inout float2 bezel_outside_curved_coord, inout float2 frame_outside_curved_coord)
{
    ...
    if (curve_coords_on)
    {
        bool _5302 = HSM_CURVATURE_MODE > 0.0f;
        bool _5310;
        if (_5302)
        {
            _5310 = (HSM_CURVATURE_3D_TILT_ANGLE_X != 0.0f) || (HSM_CURVATURE_3D_TILT_ANGLE_Y != 0.0f);
        }
        else
        {
            _5310 = _5302;
...
void frag_main()
{
    ...
    float4 _5460 = HSM_UpdateGlobalScreenValuesFromCache(InfoCachePass, _InfoCachePass_sampler, param_1);
    if (HSM_SHOW_PASS_INDEX > 0.5f)
    {
        ...
        float2 coord_for_raw_passes = _5470;
        ...
        {
            uint _5513_dummy_parameter;
            uint _5521_dummy_parameter;
            FragColor = LinearizePass.Sample(_LinearizePass_sampler, (floor(coord_for_raw_passes * float2(int2(spvTextureSize(LinearizePass, uint(0), _5513_dummy_parameter)))) + 0.5f.xx) / float2(int2(spvTextureSize(LinearizePass, uint(0), _5521_dummy_parameter))));
        }
        if (HSM_SHOW_PASS_INDEX == 5.0f)
        {
            uint _5534_dummy_parameter;
            uint _5542_dummy_parameter;
            FragColor = ColorCorrectPass.Sample(_ColorCorrectPass_sampler, (floor(coord_for_raw_passes * float2(int2(spvTextureSize(ColorCorrectPass, uint(0), _5534_dummy_parameter)))) + 0.5f.xx) / float2(int2(spvTextureSize(ColorCorrectPass, uint(0), _5542_dummy_parameter))));
        }
        if (HSM_SHOW_PASS_INDEX == 6.0f)
        {
            uint _5556_dummy_parameter;
            uint _5564_dummy_parameter;
            FragColor = DeditherPass.Sample(_DeditherPass_sampler, (floor(coord_for_raw_passes * float2(int2(spvTextureSize(DeditherPass, uint(0), _5556_dummy_parameter)))) + 0.5f.xx) / float2(int2(spvTextureSize(DeditherPass, uint(0), _5564_dummy_parameter))));
        }
        ...
        {
            FragColor = CorePass.Sample(_CorePass_sampler, coord_for_raw_passes);
        }
    ...
    float _5640 = HSM_GetBezelCoords(param_5, param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13);
    ...
    if (HHLP_IsOutsideCoordSpace(param_14))
    {
        ...
        bool _5655 = HSM_CACHE_GRAPHICS_ON > 0.5f;
        bool _5661;
        if (_5655)
        {
            _5661 = feedback_color_test.w < 0.0f;
        }
        else
        {
            _5661 = _5655;
        }
        if ((_5661 && (!CACHE_INFO_CHANGED)) && (HSM_RENDER_SIMPLE_MODE < 0.5f))
        {
    ...
    float3 _5681 = PrepGraphicsForHDR(param_15);
    ...
    float3 _5764 = PrepGraphicsForHDR(param_28);
    ...
    bool _5992 = UNFLIPPED_VIEWPORT_COORD.x < (2.0f / global_OutputSize.x);
    bool _6001;
    if (_5992)
    {
        _6001 = UNFLIPPED_VIEWPORT_COORD.y < (2.0f / global_OutputSize.y);
    }
    else
    {
        _6001 = _5992;
