...
cbuffer Push
{
    float4 params_OriginalSize : packoffset(c0);
    float4 params_OutputSize : packoffset(c1);
    uint params_FrameCount : packoffset(c2);
};

Texture2D<float4> NegativeCropAddedPass : register(t4);
SamplerState _NegativeCropAddedPass_sampler : register(s4);
Texture2D<float4> PrePass : register(t3);
SamplerState _PrePass_sampler : register(s3);
...
static bool use_vert_scanlines_bool;
...
{
    uint _1559_dummy_parameter;
    float2 prescale_tex_size = float2(int2(spvTextureSize(NegativeCropAddedPass, uint(0), _1559_dummy_parameter)));
    float2 dx = float2(1.0f / prescale_tex_size.x, 0.0f) * HSM_DOWNSAMPLE_BLUR_SCANLINE_DIR;
    float2 dy = float2(0.0f, 1.0f / prescale_tex_size.y) * HSM_DOWNSAMPLE_BLUR_OPPOSITE_DIR;
    if (use_vert_scanlines_bool)
    {
        dx = float2(1.0f / prescale_tex_size.x, 0.0f) * HSM_DOWNSAMPLE_BLUR_OPPOSITE_DIR;
        dy = float2(0.0f, 1.0f / prescale_tex_size.y) * HSM_DOWNSAMPLE_BLUR_SCANLINE_DIR;
    }
    float2 d1 = dx + dy;
    float2 d2 = dx - dy;
    float sum = 15.0f;
    float3 result = ((((((((PrePass.Sample(_PrePass_sampler, coord).xyz * 3.0f) + (PrePass.Sample(_PrePass_sampler, coord + dx).xyz * 2.0f)) + (PrePass.Sample(_PrePass_sampler, coord - dx).xyz * 2.0f)) + (PrePass.Sample(_PrePass_sampler, coord + dy).xyz * 2.0f)) + (PrePass.Sample(_PrePass_sampler, coord - dy).xyz * 2.0f)) + PrePass.Sample(_PrePass_sampler, coord + d1).xyz) + PrePass.Sample(_PrePass_sampler, coord - d1).xyz) + PrePass.Sample(_PrePass_sampler, coord + d2).xyz) + PrePass.Sample(_PrePass_sampler, coord - d2).xyz;
    return result / sum.xxx;
}
...
float3 _noise(inout float3 v, float noise_amount)
{
...
void frag_main()
{
    ...
    {
        _362 = global_HSM_SNAP_TO_CLOSEST_INT_SCALE_TOLERANCE / 100.0f;
    }
    else
    {
        _362 = 0.0f;
    ...
    float4 _1746 = HSM_UpdateGlobalScreenValuesFromCache(InfoCachePass, _InfoCachePass_sampler, param);
    use_vert_scanlines_bool = USE_VERTICAL_SCANLINES > 0.5f;
    if (abs(HSM_ROTATE_CORE_IMAGE) > 0.5f)
    {
        use_vert_scanlines_bool = !use_vert_scanlines_bool;
    }
    float2 rotated_derezed_size = HSM_GetRotatedDerezedSize();
    float _1760;
    if (use_vert_scanlines_bool)
    {
    ...
    float2 downsample_offset = float2(0.0f, (1.0f / orig_opposite_dir_res) / NEGATIVE_CROP_EXPAND_MULTIPLIER);
    if (use_vert_scanlines_bool)
    {
        downsample_offset = float2((1.0f / orig_opposite_dir_res) / NEGATIVE_CROP_EXPAND_MULTIPLIER, 0.0f);
    }
    if ((HSM_DOWNSAMPLE_BLUR_SCANLINE_DIR + HSM_DOWNSAMPLE_BLUR_OPPOSITE_DIR) > 0.0f)
    {
        float2 param_1 = vTexCoord;
        c1 = fetch_pixel(param_1);
        float2 param_2 = vTexCoord + downsample_offset;
        c2 = fetch_pixel(param_2);
    }
    else
    {
        c1 = PrePass.Sample(_PrePass_sampler, vTexCoord).xyz;
        c2 = PrePass.Sample(_PrePass_sampler, vTexCoord + downsample_offset).xyz;
    }
    float3 c = c1;
    float intera = 1.0f;
    float gamma_in = clamp(global_GAMMA_INPUT, 1.0f, 5.0f);
    float m1 = max(max(c1.x, c1.y), c1.z);
    float m2 = max(max(c2.x, c2.y), c2.z);
    float3 df = abs(c1 - c2);
    float d = max(max(df.x, df.y), df.z);
    if (global_HSM_INTERLACE_MODE == 2.0f)
    {
        d = lerp(0.100000001490116119384765625f * d, 10.0f * d, step(m1 / (m2 + 9.9999997473787516355514526367188e-05f), m2 / (m1 + 9.9999997473787516355514526367188e-05f)));
    }
    float r = m1;
    bool _1918 = global_HSM_INTERLACE_TRIGGER_RES <= derezed_scaled_opposite_dir_res;
    bool _1924;
    if (_1918)
    {
        _1924 = global_HSM_INTERLACE_MODE > 0.5f;
    }
    else
    {
        _1924 = _1918;
    ...
    {
        intera = 0.25f;
        float line_no = clamp(floor(mod(orig_opposite_dir_res * tex_coord_scan_dir, 2.0f)), 0.0f, 1.0f);
        float frame_no = clamp(floor(mod(float(params_FrameCount), 2.0f)), 0.0f, 1.0f);
        float ii = abs(line_no - frame_no);
        if (global_HSM_INTERLACE_MODE < 3.5f)
        {
            float3 param_3 = lerp(c2, c2 * c2, global_iscans.xxx);
            float param_4 = max(max(c2.x, c2.y), c2.z);
            c2 = plant(param_3, param_4);
            r = clamp(max(m1 * ii, (1.0f - (global_HSM_INTERLACE_SCANLINE_EFFECT / 100.0f)) * min(m1, m2)), 0.0f, 1.0f);
            float3 param_5 = lerp(lerp(c1, c2, min(lerp(m1, 1.0f - m2, min(m1, 1.0f - m1)) / (d + 9.9999997473787516355514526367188e-06f), 1.0f).xxx), c1, ii.xxx);
            float param_6 = r;
            c = plant(param_5, param_6);
            if (global_HSM_INTERLACE_MODE == 3.0f)
            {
                c = lerp(c2, c1, ii.xxx) * (1.0f - ((0.5f * global_HSM_INTERLACE_SCANLINE_EFFECT) / 100.0f));
            }
            intera = 0.0f;
        }
        if (global_HSM_INTERLACE_MODE == 4.0f)
        {
            float3 param_7 = lerp(c, c * c, (0.5f * global_iscans).xxx);
            float param_8 = max(max(c.x, c.y), c.z);
            c = plant(param_7, param_8);
            intera = 0.449999988079071044921875f;
        }
        if (global_HSM_INTERLACE_MODE == 5.0f)
        {
            c = lerp(c2, c1, 0.5f.xxx);
            float3 param_9 = lerp(c, c * c, (0.5f * global_iscans).xxx);
            float param_10 = max(max(c.x, c.y), c.z);
            c = plant(param_9, param_10);
            intera = 0.449999988079071044921875f;
        }
    }
    c = pow(c, gamma_in.xxx);
    if (global_HSM_SIGNAL_NOISE_ON > 0.5f)
    {
        float rc = (0.60000002384185791015625f * sqrt(max(max(c.x, c.y), c.z))) + 0.4000000059604644775390625f;
        float2 upscale_mult = global_SourceSize.xy / global_DerezedPassSize.xy;
        float noise_size = 1.0f;
        if (global_HSM_SIGNAL_NOISE_SIZE_MODE == 0.0f)
        {
            noise_size = (upscale_mult.x * global_HSM_SIGNAL_NOISE_SIZE_MULT) / SAMPLING_OPPOSITE_DIR_MULT;
        }
        if (global_HSM_SIGNAL_NOISE_SIZE_MODE == 1.0f)
        {
            noise_size = upscale_mult.x * global_HSM_SIGNAL_NOISE_SIZE_MULT;
        }
        if (global_HSM_SIGNAL_NOISE_SIZE_MODE == 2.0f)
        {
            noise_size = global_HSM_SIGNAL_NOISE_SIZE_MULT;
        }
        float3 param_11 = float3(floor((global_SourceSize.xy * vTexCoord) / noise_size.xx), float(global_FrameCount));
        float param_12 = global_HSM_SIGNAL_NOISE_AMOUNT;
        float3 _2173 = _noise(param_11, param_12);
        float3 noise0 = _2173;
        noise0 = clamp(((noise0 - 0.5f.xxx) * (global_HSM_SIGNAL_NOISE_BLACK_LEVEL / 100.0f)) + 0.5f.xxx, 0.0f.xxx, 1.0f.xxx);
        if (global_HSM_SIGNAL_NOISE_TYPE < 0.5f)
        {
            c = lerp(c, noise0, ((0.25f * abs(global_HSM_SIGNAL_NOISE_AMOUNT)) * rc).xxx);
        }
        else
        {
            c = min(c * lerp(1.0f, 1.5f * noise0.x, 0.5f * abs(global_HSM_SIGNAL_NOISE_AMOUNT)), 1.0f.xxx);
        }
    }
    if (vTexCoord.x > 0.5f)
    {
    ...
    {
        gamma_in = 1.0f / gamma_in;
    }
    FragColor = float4(c, gamma_in);
}
