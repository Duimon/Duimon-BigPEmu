...
bool HSM_GetIsInABCompareArea(float2 viewport_coord)
{
    ...
    {
        _5976 = 1.0f - HSM_AB_COMPARE_SPLIT_POSITION;
    }
    else
    {
    ...
    bool _5990 = (mod(HSM_AB_COMPARE_AREA, 2.0f) == 0.0f) && (test_value < position);
    bool _6001;
    if (!_5990)
    {
        _6001 = (mod(HSM_AB_COMPARE_AREA, 2.0f) == 1.0f) && (test_value > position);
    }
    else
    {
        _6001 = _5990;
...
float2 GetSimpleImageScaledCoord(inout float2 in_viewport_coord, float2 in_viewport_unscaled_coord, float2 in_tube_coord, float2 in_tube_scale, Texture2D<float4> in_sampler, SamplerState _in_sampler_sampler, float2 in_pos, float in_inherit_pos, float2 in_scale, float in_scale_inherit_mode, float in_keep_aspect, float in_mirror_horz, float in_rotate)
{
    ...
    uint _5016_dummy_parameter;
    float2 tex_size = float2(int2(spvTextureSize(in_sampler, uint(0), _5016_dummy_parameter)));
    float _5021;
    if (in_rotate == 1.0f)
    {
    ...
    if (in_rotate > 0.5f)
    {
        _5052 = global_FinalViewportSize.x / global_FinalViewportSize.y;
    }
    else
    {
        _5052 = 1.0f;
...
{
    uint _6007_dummy_parameter;
    float2 tex_size = float2(int2(spvTextureSize(in_sampler, uint(0), _6007_dummy_parameter)));
...
float3 HSM_RGBtoHSV(float3 c)
{
    ...
    if (c.y < c.z)
    {
        _2624 = float4(c.zy, K.wz);
    }
    else
    {
        _2624 = float4(c.yz, K.xy);
    }
    float4 p = _2624;
    float4 _2654;
    if (c.x < p.x)
    {
        _2654 = float4(p.xyw, c.x);
    }
    else
    {
        _2654 = float4(c.x, p.yzx);
    }
    float4 q = _2654;
...
float4 HSM_GetNightLightingMultiplyColor(float2 in_coord, float hue, float saturation, float value, float contrast, float global_ambient_opacity, Texture2D<float4> NightLightingImage_1, SamplerState _NightLightingImage_1_sampler)
{
    ...
    float4 _6136 = lighting_image;
    float3 _6140 = lerp(1.0f.xxx, _6136.xyz, global_ambient_opacity.xxx);
...
bool HSM_Fill_Ambient_Images(float2 in_viewport_coord, float2 in_viewport_unscaled_coord, float2 in_tube_coord, float2 in_tube_scale, float in_swap_images, Texture2D<float4> in_ambient_sampler, SamplerState _in_ambient_sampler_sampler, Texture2D<float4> in_ambient2_sampler, SamplerState _in_ambient2_sampler_sampler, out float4 ambient_lighting_image, out float4 ambient2_lighting_image)
{
    ...
    if (HSM_AMBIENT1_OPACITY > 0.0f)
    {
        ...
        bool _6158 = HSM_AMBIENT1_SCALE_INHERIT_MODE == 1.0f;
        bool _6165;
        if (_6158)
        {
            _6165 = (HSM_AMBIENT1_SCALE * HSM_VIEWPORT_ZOOM) < 1.0f;
        }
        else
        {
            _6165 = _6158;
        ...
        float2 _6199 = GetSimpleImageScaledCoord(param, param_1, param_2, param_3, in_ambient_sampler, _in_ambient_sampler_sampler, param_4, param_5, param_6, param_7, param_8, param_9, param_10);
        float2 lighting_coord = _6199;
    ...
    if (HSM_AMBIENT2_OPACITY > 0.0f)
    {
        ...
        bool _6220 = HSM_AMBIENT2_SCALE_INHERIT_MODE == 1.0f;
        bool _6227;
        if (_6220)
        {
            _6227 = (HSM_AMBIENT2_SCALE * HSM_VIEWPORT_ZOOM) < 1.0f;
        }
        else
        {
            _6227 = _6220;
        ...
        float2 _6261 = GetSimpleImageScaledCoord(param_17, param_18, param_19, param_20, in_ambient2_sampler, _in_ambient2_sampler_sampler, param_21, param_22, param_23, param_24, param_25, param_26, param_27);
        float2 lighting2_coord = _6261;
...
float3 maxscale(float2 sinangle, float2 cosangle, float in_radius, float in_distance, float in_aspect)
{
    ...
    float2 _5378 = fwtrans(param_5, param_6, param_7, param_8, param_9);
    ...
    float2 _5395 = fwtrans(param_10, param_11, param_12, param_13, param_14);
    float2 lo = float2(_5378.x, _5395.y) / aspect_vec2;
    ...
    float2 _5415 = fwtrans(param_15, param_16, param_17, param_18, param_19);
    ...
    float2 _5431 = fwtrans(param_20, param_21, param_22, param_23, param_24);
    float2 hi = float2(_5415.x, _5431.y) / aspect_vec2;
...
float2 HSM_GetGeomCurvedCoord(float2 in_coord, float tilt_x, float tilt_y, float in_radius, inout float in_distance, float in_aspect)
{
    ...
    float2 _5528 = transform(param_5, param_6, param_7, param_8, param_9, param_10, param_11);
    float2 curved_coord = _5528;
    return curved_coord;
}
...
float2 HSM_GetGeomCurvedCoordRetainWidth(float2 in_coord, float tilt_x, float tilt_y, float in_radius, float in_distance, float in_aspect)
{
    ...
    float2 _5545 = HSM_GetGeomCurvedCoord(param, param_1, param_2, param_3, param_4, param_5);
    float2 ctr_curved_coord = _5545 - 0.5f.xx;
    ...
    float2 _5560 = HSM_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11);
    float2 right_edge_curved_ctr_coord = _5560 - 0.5f.xx;
...
float3 hrg_get_ideal_global_eye_pos_for_points(inout float3 eye_pos, float2 output_aspect, float3 global_coords[9], int num_points, float in_geom_radius, float in_geom_view_dist)
{
    ...
    for (int iter = 0; iter < max_centering_iters; iter++)
    {
        ...
        float3 _3958 = eye_pos;
        float2 _3960 = _3958.xy + center_offset;
        eye_pos.x = _3960.x;
        eye_pos.y = _3960.y;
...
float3 hrg_get_ideal_global_eye_pos(float3x3 local_to_global, float2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist)
{
    ...
    if (in_geom_mode < 2.5f)
    {
        _4119 = float3(0.0f, 0.0f, eye_z_spherical);
    }
    else
    {
        _4119 = float3(0.0f, 0.0f, max(in_geom_view_dist, eye_z_spherical));
    }
    float3 eye_pos = _4119;
    int num_points = 9;
    ...
    else
    {
        ...
        float3 _4278 = hrg_get_ideal_global_eye_pos_for_points(param_36, param_37, param_38, param_39, param_40, param_41);
        _4261 = _4278;
    }
    return _4261;
}
...
float2 hrg_get_curved_video_uv_coords_and_tangent_matrix(float2 flat_video_uv, float3 eye_pos_local, float2 output_pixel_size, float2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, float3x3 global_to_local, out float2x2 pixel_to_tangent_video_uv)
{
    ...
    float3 param_5;
    float2 _4503 = hrg_view_vec_to_uv(param, param_1, param_2, param_3, param_4, param_5);
    float3 pos = param_5;
    float2 centered_uv = _4503;
    ...
    if (geom_force_correct_tangent_matrix)
    {
        ...
        {
            _4515 = float3(pos.x, 0.0f, pos.z);
        }
        float3 normal_base = _4515;
...
float2 HRG_GetGeomCurvedCoord(float2 in_coord, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, float in_geom_tilt_angle_x, float in_geom_tilt_angle_y, float in_screen_aspect, float pin_inner_edge, float2 in_source_size, float2 in_output_size, out float2x2 pixel_to_video_uv)
{
    ...
    if (in_geom_mode > 0.5f)
    {
        ...
        float2x2 param_14;
        float2 _4683 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14);
        pixel_to_video_uv = param_14;
        float2 ctr_curved_coord = _4683 - 0.5f.xx;
        if ((in_geom_tilt_angle_x == 0.0f) && (in_geom_tilt_angle_y == 0.0f))
        {
            ...
            float2x2 param_23;
            float2 _4712 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_15, param_16, param_17, param_18, param_19, param_20, param_21, param_22, param_23);
            pixel_to_video_uv = param_23;
            float2 right_edge_curved_ctr_coord = _4712 - 0.5f.xx;
            ...
            float2x2 param_32;
            float2 _4734 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_24, param_25, param_26, param_27, param_28, param_29, param_30, param_31, param_32);
            pixel_to_video_uv = param_32;
            float2 bottom_edge_curved_ctr_coord = _4734 - 0.5f.xx;
            ctr_curved_coord.x = (ctr_curved_coord.x * 0.5f) / right_edge_curved_ctr_coord.x;
        ...
        if (pin_inner_edge == 1.0f)
        {
            if (in_geom_tilt_angle_y != 0.0f)
            {
                ...
                float2x2 param_41;
                float2 _4777 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_33, param_34, param_35, param_36, param_37, param_38, param_39, param_40, param_41);
                pixel_to_video_uv = param_41;
                float2 top_edge_curved_ctr_coord = _4777;
            ...
            if (in_geom_tilt_angle_x != 0.0f)
            {
                ...
                float2x2 param_50;
                float2 _4806 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_42, param_43, param_44, param_45, param_46, param_47, param_48, param_49, param_50);
                pixel_to_video_uv = param_50;
                float2 left_edge_curved_ctr_coord = _4806;
...
float2 HSM_GetCurvatureValues(float screen_aspect)
{
    ...
    {
        _5090 = float2((((2.0f * global_HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f, ((global_HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f);
    }
    else
    {
        _5090 = float2(((global_HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f, (((2.0f * global_HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f);
    }
    float2 curvature_values = _5090;
...
float2 HSM_Get2DCurvedCoord(float2 in_coord, float2 curvature_values)
{
    ...
    float2 _5624 = HSM_GetCrtPiCurvedCoord(param, param_1);
    ctr_curved_coord = _5624 - 0.5f.xx;
    float2 param_2 = float2(1.0f, 0.5f);
    float2 param_3 = curvature_values;
    float2 _5631 = HSM_GetCrtPiCurvedCoord(param_2, param_3);
    float2 right_edge_curved_ctr_coord = _5631 - 0.5f.xx;
    ctr_curved_coord.x = (ctr_curved_coord.x * 0.5f) / right_edge_curved_ctr_coord.x;
    float2 param_4 = float2(0.5f, 1.0f);
    float2 param_5 = curvature_values;
    float2 _5645 = HSM_GetCrtPiCurvedCoord(param_4, param_5);
    float2 bottom_edge_curved_ctr_coord = _5645 - 0.5f.xx;
    ctr_curved_coord.y = (ctr_curved_coord.y * 0.5f) / bottom_edge_curved_ctr_coord.y;
    return ctr_curved_coord + 0.5f.xx;
}
...
float2 HSM_GetCurvedCoord(float2 in_coord, float curvature_multiplier, float screen_aspect)
{
    ...
    if (HSM_CURVATURE_MODE > CURVATURE_MODE_2D_CYLINDER)
    {
        ...
        else
        {
            ...
            float2x2 param_16;
            float2 _5753 = HRG_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14, param_15, param_16);
...
float2 HSM_GetTubeCurvedCoord(float2 screen_coord, float curvature_scale, float2 screen_scale, float2 tube_scale, float screen_aspect, float apply_black_edge_offset)
{
    ...
    if (global_HSM_BZL_USE_INDEPENDENT_CURVATURE == 1.0f)
    {
        ...
        if (screen_aspect < 1.0f)
        {
            _5892 = float2((((2.0f * global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f, ((global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f);
        }
        else
        {
            _5892 = float2(((global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f, (((2.0f * global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f);
        }
        float2 curvature_values = _5892;
...
float HSM_GetBezelCoords(float2 tube_diffuse_coord, float2 tube_diffuse_scale, float2 tube_scale, float screen_aspect, bool curve_coords_on, out float2 bezel_outside_scale, inout float2 bezel_outside_coord, inout float2 bezel_outside_curved_coord, inout float2 frame_outside_curved_coord)
{
    ...
    if (curve_coords_on)
    {
        bool _6421 = HSM_CURVATURE_MODE > 0.0f;
        bool _6429;
        if (_6421)
        {
            _6429 = (HSM_CURVATURE_3D_TILT_ANGLE_X != 0.0f) || (HSM_CURVATURE_3D_TILT_ANGLE_Y != 0.0f);
        }
        else
        {
            _6429 = _6421;
...
float4 HSM_GetPreMultipliedColorLinear(float4 in_color, float matte_type, float encoded_gamma)
{
    ...
    {
        float4 _2830 = out_color;
        float _2833 = out_color.w;
        float3 _2839 = clamp(_2830.xyz - (1.0f - _2833).xxx, 0.0f.xxx, 1.0f.xxx);
        out_color.x = _2839.x;
        out_color.y = _2839.y;
        out_color.z = _2839.z;
    }
    ...
    if (matte_type == SOURCE_MATTE_NONE)
    {
        ...
        float4 _2858 = out_color;
        float3 _2860 = _2858.xyz * _2857;
        out_color.x = _2860.x;
        out_color.y = _2860.y;
        out_color.z = _2860.z;
    }
...
float3 ApplyAmbientImages(float3 base_image, inout float3 ambient_image, inout float3 ambient2_image, float blend_ambient, float blend_ambient2, float apply_in_add_mode, float layer_blend_mode, float swap_images)
{
    ...
    bool _6301 = (HSM_AMBIENT1_OPACITY > 0.0f) || (HSM_AMBIENT2_OPACITY > 0.0f);
    bool _6309;
    if (_6301)
    {
        _6309 = (blend_ambient > 0.0f) || (blend_ambient2 > 0.0f);
    }
    else
    {
        _6309 = _6301;
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    _6613 = (mask_mode == 8.0f) ? OUTSIDE_FRAME_MASK : 0.5f;
                                }
                                _6605 = _6613;
                            }
                            _6598 = _6605;
...
{
    bool _6634 = blend_mode == 0.0f;
    bool _6644;
    if (!_6634)
    {
        _6644 = (dualscreen_mode != 0.0f) && (dualscreen_mode != SCREEN_INDEX);
    }
    else
    {
        _6644 = _6634;
    ...
    if (blend_mode == BLEND_MODE_NORMAL)
    {
        ...
        float4 _6687 = color_over;
        float3 _6689 = _6687.xyz * _6686;
    ...
    else
    {
        ...
        {
            float3 _6713 = color_under.xyz + color_over.xyz;
        ...
        {
            float3 _6729 = color_under.xyz * color_over.xyz;
...
void frag_main()
{
    ...
    bool _7042 = HSM_AB_COMPARE_FREEZE_GRAPHICS == 1.0f;
    bool _7050;
    if (_7042)
    {
    ...
    float4 _7069 = HSM_UpdateGlobalScreenValuesFromCache(InfoCachePass, _InfoCachePass_sampler, param_3);
    float4 feedback_color_test = BR_LayersOverCRTPassFeedback.Sample(_BR_LayersOverCRTPassFeedback_sampler, 0.0f.xx);
    bool _7074 = HSM_CACHE_GRAPHICS_ON > 0.5f;
    bool _7080;
    if (_7074)
    {
        _7080 = feedback_color_test.w < 0.0f;
    }
    else
    {
        _7080 = _7074;
    ...
    bool _7109 = HSM_Fill_Ambient_Images(param_4, param_5, param_6, param_7, param_8, NightLightingImage, _NightLightingImage_sampler, NightLighting2Image, _NightLighting2Image_sampler, param_9, param_10);
    ...
    bool _7332 = SCREEN_ASPECT < 1.0f;
    bool _7341;
    if (_7332)
    {
        uint _7338_dummy_parameter;
        _7341 = int2(spvTextureSize(BackgroundVertImage, uint(0), _7338_dummy_parameter)).y > 16;
    }
    else
    {
        _7341 = _7332;
    ...
    {
        uint _7351_dummy_parameter;
        _7346 = int2(spvTextureSize(BackgroundVertImage, uint(0), _7351_dummy_parameter));
    }
    else
    {
        uint _7356_dummy_parameter;
        _7346 = int2(spvTextureSize(BackgroundImage, uint(0), _7356_dummy_parameter));
    }
    float2 bg_size = float2(_7346);
    ...
    float2 _7397 = HSM_GetScaledCoord(param_57, param_58, param_59, param_60, param_61, param_62, param_63, param_64, param_65, param_66, param_67, param_68, param_69, param_70, param_71);
    ...
    bool _7416 = HSM_BG_OPACITY > 0.0f;
    bool _7423;
    if (_7416)
    {
        _7423 = bg_size.y > 16.0f;
    }
    else
    {
        _7423 = _7416;
    ...
    if (_7423)
    {
        ...
        float3 _7463 = HSM_ApplyHSVAdjustment(param_82, param_83, param_84, param_85, param_86, param_87);
        bg_image.x = _7463.x;
        bg_image.y = _7463.y;
        bg_image.z = _7463.z;
        ...
        float3 _7498 = ApplyAmbientImages(param_90, param_91, param_92, param_93, param_94, param_95, param_96, param_97);
        bg_image.x = _7498.x;
        bg_image.y = _7498.y;
        bg_image.z = _7498.z;
    }
    bool _7507 = SCREEN_ASPECT < 1.0f;
    bool _7516;
    if (_7507)
    {
        uint _7513_dummy_parameter;
        _7516 = int2(spvTextureSize(DeviceVertImage, uint(0), _7513_dummy_parameter)).y > 16;
    }
    else
    {
        _7516 = _7507;
    ...
    {
        uint _7525_dummy_parameter;
        _7520 = int2(spvTextureSize(DeviceVertImage, uint(0), _7525_dummy_parameter));
    }
    else
    {
        uint _7530_dummy_parameter;
        _7520 = int2(spvTextureSize(DeviceImage, uint(0), _7530_dummy_parameter));
    }
    float2 device_size = float2(_7520);
    ...
    float2 _7571 = HSM_GetScaledCoord(param_98, param_99, param_100, param_101, param_102, param_103, param_104, param_105, param_106, param_107, param_108, param_109, param_110, param_111, param_112);
    DEVICE_COORD = param_111;
    DEVICE_SCALE = param_112;
    DEVICE_CURVED_COORD = _7571;
    float4 device_image = 0.0f.xxxx;
    bool _7576 = HSM_DEVICE_OPACITY > 0.0f;
    bool _7582;
    if (_7576)
    {
        _7582 = device_size.y > 16.0f;
    }
    else
    {
        _7582 = _7576;
    ...
    if (_7582)
    {
        ...
        if (device_use_vert_image)
        {
            ...
            _7586 = HSM_GetMipmappedTexSample(DeviceVertImage, _DeviceVertImage_sampler, param_113, param_114, param_115);
        }
        else
        {
            ...
            _7586 = HSM_GetMipmappedTexSample(DeviceImage, _DeviceImage_sampler, param_116, param_117, param_118);
        }
        device_image = _7586;
        ...
        float3 _7624 = HSM_ApplyHSVAdjustment(param_122, param_123, param_124, param_125, param_126, param_127);
        ...
        float3 _7659 = ApplyAmbientImages(param_130, param_131, param_132, param_133, param_134, param_135, param_136, param_137);
        device_image.x = _7659.x;
        device_image.y = _7659.y;
        device_image.z = _7659.z;
    }
    uint _7670_dummy_parameter;
    float2 deviceled_size = float2(int2(spvTextureSize(DeviceLEDImage, uint(0), _7670_dummy_parameter)));
    ...
    float2 _7710 = HSM_GetScaledCoord(param_138, param_139, param_140, param_141, param_142, param_143, param_144, param_145, param_146, param_147, param_148, param_149, param_150, param_151, param_152);
    DEVICELED_COORD = param_151;
    DEVICELED_SCALE = param_152;
    DEVICELED_CURVED_COORD = _7710;
    float4 deviceled_image = 0.0f.xxxx;
    bool _7715 = HSM_DEVICELED_OPACITY > 0.0f;
    bool _7721;
    if (_7715)
    {
        _7721 = deviceled_size.y > 16.0f;
    }
    else
    {
        _7721 = _7715;
    ...
    if (_7721)
    {
        ...
        float3 _7750 = HSM_ApplyHSVAdjustment(param_159, param_160, param_161, param_162, param_163, param_164);
        ...
        float3 _7785 = ApplyAmbientImages(param_167, param_168, param_169, param_170, param_171, param_172, param_173, param_174);
    ...
    {
        uint _7801_dummy_parameter;
        float2 led_size = float2(int2(spvTextureSize(LEDImage, uint(0), _7801_dummy_parameter)));
        ...
        float2 _7841 = HSM_GetScaledCoord(param_175, param_176, param_177, param_178, param_179, param_180, param_181, param_182, param_183, param_184, param_185, param_186, param_187, param_188, param_189);
        LED_COORD = param_188;
        LED_SCALE = param_189;
        LED_CURVED_COORD = _7841;
        bool _7845 = HSM_LED_OPACITY > 0.0f;
        bool _7851;
        if (_7845)
        {
            _7851 = led_size.y > 16.0f;
        }
        else
        {
            _7851 = _7845;
        ...
        if (_7851)
        {
            ...
            float3 _7880 = HSM_ApplyHSVAdjustment(param_196, param_197, param_198, param_199, param_200, param_201);
            led_image.x = _7880.x;
            led_image.y = _7880.y;
            led_image.z = _7880.z;
            if (HSM_STATIC_LAYERS_GAMMA != 1.0f)
            ...
            float3 _7915 = ApplyAmbientImages(param_204, param_205, param_206, param_207, param_208, param_209, param_210, param_211);
            led_image.x = _7915.x;
            led_image.y = _7915.y;
            led_image.z = _7915.z;
        }
    }
    uint _7926_dummy_parameter;
    float2 decal_size = float2(int2(spvTextureSize(DecalImage, uint(0), _7926_dummy_parameter)));
    ...
    float2 _7966 = HSM_GetScaledCoord(param_212, param_213, param_214, param_215, param_216, param_217, param_218, param_219, param_220, param_221, param_222, param_223, param_224, param_225, param_226);
    DECAL_COORD = param_225;
    DECAL_SCALE = param_226;
    DECAL_CURVED_COORD = _7966;
    float4 decal_image = 0.0f.xxxx;
    bool _7971 = HSM_DECAL_OPACITY > 0.0f;
    bool _7977;
    if (_7971)
    {
        _7977 = decal_size.y > 16.0f;
    }
    else
    {
        _7977 = _7971;
    ...
    if (_7977)
    {
        ...
        float3 _8006 = HSM_ApplyHSVAdjustment(param_233, param_234, param_235, param_236, param_237, param_238);
        ...
        float3 _8041 = ApplyAmbientImages(param_241, param_242, param_243, param_244, param_245, param_246, param_247, param_248);
        decal_image.x = _8041.x;
        decal_image.y = _8041.y;
        decal_image.z = _8041.z;
    }
    uint _8052_dummy_parameter;
    float2 top_size = float2(int2(spvTextureSize(TopLayerImage, uint(0), _8052_dummy_parameter)));
    ...
    float2 _8092 = HSM_GetScaledCoord(param_249, param_250, param_251, param_252, param_253, param_254, param_255, param_256, param_257, param_258, param_259, param_260, param_261, param_262, param_263);
    ...
    bool _8104 = HSM_TOP_OPACITY > 0.0f;
    bool _8110;
    if (_8104)
    {
        _8110 = top_size.y > 16.0f;
    }
    else
    {
        _8110 = _8104;
    ...
    if (_8110)
    {
        ...
        float3 _8139 = HSM_ApplyHSVAdjustment(param_271, param_272, param_273, param_274, param_275, param_276);
        top_image.x = _8139.x;
        top_image.y = _8139.y;
        top_image.z = _8139.z;
        if (HSM_STATIC_LAYERS_GAMMA != 1.0f)
        ...
        float3 _8174 = ApplyAmbientImages(param_279, param_280, param_281, param_282, param_283, param_284, param_285, param_286);
        top_image.x = _8174.x;
        top_image.y = _8174.y;
        top_image.z = _8174.z;
    }
    ...
    {
        uint _8190_dummy_parameter;
        float2 cab_glass_size = float2(int2(spvTextureSize(CabinetGlassImage, uint(0), _8190_dummy_parameter)));
        ...
        float2 _8230 = HSM_GetScaledCoord(param_287, param_288, param_289, param_290, param_291, param_292, param_293, param_294, param_295, param_296, param_297, param_298, param_299, param_300, param_301);
        CAB_GLASS_COORD = param_300;
        CAB_GLASS_SCALE = param_301;
        CAB_GLASS_CURVED_COORD = _8230;
        bool _8234 = HSM_CAB_GLASS_OPACITY > 0.0f;
        bool _8240;
        if (_8234)
        {
            _8240 = cab_glass_size.y > 16.0f;
        }
        else
        {
            _8240 = _8234;
        ...
        if (_8240)
        {
            ...
            float3 _8269 = HSM_ApplyHSVAdjustment(param_308, param_309, param_310, param_311, param_312, param_313);
            ...
            float3 _8304 = ApplyAmbientImages(param_316, param_317, param_318, param_319, param_320, param_321, param_322, param_323);
    ...
    float2 _8366 = HSM_GetScaledCoord(param_324, param_325, param_326, param_327, param_328, param_329, param_330, param_331, param_332, param_333, param_334, param_335, param_336, param_337, param_338);
    temp_coord = param_337;
    temp_scale = param_338;
    float2 cutout_coord = _8366;
    ...
    if (HSM_VIEWPORT_VIGNETTE_OPACITY > 0.0f)
    {
        ...
        float2 _8418 = HSM_GetScaledCoord(param_343, param_344, param_345, param_346, param_347, param_348, param_349, param_350, param_351, param_352, param_353, param_354, param_355, param_356, param_357);
        temp_coord = param_356;
        temp_scale = param_357;
        float2 vignette_coord = _8418;
    ...
    for (int i = start_layer; i <= end_layer; i++)
    {
        if (HSM_BG_LAYER_ORDER == float(i))
        {
            ...
            float4 _8488 = BlendModeMaskLayerMix(param_367, param_368, param_369, param_370, param_371, param_372, param_373);
        ...
        if (HSM_VIEWPORT_VIGNETTE_LAYER_ORDER == float(i))
        {
            ...
            float4 _8508 = BlendModeMaskLayerMix(param_374, param_375, param_376, param_377, param_378, param_379, param_380);
        ...
        if ((HSM_LED_LAYER_ORDER == float(i)) && (HSM_LED_OPACITY > 0.0f))
        {
            ...
            float4 _8532 = BlendModeMaskLayerMix(param_381, param_382, param_383, param_384, param_385, param_386, param_387);
        ...
        if ((HSM_DEVICE_LAYER_ORDER == float(i)) && (HSM_DEVICE_OPACITY > 0.0f))
        {
            ...
            float4 _8556 = BlendModeMaskLayerMix(param_388, param_389, param_390, param_391, param_392, param_393, param_394);
        ...
        if ((HSM_DEVICELED_LAYER_ORDER == float(i)) && (HSM_DEVICELED_OPACITY > 0.0f))
        {
            ...
            float4 _8580 = BlendModeMaskLayerMix(param_395, param_396, param_397, param_398, param_399, param_400, param_401);
        ...
        if ((HSM_DECAL_LAYER_ORDER == float(i)) && (HSM_DECAL_OPACITY > 0.0f))
        {
            ...
            float4 _8604 = BlendModeMaskLayerMix(param_402, param_403, param_404, param_405, param_406, param_407, param_408);
        ...
        if ((HSM_CAB_GLASS_LAYER_ORDER == float(i)) && (HSM_CAB_GLASS_OPACITY > 0.0f))
        {
            ...
            float4 _8628 = BlendModeMaskLayerMix(param_409, param_410, param_411, param_412, param_413, param_414, param_415);
        ...
        if ((HSM_TOP_LAYER_ORDER == float(i)) && (HSM_TOP_OPACITY > 0.0f))
        {
            ...
            float4 _8652 = BlendModeMaskLayerMix(param_416, param_417, param_418, param_419, param_420, param_421, param_422);
    ...
    bool _8696 = UNFLIPPED_VIEWPORT_COORD.x < (2.0f / global_OutputSize.x);
    bool _8705;
    if (_8696)
    {
        _8705 = UNFLIPPED_VIEWPORT_COORD.y < (2.0f / global_OutputSize.y);
    }
    else
    {
        _8705 = _8696;
