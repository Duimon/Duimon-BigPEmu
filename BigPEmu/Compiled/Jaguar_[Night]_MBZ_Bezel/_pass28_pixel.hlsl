...
bool HSM_GetIsInABCompareArea(float2 viewport_coord)
{
    ...
    {
        _4941 = viewport_coord.y;
    }
    else
    {
        _4941 = viewport_coord.x;
    }
    float test_value = _4941;
    float _4954;
    if (mod(HSM_AB_COMPARE_AREA, 2.0f) == 1.0f)
    {
        _4954 = 1.0f - HSM_AB_COMPARE_SPLIT_POSITION;
    }
    else
    {
        _4954 = HSM_AB_COMPARE_SPLIT_POSITION;
    }
    float position = _4954;
    bool _4968 = (mod(HSM_AB_COMPARE_AREA, 2.0f) == 0.0f) && (test_value < position);
    bool _4979;
    if (!_4968)
    {
        _4979 = (mod(HSM_AB_COMPARE_AREA, 2.0f) == 1.0f) && (test_value > position);
    }
    else
    {
        _4979 = _4968;
...
float3 maxscale(float2 sinangle, float2 cosangle, float in_radius, float in_distance, float in_aspect)
{
    ...
    float2 _4030 = fwtrans(param_5, param_6, param_7, param_8, param_9);
    ...
    float2 _4047 = fwtrans(param_10, param_11, param_12, param_13, param_14);
    float2 lo = float2(_4030.x, _4047.y) / aspect_vec2;
    ...
    float2 _4067 = fwtrans(param_15, param_16, param_17, param_18, param_19);
    ...
    float2 _4083 = fwtrans(param_20, param_21, param_22, param_23, param_24);
    float2 hi = float2(_4067.x, _4083.y) / aspect_vec2;
...
float2 HSM_GetGeomCurvedCoord(float2 in_coord, float tilt_x, float tilt_y, float in_radius, inout float in_distance, float in_aspect)
{
    ...
    float2 _4180 = transform(param_5, param_6, param_7, param_8, param_9, param_10, param_11);
    float2 curved_coord = _4180;
    return curved_coord;
}
...
float2 HSM_GetGeomCurvedCoordRetainWidth(float2 in_coord, float tilt_x, float tilt_y, float in_radius, float in_distance, float in_aspect)
{
    ...
    float2 _4197 = HSM_GetGeomCurvedCoord(param, param_1, param_2, param_3, param_4, param_5);
    float2 ctr_curved_coord = _4197 - 0.5f.xx;
    ...
    float2 _4212 = HSM_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11);
    float2 right_edge_curved_ctr_coord = _4212 - 0.5f.xx;
...
float3 hrg_uv_to_xyz(float2 uv, float2 output_aspect, float in_geom_mode, float in_geom_radius)
{
    ...
    if (in_geom_mode < 1.5f)
    {
        ...
        _2689 = hrg_sphere_uv_to_xyz(param, param_1, param_2);
    }
    else
    {
        float3 _2702;
        if (in_geom_mode < 2.5f)
        {
            ...
            _2702 = hrg_sphere_alt_uv_to_xyz(param_3, param_4, param_5);
        }
        else
        {
            float2 param_6 = uv;
            ...
            _2702 = hrg_cylinder_uv_to_xyz(param_6, param_7, param_8);
        }
        _2689 = _2702;
    }
    return _2689;
...
float3 hrg_get_ideal_global_eye_pos_for_points(inout float3 eye_pos, float2 output_aspect, float3 global_coords[9], int num_points, float in_geom_radius, float in_geom_view_dist)
{
    ...
    for (int iter = 0; iter < max_centering_iters; iter++)
    {
        ...
        float3 _2867 = eye_pos;
        float2 _2869 = _2867.xy + center_offset;
        eye_pos.x = _2869.x;
        eye_pos.y = _2869.y;
        ...
        for (int i_3 = 0; i_3 < num_points; i_3++)
        {
            ...
            {
                _2933 = max(offset_z_max, offset_zzzz.x);
            }
            else
            {
                _2933 = offset_z_max;
            ...
            {
                _2946 = max(offset_z_max, offset_zzzz.y);
            }
            else
            {
                _2946 = offset_z_max;
            ...
            if (eyespace_xyz_flipy.x > 0.0f)
            {
                _2959 = max(offset_z_max, offset_zzzz.z);
            }
            else
            {
                _2959 = offset_z_max;
            ...
            if (eyespace_xyz_flipy.y > 0.0f)
            {
                _2972 = max(offset_z_max, offset_zzzz.w);
            }
            else
            {
                _2972 = offset_z_max;
...
float3 hrg_get_ideal_global_eye_pos(float3x3 local_to_global, float2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist)
{
    ...
    if (in_geom_mode < 2.5f)
    {
        _3028 = float3(0.0f, 0.0f, eye_z_spherical);
    }
    else
    {
        _3028 = float3(0.0f, 0.0f, max(in_geom_view_dist, eye_z_spherical));
    }
    float3 eye_pos = _3028;
    int num_points = 9;
    ...
    else
    {
        ...
        float3 _3187 = hrg_get_ideal_global_eye_pos_for_points(param_36, param_37, param_38, param_39, param_40, param_41);
        _3170 = _3187;
    }
    return _3170;
}
...
float2 hrg_intersect(float3 view_vec_local, float3 eye_pos_local, float in_geom_mode, float in_geom_radius)
{
    ...
    if (in_geom_mode < 2.5f)
    {
        ...
        float param_2 = in_geom_radius;
        _2628 = hrg_intersect_sphere(param, param_1, param_2);
    }
    else
    {
        ...
        float param_5 = in_geom_radius;
        _2628 = hrg_intersect_cylinder(param_3, param_4, param_5);
    }
    return _2628;
}
...
float2 hrg_xyz_to_uv(float3 intersection_pos_local, float2 output_aspect, float in_geom_mode, float in_geom_radius)
{
    ...
    if (in_geom_mode < 1.5f)
    {
        ...
        float2 param_1 = output_aspect;
        float param_2 = in_geom_radius;
        _2652 = hrg_sphere_xyz_to_uv(param, param_1, param_2);
    }
    else
    {
        float2 _2665;
        if (in_geom_mode < 2.5f)
        {
            ...
            float2 param_4 = output_aspect;
            float param_5 = in_geom_radius;
            _2665 = hrg_sphere_alt_xyz_to_uv(param_3, param_4, param_5);
        }
        else
        {
            ...
            float2 param_7 = output_aspect;
            float param_8 = in_geom_radius;
            _2665 = hrg_cylinder_xyz_to_uv(param_6, param_7, param_8);
        }
        _2652 = _2665;
    }
    return _2652;
...
float2 hrg_view_vec_to_uv(float3 view_vec_local, float3 eye_pos_local, float2 output_aspect, float in_geom_mode, float in_geom_radius, out float3 intersection_pos)
{
    ...
    if (intersect_dist_and_discriminant.y > 0.004999999888241291046142578125f)
    {
        ...
        float param_6 = in_geom_mode;
        float param_7 = in_geom_radius;
        _2746 = hrg_xyz_to_uv(param_4, param_5, param_6, param_7);
    }
    else
    {
        _2746 = 1.0f.xx;
...
float2 hrg_get_curved_video_uv_coords_and_tangent_matrix(float2 flat_video_uv, float3 eye_pos_local, float2 output_pixel_size, float2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, float3x3 global_to_local, out float2x2 pixel_to_tangent_video_uv)
{
    ...
    float3 param_5;
    float2 _3412 = hrg_view_vec_to_uv(param, param_1, param_2, param_3, param_4, param_5);
    float3 pos = param_5;
    float2 centered_uv = _3412;
    ...
    if (geom_force_correct_tangent_matrix)
    {
        ...
        {
            _3424 = float3(pos.x, 0.0f, pos.z);
        }
        float3 normal_base = _3424;
...
float2 HRG_GetGeomCurvedCoord(float2 in_coord, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, float in_geom_tilt_angle_x, float in_geom_tilt_angle_y, float in_screen_aspect, float pin_inner_edge, float2 in_source_size, float2 in_output_size, out float2x2 pixel_to_video_uv)
{
    ...
    if (in_geom_mode > 0.5f)
    {
        ...
        float2x2 param_14;
        float2 _3592 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14);
        pixel_to_video_uv = param_14;
        float2 ctr_curved_coord = _3592 - 0.5f.xx;
        if ((in_geom_tilt_angle_x == 0.0f) && (in_geom_tilt_angle_y == 0.0f))
        {
            ...
            float2x2 param_23;
            float2 _3621 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_15, param_16, param_17, param_18, param_19, param_20, param_21, param_22, param_23);
            pixel_to_video_uv = param_23;
            float2 right_edge_curved_ctr_coord = _3621 - 0.5f.xx;
            ...
            float2x2 param_32;
            float2 _3643 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_24, param_25, param_26, param_27, param_28, param_29, param_30, param_31, param_32);
            pixel_to_video_uv = param_32;
            float2 bottom_edge_curved_ctr_coord = _3643 - 0.5f.xx;
            ctr_curved_coord.x = (ctr_curved_coord.x * 0.5f) / right_edge_curved_ctr_coord.x;
        ...
        if (pin_inner_edge == 1.0f)
        {
            if (in_geom_tilt_angle_y != 0.0f)
            {
                ...
                float2x2 param_41;
                float2 _3686 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_33, param_34, param_35, param_36, param_37, param_38, param_39, param_40, param_41);
                pixel_to_video_uv = param_41;
                float2 top_edge_curved_ctr_coord = _3686;
            ...
            if (in_geom_tilt_angle_x != 0.0f)
            {
                ...
                float2x2 param_50;
                float2 _3715 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_42, param_43, param_44, param_45, param_46, param_47, param_48, param_49, param_50);
                pixel_to_video_uv = param_50;
                float2 left_edge_curved_ctr_coord = _3715;
...
float2 HSM_GetCurvatureValues(float screen_aspect)
{
    ...
    {
        _3742 = float2((((2.0f * global_HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f, ((global_HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f);
    }
    else
    {
        _3742 = float2(((global_HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f, (((2.0f * global_HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f);
    }
    float2 curvature_values = _3742;
...
float2 HSM_GetCrtPiCurvedCoord(inout float2 in_coord, inout float2 in_curvature)
{
    ...
    {
        _4245 = in_coord.y * in_coord.y;
    }
    float rsq = (in_coord.x * in_coord.x) + _4245;
...
float2 HSM_Get2DCurvedCoord(float2 in_coord, float2 curvature_values)
{
    ...
    float2 _4276 = HSM_GetCrtPiCurvedCoord(param, param_1);
    ctr_curved_coord = _4276 - 0.5f.xx;
    float2 param_2 = float2(1.0f, 0.5f);
    float2 param_3 = curvature_values;
    float2 _4283 = HSM_GetCrtPiCurvedCoord(param_2, param_3);
    float2 right_edge_curved_ctr_coord = _4283 - 0.5f.xx;
    ctr_curved_coord.x = (ctr_curved_coord.x * 0.5f) / right_edge_curved_ctr_coord.x;
    float2 param_4 = float2(0.5f, 1.0f);
    float2 param_5 = curvature_values;
    float2 _4297 = HSM_GetCrtPiCurvedCoord(param_4, param_5);
    float2 bottom_edge_curved_ctr_coord = _4297 - 0.5f.xx;
    ctr_curved_coord.y = (ctr_curved_coord.y * 0.5f) / bottom_edge_curved_ctr_coord.y;
    return ctr_curved_coord + 0.5f.xx;
}
...
float2 HSM_GetCurvedCoord(float2 in_coord, float curvature_multiplier, float screen_aspect)
{
    ...
    if (HSM_CURVATURE_MODE > CURVATURE_MODE_2D_CYLINDER)
    {
        ...
        else
        {
            ...
            float2x2 param_16;
            float2 _4405 = HRG_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14, param_15, param_16);
...
float2 HSM_GetTubeCurvedCoord(float2 screen_coord, float curvature_scale, float2 screen_scale, float2 tube_scale, float screen_aspect, float apply_black_edge_offset)
{
    ...
    if (global_HSM_BZL_USE_INDEPENDENT_CURVATURE == 1.0f)
    {
        ...
        if (screen_aspect < 1.0f)
        {
            _4474 = float2((((2.0f * global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f, ((global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f);
        }
        else
        {
            _4474 = float2(((global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f, (((2.0f * global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f);
        }
        float2 curvature_values = _4474;
...
float HSM_GetBezelCoords(float2 tube_diffuse_coord, float2 tube_diffuse_scale, float2 tube_scale, float screen_aspect, bool curve_coords_on, out float2 bezel_outside_scale, inout float2 bezel_outside_coord, inout float2 bezel_outside_curved_coord, inout float2 frame_outside_curved_coord)
{
    ...
    if (curve_coords_on)
    {
        bool _5101 = HSM_CURVATURE_MODE > 0.0f;
        bool _5109;
        if (_5101)
        {
            _5109 = (HSM_CURVATURE_3D_TILT_ANGLE_X != 0.0f) || (HSM_CURVATURE_3D_TILT_ANGLE_Y != 0.0f);
        }
        else
        {
            _5109 = _5101;
...
{
    bool _1848 = abs(in_coord.x - 0.5f) > 0.500999987125396728515625f;
    bool _1857;
    if (!_1848)
    {
        _1857 = abs(in_coord.y - 0.5f) > 0.500999987125396728515625f;
    }
    else
    {
        _1857 = _1848;
...
float4 HSM_GetStoichaicBlurredSample(Texture2D<float4> in_sampler, SamplerState _in_sampler_sampler, float2 in_coord, float num_samples, float max_blur_size, float blur_ratio)
{
    ...
    {
        float param = radius;
        float _4584 = HSM_rand(param);
        radius = param;
        radius_vector.x = _4584;
        float param_1 = radius;
        float _4589 = HSM_rand(param_1);
        radius = param_1;
        radius_vector.y = _4589;
    ...
    if (num_samples < 2.5f)
    {
        ...
        float _4611 = HSM_rand(param_2);
        radius = param_2;
        radius_vector.x = _4611;
        float param_3 = radius;
        float _4616 = HSM_rand(param_3);
        radius = param_3;
        radius_vector.y = _4616;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 2.0f.xxxx);
        float param_4 = radius;
        float _4634 = HSM_rand(param_4);
        radius = param_4;
        radius_vector.x = _4634;
        float param_5 = radius;
        float _4639 = HSM_rand(param_5);
        radius = param_5;
        radius_vector.y = _4639;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 2.0f.xxxx);
    }
    if (num_samples > 2.5f)
    {
        ...
        float _4661 = HSM_rand(param_6);
        radius = param_6;
        radius_vector.x = _4661;
        float param_7 = radius;
        float _4666 = HSM_rand(param_7);
        radius = param_7;
        radius_vector.y = _4666;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_8 = radius;
        float _4684 = HSM_rand(param_8);
        radius = param_8;
        radius_vector.x = _4684;
        float param_9 = radius;
        float _4689 = HSM_rand(param_9);
        radius = param_9;
        radius_vector.y = _4689;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_10 = radius;
        float _4707 = HSM_rand(param_10);
        radius = param_10;
        radius_vector.x = _4707;
        float param_11 = radius;
        float _4712 = HSM_rand(param_11);
        radius = param_11;
        radius_vector.y = _4712;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_12 = radius;
        float _4730 = HSM_rand(param_12);
        radius = param_12;
        radius_vector.x = _4730;
        float param_13 = radius;
        float _4735 = HSM_rand(param_13);
        radius = param_13;
        radius_vector.y = _4735;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_14 = radius;
        float _4753 = HSM_rand(param_14);
        radius = param_14;
        radius_vector.x = _4753;
        float param_15 = radius;
        float _4758 = HSM_rand(param_15);
        radius = param_15;
        radius_vector.y = _4758;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_16 = radius;
        float _4776 = HSM_rand(param_16);
        radius = param_16;
        radius_vector.x = _4776;
        float param_17 = radius;
        float _4781 = HSM_rand(param_17);
        radius = param_17;
        radius_vector.y = _4781;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_18 = radius;
        float _4799 = HSM_rand(param_18);
        radius = param_18;
        radius_vector.x = _4799;
        float param_19 = radius;
        float _4804 = HSM_rand(param_19);
        radius = param_19;
        radius_vector.y = _4804;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_20 = radius;
        float _4822 = HSM_rand(param_20);
        radius = param_20;
        radius_vector.x = _4822;
        float param_21 = radius;
        float _4827 = HSM_rand(param_21);
        radius = param_21;
        radius_vector.y = _4827;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_22 = radius;
        float _4845 = HSM_rand(param_22);
        radius = param_22;
        radius_vector.x = _4845;
        float param_23 = radius;
        float _4850 = HSM_rand(param_23);
        radius = param_23;
        radius_vector.y = _4850;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_24 = radius;
        float _4868 = HSM_rand(param_24);
        radius = param_24;
        radius_vector.x = _4868;
        float param_25 = radius;
        float _4873 = HSM_rand(param_25);
        radius = param_25;
        radius_vector.y = _4873;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_26 = radius;
        float _4891 = HSM_rand(param_26);
        radius = param_26;
        radius_vector.x = _4891;
        float param_27 = radius;
        float _4896 = HSM_rand(param_27);
        radius = param_27;
        radius_vector.y = _4896;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_28 = radius;
        float _4914 = HSM_rand(param_28);
        radius = param_28;
        radius_vector.x = _4914;
        float param_29 = radius;
        float _4919 = HSM_rand(param_29);
        radius = param_29;
        radius_vector.y = _4919;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
    }
...
void frag_main()
{
    ...
    bool _5254 = HSM_AB_COMPARE_FREEZE_CRT_TUBE == 1.0f;
    bool _5262;
    if (_5254)
    {
        float2 param = vTexCoord;
        _5262 = HSM_GetIsInABCompareArea(param);
    }
    else
    {
        _5262 = _5254;
    ...
    float4 _5281 = HSM_UpdateGlobalScreenValuesFromCache(InfoCachePass, _InfoCachePass_sampler, param_3);
    ...
    float _5299 = HSM_GetBezelCoords(param_4, param_5, param_6, param_7, param_8, param_9, param_10, param_11, param_12);
    ...
    {
        _5501 = -HSM_REFLECT_CORNER_ROTATION_OFFSET_TOP;
    }
    else
    {
        _5501 = -HSM_REFLECT_CORNER_ROTATION_OFFSET_BOTTOM;
    }
    float corner_rotation_offset = _5501;
    ...
    float _5572 = HHLP_QuadraticBezier(param_39, param_40);
    float fade_out_to_corner = _5572;
    ...
    {
        _5664 = 1.0f - smoothstep(0.0f, corner_fade_width / 2.0f, distance_from_crease_shifted);
    }
    else
    {
        _5664 = 1.0f;
    }
    float top_bottom_vs_sides_mask = _5664;
    ...
    else
    {
        ...
        _5703 = 1.0f - HSM_GetCornerMask(param_46, param_47, param_48, param_49);
    }
    float frame_inner_edge_mask = _5703;
    ...
    float4 _5768 = crt_linear;
    float3 _5770 = _5768.xyz * (tube_edge_shadow_mult * (1.0f - outside_tube_mask_wider));
    crt_linear.x = _5770.x;
    crt_linear.y = _5770.y;
    crt_linear.z = _5770.z;
    ...
    float _5802 = HHLP_QuadraticBezier(param_58, param_59);
    float edge_top_center_highlight_mask = (hmbz_bezel_highlight_top * top_mask) * _5802;
    ...
    float _5815 = HHLP_QuadraticBezier(param_60, param_61);
    float edge_bottom_center_highlight_mask = (hmbz_bezel_highlight_bottom * bottom_mask) * _5815;
    ...
    float _5828 = HHLP_QuadraticBezier(param_62, param_63);
    float edge_sides_highlight_mask = (hmbz_bezel_highlight_sides * sides_mask) * _5828;
    if (HSM_GLASS_BORDER_ON == 1.0f)
    {
        ...
        float _5845 = HHLP_QuadraticBezier(param_64, param_65);
        edge_top_center_highlight_mask = (0.60000002384185791015625f * top_mask) * _5845;
        ...
        float _5855 = HHLP_QuadraticBezier(param_66, param_67);
        edge_bottom_center_highlight_mask = bottom_mask * _5855;
        ...
        float _5866 = HHLP_QuadraticBezier(param_68, param_69);
        edge_sides_highlight_mask = (0.699999988079071044921875f * sides_mask) * _5866;
    }
    ...
    if (HSM_REFLECT_GLOBAL_AMOUNT > 0.0f)
    {
        ...
        float _5919 = HHLP_QuadraticBezier(param_70, param_71);
        float radial_fade_sides = 1.0f - _5919;
        ...
        float _5932 = HHLP_QuadraticBezier(param_72, param_73);
        float radial_fade_top_bottom = 1.0f - _5932;
        ...
        float _5953 = HHLP_QuadraticBezier(param_74, param_75);
        float radial_inner_fade_mask = clamp(0.300000011920928955078125f + (0.699999988079071044921875f * _5953), 0.0f, 1.0f);
        ...
        float _6036 = HHLP_QuadraticBezier(param_82, param_83);
        float stoichaic_blur_max = _6036 * 3.0f;
        ...
        if (HSM_GLASS_BORDER_ON == 1.0f)
        {
            ...
            float _6153 = HHLP_QuadraticBezier(param_99, param_100);
            float vignette_shadow_mask = 0.75f * _6153;
            float param_101 = smoothstep(outside_ratio_warped, 0.0f, 0.100000001490116119384765625f);
            float2 param_102 = float2(1.0f, 0.5f);
            float _6161 = HHLP_QuadraticBezier(param_101, param_102);
            vignette_shadow_mask += ((0.5f * HSM_REFLECT_VIGNETTE_AMOUNT) * _6161);
        ...
        else
        {
            ...
            float _6186 = HHLP_EasePowerIn(param_103, param_104);
            reflection_color += ((edge_reflection_color + (corner_reflection_color * _6186)) * edge_mask);
        }
        ...
        float4 _6266 = reflection_color;
        float3 _6268 = _6266.xyz * (((1.0f - HSM_REFLECT_FRAME_INNER_EDGE_AMOUNT) * (1.0f - frame_inner_edge_mask)) + HSM_REFLECT_FRAME_INNER_EDGE_AMOUNT);
        reflection_color.x = _6268.x;
        reflection_color.y = _6268.y;
        reflection_color.z = _6268.z;
    }
    float4 _6276 = crt_linear;
    float3 _6278 = _6276.xyz * TUBE_MASK;
    crt_linear.x = _6278.x;
    crt_linear.y = _6278.y;
    crt_linear.z = _6278.z;
    crt_linear = clamp(crt_linear, 0.0f.xxxx, 1.0f.xxxx);
    FragColor = crt_linear;
    float4 _6292 = FragColor;
    float3 _6294 = _6292.xyz + reflection_color.xyz;
    FragColor.x = _6294.x;
    FragColor.y = _6294.y;
    FragColor.z = _6294.z;
