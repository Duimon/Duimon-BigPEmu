...
vec4 HSM_GetBlackOnlyCropInPixels(sampler2D in_sampler_2D, vec2 sample_start_pixel_coord, vec2 window_size, float num_samples, vec4 max_crop)
{
    ...
    for (int i = 0; float(i) < crop_left_px; i++)
    {
        ...
        vec4 _3954 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler_2D, param, param_1, param_2);
        tex_sample = _3954;
    ...
    for (int i_1 = 0; float(i_1) < crop_top_px; i_1++)
    {
        ...
        vec4 _4001 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler_2D, param_3, param_4, param_5);
        tex_sample = _4001;
        brightness = (tex_sample.x + tex_sample.y) + tex_sample.z;
        if (brightness > HSM_CROP_BLACK_THRESHOLD)
        {
    ...
    for (int i_2 = 0; float(i_2) < crop_right_px; i_2++)
    {
        ...
        vec4 _4049 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler_2D, param_6, param_7, param_8);
        tex_sample = _4049;
        brightness = (tex_sample.x + tex_sample.y) + tex_sample.z;
        if (brightness > HSM_CROP_BLACK_THRESHOLD)
        {
    ...
    for (int i_3 = 0; float(i_3) < crop_bottom_px; i_3++)
    {
        ...
        vec4 _4097 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler_2D, param_9, param_10, param_11);
        tex_sample = _4097;
        brightness = (tex_sample.x + tex_sample.y) + tex_sample.z;
        if (brightness > 0.0)
        {
...
vec2 HSM_GetResMult(float use_vert_scanlines)
{
    ...
    bool _3511 = (use_vert_scanlines == 1.0) && (HSM_ROTATE_CORE_IMAGE == 0.0);
    bool _3523;
    if (!_3511)
    {
        bool _3516 = use_vert_scanlines == 0.0;
        bool _3522;
        if (_3516)
        {
            _3522 = abs(HSM_ROTATE_CORE_IMAGE) == 1.0;
        }
        else
        {
            _3522 = _3516;
    ...
    else
    {
        ...
        if (global.HSM_INT_SCALE_MODE == 0.0)
        {
            if (HSM_USE_PHYSICAL_SIZE_FOR_NON_INTEGER == 1.0)
            {
                ...
                {
                    _3183 = 1.0 / HSM_PHYSICAL_MONITOR_ASPECT_RATIO;
                }
                else
                {
        ...
        if (global.HSM_INT_SCALE_MODE > 0.5)
        {
            ...
            bool _3392 = HSM_GetUseVerticalScanlines(param) == 1.0;
            bool _3399;
            if (!_3392)
            {
                _3399 = global.HSM_INT_SCALE_MODE == 2.0;
            }
            else
            {
                _3399 = _3392;
...
vec2 HSM_GetScreenPositionOffset(inout vec2 placement_image_pos, vec2 screen_scale, float screen_index)
{
    ...
    if (screen_index == 1.0)
    {
        _2623 = vec2(HSM_SCREEN_POSITION_X / output_aspect, HSM_SCREEN_POSITION_Y) + (placement_image_pos - vec2(0.5));
    }
    else
    {
        _2623 = vec2(HSM_2ND_SCREEN_POS_X / output_aspect, HSM_2ND_SCREEN_POS_Y);
    }
    vec2 pos_offset = _2623;
...
vec4 HSM_GetColorForScreenInfoCache(vec2 viewport_coord, sampler2D feedback_pass, sampler2D original_pass, sampler2D screen_placement_image)
{
    ...
    vec2 _5558 = HSM_GetCroppedRotatedSizeWithResMult(param_7, param_8);
    vec2 cropped_rotated_size_with_res_mult = _5558;
    float param_9 = 60.0;
    ...
    {
        _5583 = (0.60000002384185791015625 * HSM_SINDEN_BORDER_THICKNESS) * 60.0;
    }
    else
    {
        _5583 = 0.0;
    ...
    vec2 _5617 = HSM_GetTubeScale(param_14, param_15, param_16);
    vec2 tube_diffuse_scale = _5617;
    ...
    if (global.HSM_TUBE_DIFFUSE_FORCE_ASPECT > 0.0)
    {
        ...
        if (global.HSM_TUBE_DIFFUSE_FORCE_ASPECT == 2.0)
        {
            ...
            {
                _5644 = global.HSM_TUBE_EXPLICIT_ASPECT;
            }
            else
            {
                _5644 = 1.0 / global.HSM_TUBE_EXPLICIT_ASPECT;
            }
    ...
    vec2 _5683 = HSM_GetTubeScale(param_17, param_18, param_19);
    vec2 tube_scale = _5683;
    vec2 param_20 = screen_pos_and_height.xy;
    vec2 param_21 = screen_scale;
    float param_22 = 1.0;
    vec2 _5691 = HSM_GetScreenPositionOffset(param_20, param_21, param_22);
    vec2 pos_offset = _5691;
    float param_23 = 1.0;
    ...
    vec2 _5734 = HSM_GetCroppedRotatedSizeWithResMult(param_30, param_31);
    vec2 cropped_size_with_res_mult_2nd_screen = _5734;
    ...
    vec2 _5753 = HSM_GetTubeScale(param_32, param_33, param_34);
    vec2 tube_diffuse_scale_2nd_screen = _5753;
    ...
    vec2 _5772 = HSM_GetTubeScale(param_35, param_36, param_37);
    vec2 tube_scale_2nd_screen = _5772;
    ...
    float param_40 = 2.0;
    vec2 _5786 = HSM_GetScreenPositionOffset(param_38, param_39, param_40);
    vec2 pos_offset_2nd_screen = _5786;
    ...
    if (HSM_IsCoordIn2DRange(param_56, param_57) == 1)
    {
        ...
        vec2 _5867 = HSM_GetRotatedDerezedSize();
        out_color.z = _5867.x;
        out_color.w = _5867.y;
    }
    float param_58 = 1.0;
    ...
    if (HSM_IsCoordIn2DRange(param_80, param_81) == 1)
    {
        ...
        vec2 _5987 = HSM_GetRotatedCoreOriginalSize();
        out_color.z = _5987.x;
        out_color.w = _5987.y;
    }
    float param_82 = 3.0;
    ...
    {
        vec2 _6005 = vec2(HSM_VIEWPORT_ZOOM, HSM_VIEWPORT_ZOOM);
        out_color.x = _6005.x;
        out_color.y = _6005.y;
        vec2 _6012 = vec2(HSM_VIEWPORT_POSITION_X, HSM_VIEWPORT_POSITION_Y);
        out_color.z = _6012.x;
        out_color.w = _6012.y;
    }
    float param_86 = 4.0;
    ...
    if (!_6194)
    {
        _6204 = abs(parameter_sum - parameter_sum_feedback) > 1.0000000116860974230803549289703e-07;
    }
    else
    {
        _6204 = _6194;
    ...
    {
        _6205 = false || (HSM_CACHE_GRAPHICS_ON < 0.5);
    }
...
void main()
{
    ...
    vec4 _6241 = HSM_GetColorForScreenInfoCache(param, InfoCachePassFeedback, Source, ScreenPlacementImage);
    FragColor = _6241;
}
