...
{
    bool _1614 = abs(in_coord.x - 0.5f) > 0.500999987125396728515625f;
    bool _1624;
    if (!_1614)
    {
        _1624 = abs(in_coord.y - 0.5f) > 0.500999987125396728515625f;
    }
    else
    {
        _1624 = _1614;
...
float4 HSM_GetCroppedTexSample(Texture2D<float4> in_sampler, SamplerState _in_sampler_sampler, float2 in_screen_coord)
{
    ...
    float4 _4550 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler, _in_sampler_sampler, param, param_1, param_2);
    return _4550;
}
...
float3 maxscale(float2 sinangle, float2 cosangle, float in_radius, float in_distance, float in_aspect)
{
    ...
    float2 _3907 = fwtrans(param_5, param_6, param_7, param_8, param_9);
    ...
    float2 _3924 = fwtrans(param_10, param_11, param_12, param_13, param_14);
    float2 lo = float2(_3907.x, _3924.y) / aspect_vec2;
    ...
    float2 _3944 = fwtrans(param_15, param_16, param_17, param_18, param_19);
    ...
    float2 _3960 = fwtrans(param_20, param_21, param_22, param_23, param_24);
    float2 hi = float2(_3944.x, _3960.y) / aspect_vec2;
...
float2 HSM_GetGeomCurvedCoord(float2 in_coord, float tilt_x, float tilt_y, float in_radius, inout float in_distance, float in_aspect)
{
    ...
    float2 _4057 = transform(param_5, param_6, param_7, param_8, param_9, param_10, param_11);
    float2 curved_coord = _4057;
    return curved_coord;
}
...
float2 HSM_GetGeomCurvedCoordRetainWidth(float2 in_coord, float tilt_x, float tilt_y, float in_radius, float in_distance, float in_aspect)
{
    ...
    float2 _4074 = HSM_GetGeomCurvedCoord(param, param_1, param_2, param_3, param_4, param_5);
    float2 ctr_curved_coord = _4074 - 0.5f.xx;
    ...
    float2 _4089 = HSM_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11);
    float2 right_edge_curved_ctr_coord = _4089 - 0.5f.xx;
...
float3 hrg_get_ideal_global_eye_pos_for_points(inout float3 eye_pos, float2 output_aspect, float3 global_coords[9], int num_points, float in_geom_radius, float in_geom_view_dist)
{
    ...
    for (int iter = 0; iter < max_centering_iters; iter++)
    {
        ...
        float3 _2636 = eye_pos;
        float2 _2638 = _2636.xy + center_offset;
        eye_pos.x = _2638.x;
        eye_pos.y = _2638.y;
...
float3 hrg_get_ideal_global_eye_pos(float3x3 local_to_global, float2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist)
{
    ...
    if (in_geom_mode < 2.5f)
    {
        _2797 = float3(0.0f, 0.0f, eye_z_spherical);
    }
    else
    {
        _2797 = float3(0.0f, 0.0f, max(in_geom_view_dist, eye_z_spherical));
    }
    float3 eye_pos = _2797;
    int num_points = 9;
    ...
    else
    {
        ...
        float3 _2956 = hrg_get_ideal_global_eye_pos_for_points(param_36, param_37, param_38, param_39, param_40, param_41);
        _2939 = _2956;
    }
    return _2939;
}
...
float2 hrg_get_curved_video_uv_coords_and_tangent_matrix(float2 flat_video_uv, float3 eye_pos_local, float2 output_pixel_size, float2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, float3x3 global_to_local, out float2x2 pixel_to_tangent_video_uv)
{
    ...
    float3 param_5;
    float2 _3181 = hrg_view_vec_to_uv(param, param_1, param_2, param_3, param_4, param_5);
    float3 pos = param_5;
    float2 centered_uv = _3181;
    ...
    if (geom_force_correct_tangent_matrix)
    {
        ...
        {
            _3194 = float3(pos.x, 0.0f, pos.z);
        }
        float3 normal_base = _3194;
...
float2 HRG_GetGeomCurvedCoord(float2 in_coord, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, float in_geom_tilt_angle_x, float in_geom_tilt_angle_y, float in_screen_aspect, float pin_inner_edge, float2 in_source_size, float2 in_output_size, out float2x2 pixel_to_video_uv)
{
    ...
    if (in_geom_mode > 0.5f)
    {
        ...
        float2x2 param_14;
        float2 _3362 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14);
        pixel_to_video_uv = param_14;
        float2 ctr_curved_coord = _3362 - 0.5f.xx;
        if ((in_geom_tilt_angle_x == 0.0f) && (in_geom_tilt_angle_y == 0.0f))
        {
            ...
            float2x2 param_23;
            float2 _3391 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_15, param_16, param_17, param_18, param_19, param_20, param_21, param_22, param_23);
            pixel_to_video_uv = param_23;
            float2 right_edge_curved_ctr_coord = _3391 - 0.5f.xx;
            ...
            float2x2 param_32;
            float2 _3413 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_24, param_25, param_26, param_27, param_28, param_29, param_30, param_31, param_32);
            pixel_to_video_uv = param_32;
            float2 bottom_edge_curved_ctr_coord = _3413 - 0.5f.xx;
            ctr_curved_coord.x = (ctr_curved_coord.x * 0.5f) / right_edge_curved_ctr_coord.x;
        ...
        if (pin_inner_edge == 1.0f)
        {
            if (in_geom_tilt_angle_y != 0.0f)
            {
                ...
                float2x2 param_41;
                float2 _3456 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_33, param_34, param_35, param_36, param_37, param_38, param_39, param_40, param_41);
                pixel_to_video_uv = param_41;
                float2 top_edge_curved_ctr_coord = _3456;
            ...
            if (in_geom_tilt_angle_x != 0.0f)
            {
                ...
                float2x2 param_50;
                float2 _3485 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_42, param_43, param_44, param_45, param_46, param_47, param_48, param_49, param_50);
                pixel_to_video_uv = param_50;
                float2 left_edge_curved_ctr_coord = _3485;
...
float2 HSM_GetCurvatureValues(float screen_aspect)
{
    ...
    {
        _3618 = float2((((2.0f * global_HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f, ((global_HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f);
    }
    else
    {
        _3618 = float2(((global_HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f, (((2.0f * global_HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f);
    }
    float2 curvature_values = _3618;
...
float2 HSM_GetCrtPiCurvedCoord(inout float2 in_coord, inout float2 in_curvature)
{
    ...
    float rsq = (in_coord.x * in_coord.x) + _4122;
...
float2 HSM_Get2DCurvedCoord(float2 in_coord, float2 curvature_values)
{
    ...
    float2 _4153 = HSM_GetCrtPiCurvedCoord(param, param_1);
    ctr_curved_coord = _4153 - 0.5f.xx;
    float2 param_2 = float2(1.0f, 0.5f);
    float2 param_3 = curvature_values;
    float2 _4160 = HSM_GetCrtPiCurvedCoord(param_2, param_3);
    float2 right_edge_curved_ctr_coord = _4160 - 0.5f.xx;
    ctr_curved_coord.x = (ctr_curved_coord.x * 0.5f) / right_edge_curved_ctr_coord.x;
    float2 param_4 = float2(0.5f, 1.0f);
    float2 param_5 = curvature_values;
    float2 _4174 = HSM_GetCrtPiCurvedCoord(param_4, param_5);
    float2 bottom_edge_curved_ctr_coord = _4174 - 0.5f.xx;
    ctr_curved_coord.y = (ctr_curved_coord.y * 0.5f) / bottom_edge_curved_ctr_coord.y;
    return ctr_curved_coord + 0.5f.xx;
}
...
float2 HSM_GetCurvedCoord(float2 in_coord, float curvature_multiplier, float screen_aspect)
{
    ...
    if (HSM_CURVATURE_MODE > CURVATURE_MODE_2D_CYLINDER)
    {
        ...
        else
        {
            ...
            float2x2 param_16;
            float2 _4281 = HRG_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14, param_15, param_16);
...
{
    float2 ctr_coord = in_coord - 0.5f.xx;
    bool _4343 = abs(ctr_coord.x) > 0.5f;
    bool _4351;
    if (!_4343)
    {
        _4351 = abs(ctr_coord.y) > 0.5f;
    }
    else
    {
        _4351 = _4343;
...
float2 HSM_GetCrtShaderFinalCoordExtraVariables(float2 screen_coord, float2 screen_scale, float raster_bloom_avg_lum, inout float2 screen_curved_coord, inout float2 screen_curved_coord_with_overscan, inout float2 screen_coord_with_overscan, inout float2 screen_scale_with_overscan)
{
    ...
    float2 _4863 = HSM_GetMirrorWrappedCoord(param_1);
    float2 screen_curved_coord_with_overscan_and_mirror = _4863;
...
float sw0(float x, float color, float scanline)
{
    ...
    {
        _5104 = lerp(ex * ex, (ex * ex) * ex, 0.4000000059604644775390625f);
    }
    ex = _5104;
...
bool HSM_GetUseFakeScanlines()
{
    ...
    bool _4568 = (global_HSM_FAKE_SCANLINE_OPACITY / 100.0f) > 0.001000000047497451305389404296875f;
    bool _4590;
    if (_4568)
    {
        bool _4574 = global_HSM_FAKE_SCANLINE_MODE == 1.0f;
        bool _4589;
        if (!_4574)
        {
            bool _4580 = global_HSM_FAKE_SCANLINE_MODE == 2.0f;
            bool _4588;
            if (_4580)
            {
                _4588 = scane_axis_core_res > global_HSM_INTERLACE_TRIGGER_RES;
            }
            else
            {
                _4588 = _4580;
...
float2 HSM_GetTubeCurvedCoord(float2 screen_coord, float curvature_scale, float2 screen_scale, float2 tube_scale, float screen_aspect, float apply_black_edge_offset)
{
    ...
    if (global_HSM_BZL_USE_INDEPENDENT_CURVATURE == 1.0f)
    {
        ...
        if (screen_aspect < 1.0f)
        {
            _4476 = float2((((2.0f * global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f, ((global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f);
        }
        else
        {
            _4476 = float2(((global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f, (((2.0f * global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f);
        }
        float2 curvature_values = _4476;
...
float4 HSM_ApplyScanlineMask(float4 in_color, float2 screen_scale, inout float2 in_coord, float2 in_screen_curved_coord, float2 in_tube_curved_coord, float in_scanline_opacity)
{
    ...
    float scan_axis_screen_scale_res = _4638;
    float _4650;
    if (USE_VERTICAL_SCANLINES > 0.5f)
    {
    ...
    float cropped_rotated_scan_res = _4650;
    float _4664;
    if (global_HSM_FAKE_SCANLINE_RES_MODE > 0.5f)
    {
        _4664 = global_HSM_FAKE_SCANLINE_RES;
    }
    else
    {
        _4664 = cropped_rotated_scan_res;
    }
    float simulated_scanline_res = _4664;
    ...
    float param_1 = 2.0f;
    float _4710 = HHLP_EasePowerOut(param, param_1);
    float color_brightness_modulation = _4710;
    ...
    float param_3 = 2.0f;
    float _4737 = HHLP_EasePowerOut(param_2, param_3);
...
void frag_main()
{
    ...
    {
        _701 = global_HSM_SNAP_TO_CLOSEST_INT_SCALE_TOLERANCE / 100.0f;
    }
    else
    {
        _701 = 0.0f;
    ...
    float4 _5229 = HSM_UpdateGlobalScreenValuesFromCache(InfoCachePass, _InfoCachePass_sampler, param_1);
    ...
    float _5246 = HSM_GetSimpleBezelCoords(param_2, param_3, param_4, param_5, param_6, param_7);
    ...
    float2 param_9 = 0.25f.xx;
    float gamma_in = 1.0f / HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_9).w;
    float2 param_10 = float2(0.75f, 0.25f);
    float intera = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_10).w;
    ...
    float2 param_12 = SCREEN_SCALE;
    float param_13 = raster_bloom_avg_lum;
    float2 param_14 = screen_curved_coord;
    float2 param_15 = screen_curved_coord_with_overscan;
    float2 param_16 = screen_coord_with_overscan;
    float2 param_17 = screen_scale_with_overscan;
    float2 _5325 = HSM_GetCrtShaderFinalCoordExtraVariables(param_11, param_12, param_13, param_14, param_15, param_16, param_17);
    ...
    float2 screen_curved_coord_with_overscan_and_mirror = _5325;
    ...
    if (smarte)
    {
        ...
        twr3 = 0.0f;
        float2 param_18 = pC4;
        c1 = HSM_GetCroppedTexSample(AvgLumPass, _AvgLumPass_sampler, param_18).xyz;
        float2 param_19 = pC4 + offy;
        c2 = HSM_GetCroppedTexSample(AvgLumPass, _AvgLumPass_sampler, param_19).xyz;
        c1 = max(c1 - global_sth.xxx, 0.0f.xxx);
        c2 = max(c2 - global_sth.xxx, 0.0f.xxx);
    }
    float2 param_20 = pC4 - off2;
    float3 l3 = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_20).xyz;
    float2 param_21 = pC4 - offx;
    float3 l2 = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_21).xyz;
    float2 param_22 = pC4;
    float3 l1 = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_22).xyz;
    float2 param_23 = pC4 + offx;
    float3 r1 = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_23).xyz;
    float2 param_24 = pC4 + off2;
    float3 r2 = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_24).xyz;
    float2 param_25 = (pC4 + offx) + off2;
    float3 r3 = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_25).xyz;
    ...
    {
        pC4 += offy;
        float2 param_26 = pC4 - off2;
        l3 = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_26).xyz;
        float2 param_27 = pC4 - offx;
        l2 = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_27).xyz;
        float2 param_28 = pC4;
        l1 = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_28).xyz;
        float2 param_29 = pC4 + offx;
        r1 = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_29).xyz;
        float2 param_30 = pC4 + off2;
        r2 = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_30).xyz;
        float2 param_31 = (pC4 + offx) + off2;
        r3 = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_31).xyz;
    ...
    if (!interb)
    {
        ...
        float param_32 = f;
        float wt1 = st(param_32);
        float param_33 = 1.0f - f;
        float wt2 = st(param_33);
        ...
        else
        {
            if (global_gsl == 1.0f)
            {
                ...
                float _6350 = sw1(param_40, param_41, param_42);
                wf1 = _6350;
                float param_43 = f2;
                float param_44 = creff2;
                float param_45 = shape2;
                float _6357 = sw1(param_43, param_44, param_45);
                wf2 = _6357;
            }
            else
            {
                float param_46 = f1;
                float param_47 = creff1;
                float param_48 = shape1;
                wf1 = sw2(param_46, param_47, param_48);
                float param_49 = f2;
                float param_50 = creff2;
                float param_51 = shape2;
        ...
        bool _6578 = global_HSM_INTERLACE_MODE < 0.0f;
        bool _6585;
        if (_6578)
        {
            _6585 = global_HSM_INTERLACE_TRIGGER_RES <= scan_res;
        }
        else
        {
            _6585 = _6578;
        ...
        {
            _6586 = global_no_scanlines;
        }
        float hsm_no_scanlines = _6586;
        float3 param_52 = color1;
        float3 param_53 = color2;
        color = (((gc(param_52) * w1) * cd1) + ((gc(param_53) * w2) * cd2)) / lerp(1.0f.xxx, w1 + w2, hsm_no_scanlines.xxx);
        if (abs(global_rolling_scan) > 0.004999999888241291046142578125f)
        {
            float param_54 = f;
            wt1 = st1(param_54);
            float param_55 = 1.0f - f;
            wt2 = st1(param_55);
            color00 = ((color1 * wt1) + (color2 * wt2)) / (wt1 + wt2).xxx;
            float3 param_56 = color00;
            color = gc(param_56) * lerp(w1 + w2, w3.xxx, max(wf1, wf2).xxx);
        }
        color = min(color, 1.0f.xxx);
    }
    if (interb)
    {
        float3 param_57 = color1;
        color = gc(param_57);
    }
    ...
    {
        float2 param_58 = TUBE_DIFFUSE_COORD;
        float param_59 = 1.0f;
        float2 param_60 = TUBE_DIFFUSE_SCALE;
        float2 param_61 = TUBE_SCALE;
        float param_62 = TUBE_DIFFUSE_ASPECT;
        float param_63 = 1.0f;
        float2 tube_curved_coord = HSM_GetTubeCurvedCoord(param_58, param_59, param_60, param_61, param_62, param_63);
        float4 param_64 = FragColor;
        float2 param_65 = screen_scale_with_overscan;
        float2 param_66 = screen_coord_with_overscan;
        float2 param_67 = screen_curved_coord_with_overscan;
        float2 param_68 = tube_curved_coord;
        float param_69 = global_HSM_FAKE_SCANLINE_OPACITY / 100.0f;
        float4 _6719 = HSM_ApplyScanlineMask(param_64, param_65, param_66, param_67, param_68, param_69);
        FragColor = _6719;
    }
}
