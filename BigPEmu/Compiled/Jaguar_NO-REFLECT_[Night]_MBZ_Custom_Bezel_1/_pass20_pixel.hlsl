...
Texture2D<float4> DerezedPass : register(t12);
SamplerState _DerezedPass_sampler : register(s12);
...
bool HSM_GetIsInABCompareArea(float2 viewport_coord)
{
    ...
    {
        _6500 = viewport_coord.y;
    }
    else
    {
        _6500 = viewport_coord.x;
    }
    float test_value = _6500;
    float _6513;
    if (mod(HSM_AB_COMPARE_AREA, 2.0f) == 1.0f)
    {
        _6513 = 1.0f - HSM_AB_COMPARE_SPLIT_POSITION;
    }
    else
    {
        _6513 = HSM_AB_COMPARE_SPLIT_POSITION;
    }
    float position = _6513;
    bool _6527 = (mod(HSM_AB_COMPARE_AREA, 2.0f) == 0.0f) && (test_value < position);
    bool _6538;
    if (!_6527)
    {
        _6538 = (mod(HSM_AB_COMPARE_AREA, 2.0f) == 1.0f) && (test_value > position);
    }
    else
    {
        _6538 = _6527;
...
{
    bool _3044 = abs(in_coord.x - 0.5f) > 0.500999987125396728515625f;
    bool _3053;
    if (!_3044)
    {
        _3053 = abs(in_coord.y - 0.5f) > 0.500999987125396728515625f;
    }
    else
    {
        _3053 = _3044;
...
float3 maxscale(float2 sinangle, float2 cosangle, float in_radius, float in_distance, float in_aspect)
{
    ...
    float2 _5502 = fwtrans(param_5, param_6, param_7, param_8, param_9);
    ...
    float2 _5519 = fwtrans(param_10, param_11, param_12, param_13, param_14);
    float2 lo = float2(_5502.x, _5519.y) / aspect_vec2;
    ...
    float2 _5539 = fwtrans(param_15, param_16, param_17, param_18, param_19);
    ...
    float2 _5555 = fwtrans(param_20, param_21, param_22, param_23, param_24);
    float2 hi = float2(_5539.x, _5555.y) / aspect_vec2;
...
float2 HSM_GetGeomCurvedCoord(float2 in_coord, float tilt_x, float tilt_y, float in_radius, inout float in_distance, float in_aspect)
{
    ...
    float2 _5652 = transform(param_5, param_6, param_7, param_8, param_9, param_10, param_11);
    float2 curved_coord = _5652;
    return curved_coord;
}
...
float2 HSM_GetGeomCurvedCoordRetainWidth(float2 in_coord, float tilt_x, float tilt_y, float in_radius, float in_distance, float in_aspect)
{
    ...
    float2 _5669 = HSM_GetGeomCurvedCoord(param, param_1, param_2, param_3, param_4, param_5);
    float2 ctr_curved_coord = _5669 - 0.5f.xx;
    ...
    float2 _5684 = HSM_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11);
    float2 right_edge_curved_ctr_coord = _5684 - 0.5f.xx;
...
float3 hrg_uv_to_xyz(float2 uv, float2 output_aspect, float in_geom_mode, float in_geom_radius)
{
    ...
    if (in_geom_mode < 1.5f)
    {
        ...
        _3883 = hrg_sphere_uv_to_xyz(param, param_1, param_2);
    }
    else
    {
        float3 _3896;
        if (in_geom_mode < 2.5f)
        {
            ...
            _3896 = hrg_sphere_alt_uv_to_xyz(param_3, param_4, param_5);
        }
        else
        {
            float2 param_6 = uv;
            ...
            _3896 = hrg_cylinder_uv_to_xyz(param_6, param_7, param_8);
        }
        _3883 = _3896;
    }
    return _3883;
...
float3 hrg_get_ideal_global_eye_pos_for_points(inout float3 eye_pos, float2 output_aspect, float3 global_coords[9], int num_points, float in_geom_radius, float in_geom_view_dist)
{
    ...
    for (int iter = 0; iter < max_centering_iters; iter++)
    {
        ...
        float3 _4062 = eye_pos;
        float2 _4064 = _4062.xy + center_offset;
        eye_pos.x = _4064.x;
        eye_pos.y = _4064.y;
        ...
        for (int i_3 = 0; i_3 < num_points; i_3++)
        {
            ...
            {
                _4128 = max(offset_z_max, offset_zzzz.x);
            }
            else
            {
                _4128 = offset_z_max;
            ...
            {
                _4141 = max(offset_z_max, offset_zzzz.y);
            }
            else
            {
                _4141 = offset_z_max;
            ...
            if (eyespace_xyz_flipy.x > 0.0f)
            {
                _4154 = max(offset_z_max, offset_zzzz.z);
            }
            else
            {
                _4154 = offset_z_max;
            ...
            if (eyespace_xyz_flipy.y > 0.0f)
            {
                _4167 = max(offset_z_max, offset_zzzz.w);
            }
            else
            {
                _4167 = offset_z_max;
...
float3 hrg_get_ideal_global_eye_pos(float3x3 local_to_global, float2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist)
{
    ...
    if (in_geom_mode < 2.5f)
    {
        _4223 = float3(0.0f, 0.0f, eye_z_spherical);
    }
    else
    {
        _4223 = float3(0.0f, 0.0f, max(in_geom_view_dist, eye_z_spherical));
    }
    float3 eye_pos = _4223;
    int num_points = 9;
    ...
    else
    {
        ...
        float3 _4382 = hrg_get_ideal_global_eye_pos_for_points(param_36, param_37, param_38, param_39, param_40, param_41);
        _4365 = _4382;
    }
    return _4365;
}
...
float2 hrg_intersect(float3 view_vec_local, float3 eye_pos_local, float in_geom_mode, float in_geom_radius)
{
    ...
    if (in_geom_mode < 2.5f)
    {
        ...
        float param_2 = in_geom_radius;
        _3822 = hrg_intersect_sphere(param, param_1, param_2);
    }
    else
    {
        ...
        float param_5 = in_geom_radius;
        _3822 = hrg_intersect_cylinder(param_3, param_4, param_5);
    }
    return _3822;
}
...
float2 hrg_xyz_to_uv(float3 intersection_pos_local, float2 output_aspect, float in_geom_mode, float in_geom_radius)
{
    ...
    if (in_geom_mode < 1.5f)
    {
        ...
        float2 param_1 = output_aspect;
        float param_2 = in_geom_radius;
        _3846 = hrg_sphere_xyz_to_uv(param, param_1, param_2);
    }
    else
    {
        float2 _3859;
        if (in_geom_mode < 2.5f)
        {
            ...
            float2 param_4 = output_aspect;
            float param_5 = in_geom_radius;
            _3859 = hrg_sphere_alt_xyz_to_uv(param_3, param_4, param_5);
        }
        else
        {
            ...
            float2 param_7 = output_aspect;
            float param_8 = in_geom_radius;
            _3859 = hrg_cylinder_xyz_to_uv(param_6, param_7, param_8);
        }
        _3846 = _3859;
    }
    return _3846;
...
float2 hrg_view_vec_to_uv(float3 view_vec_local, float3 eye_pos_local, float2 output_aspect, float in_geom_mode, float in_geom_radius, out float3 intersection_pos)
{
    ...
    if (intersect_dist_and_discriminant.y > 0.004999999888241291046142578125f)
    {
        ...
        float param_6 = in_geom_mode;
        float param_7 = in_geom_radius;
        _3940 = hrg_xyz_to_uv(param_4, param_5, param_6, param_7);
    }
    else
    {
        _3940 = 1.0f.xx;
...
float2 hrg_get_curved_video_uv_coords_and_tangent_matrix(float2 flat_video_uv, float3 eye_pos_local, float2 output_pixel_size, float2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, float3x3 global_to_local, out float2x2 pixel_to_tangent_video_uv)
{
    ...
    float3 param_5;
    float2 _4607 = hrg_view_vec_to_uv(param, param_1, param_2, param_3, param_4, param_5);
    float3 pos = param_5;
    float2 centered_uv = _4607;
    ...
    if (geom_force_correct_tangent_matrix)
    {
        ...
        {
            _4619 = float3(pos.x, 0.0f, pos.z);
        }
        float3 normal_base = _4619;
...
float2 HRG_GetGeomCurvedCoord(float2 in_coord, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, float in_geom_tilt_angle_x, float in_geom_tilt_angle_y, float in_screen_aspect, float pin_inner_edge, float2 in_source_size, float2 in_output_size, out float2x2 pixel_to_video_uv)
{
    ...
    if (in_geom_mode > 0.5f)
    {
        ...
        float2x2 param_14;
        float2 _4787 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14);
        pixel_to_video_uv = param_14;
        float2 ctr_curved_coord = _4787 - 0.5f.xx;
        if ((in_geom_tilt_angle_x == 0.0f) && (in_geom_tilt_angle_y == 0.0f))
        {
            ...
            float2x2 param_23;
            float2 _4816 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_15, param_16, param_17, param_18, param_19, param_20, param_21, param_22, param_23);
            pixel_to_video_uv = param_23;
            float2 right_edge_curved_ctr_coord = _4816 - 0.5f.xx;
            ...
            float2x2 param_32;
            float2 _4838 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_24, param_25, param_26, param_27, param_28, param_29, param_30, param_31, param_32);
            pixel_to_video_uv = param_32;
            float2 bottom_edge_curved_ctr_coord = _4838 - 0.5f.xx;
            ctr_curved_coord.x = (ctr_curved_coord.x * 0.5f) / right_edge_curved_ctr_coord.x;
        ...
        if (pin_inner_edge == 1.0f)
        {
            if (in_geom_tilt_angle_y != 0.0f)
            {
                ...
                float2x2 param_41;
                float2 _4881 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_33, param_34, param_35, param_36, param_37, param_38, param_39, param_40, param_41);
                pixel_to_video_uv = param_41;
                float2 top_edge_curved_ctr_coord = _4881;
            ...
            if (in_geom_tilt_angle_x != 0.0f)
            {
                ...
                float2x2 param_50;
                float2 _4910 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_42, param_43, param_44, param_45, param_46, param_47, param_48, param_49, param_50);
                pixel_to_video_uv = param_50;
                float2 left_edge_curved_ctr_coord = _4910;
...
float2 HSM_GetCurvatureValues(float screen_aspect)
{
    ...
    {
        _5213 = float2((((2.0f * global_HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f, ((global_HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f);
    }
    else
    {
        _5213 = float2(((global_HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f, (((2.0f * global_HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f);
    }
    float2 curvature_values = _5213;
...
float2 HSM_GetCrtPiCurvedCoord(inout float2 in_coord, inout float2 in_curvature)
{
    ...
    {
        _5717 = in_coord.y * in_coord.y;
    }
    float rsq = (in_coord.x * in_coord.x) + _5717;
...
float2 HSM_Get2DCurvedCoord(float2 in_coord, float2 curvature_values)
{
    ...
    float2 _5748 = HSM_GetCrtPiCurvedCoord(param, param_1);
    ctr_curved_coord = _5748 - 0.5f.xx;
    float2 param_2 = float2(1.0f, 0.5f);
    float2 param_3 = curvature_values;
    float2 _5755 = HSM_GetCrtPiCurvedCoord(param_2, param_3);
    float2 right_edge_curved_ctr_coord = _5755 - 0.5f.xx;
    ctr_curved_coord.x = (ctr_curved_coord.x * 0.5f) / right_edge_curved_ctr_coord.x;
    float2 param_4 = float2(0.5f, 1.0f);
    float2 param_5 = curvature_values;
    float2 _5769 = HSM_GetCrtPiCurvedCoord(param_4, param_5);
    float2 bottom_edge_curved_ctr_coord = _5769 - 0.5f.xx;
    ctr_curved_coord.y = (ctr_curved_coord.y * 0.5f) / bottom_edge_curved_ctr_coord.y;
    return ctr_curved_coord + 0.5f.xx;
}
...
float2 HSM_GetCurvedCoord(float2 in_coord, float curvature_multiplier, float screen_aspect)
{
    ...
    if (HSM_CURVATURE_MODE > CURVATURE_MODE_2D_CYLINDER)
    {
        ...
        else
        {
            ...
            float2x2 param_16;
            float2 _5876 = HRG_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14, param_15, param_16);
...
float2 HSM_GetTubeCurvedCoord(float2 screen_coord, float curvature_scale, float2 screen_scale, float2 tube_scale, float screen_aspect, float apply_black_edge_offset)
{
    ...
    if (global_HSM_BZL_USE_INDEPENDENT_CURVATURE == 1.0f)
    {
        ...
        if (screen_aspect < 1.0f)
        {
            _6033 = float2((((2.0f * global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f, ((global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f);
        }
        else
        {
            _6033 = float2(((global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f, (((2.0f * global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f);
        }
        float2 curvature_values = _6033;
...
float HSM_GetBezelCoords(float2 tube_diffuse_coord, float2 tube_diffuse_scale, float2 tube_scale, float screen_aspect, bool curve_coords_on, out float2 bezel_outside_scale, inout float2 bezel_outside_coord, inout float2 bezel_outside_curved_coord, inout float2 frame_outside_curved_coord)
{
    ...
    if (curve_coords_on)
    {
        bool _7292 = HSM_CURVATURE_MODE > 0.0f;
        bool _7300;
        if (_7292)
        {
            _7300 = (HSM_CURVATURE_3D_TILT_ANGLE_X != 0.0f) || (HSM_CURVATURE_3D_TILT_ANGLE_Y != 0.0f);
        }
        else
        {
            _7300 = _7292;
...
float3 HSM_RGBtoHSV(float3 c)
{
    ...
    if (c.y < c.z)
    {
        _2664 = float4(c.zy, K.wz);
    }
    else
    {
        _2664 = float4(c.yz, K.xy);
    }
    float4 p = _2664;
    float4 _2694;
    if (c.x < p.x)
    {
        _2694 = float4(p.xyw, c.x);
    }
    else
    {
        _2694 = float4(c.x, p.yzx);
    }
    float4 q = _2694;
...
float4 HSM_ApplyMonochrome(float4 in_color)
{
    ...
    float4 _6838 = out_color;
    float3 _6844 = pow(_6838.xyz, global_HSM_MONOCHROME_GAMMA.xxx);
    out_color.x = _6844.x;
    out_color.y = _6844.y;
    out_color.z = _6844.z;
    ...
    bool _6884 = (global_HSM_MONOCHROME_HUE_OFFSET / 360.0f) != 0.0f;
    bool _6893;
    if (!_6884)
    {
        _6893 = (global_HSM_MONOCHROME_SATURATION / 100.0f) != 0.0f;
    }
    else
    {
        _6893 = _6884;
    ...
    float3 _6923 = mcolor * pow(luma, 1.0f / global_HSM_MONOCHROME_GAMMA);
    out_color.x = _6923.x;
    out_color.y = _6923.y;
    out_color.z = _6923.z;
    return out_color;
...
float2 HSM_GetMirrorWrappedCoord(inout float2 in_coord)
{
    ...
    bool _5900 = abs(ctr_coord.x) > 0.5f;
    bool _5908;
    if (!_5900)
    {
        _5908 = abs(ctr_coord.y) > 0.5f;
    }
    else
    {
        _5908 = _5900;
...
{
    float2 param = in_coord;
    float2 _6579 = HSM_GetMirrorWrappedCoord(param);
    float2 vpos = _6579;
...
{
    bool _6628 = global_HSM_SCREEN_VIGNETTE_ON > 0.5f;
    bool _6634;
    if (_6628)
    {
        float param = HSM_SCREEN_VIGNETTE_DUALSCREEN_VIS_MODE;
        _6634 = HSM_GetUseOnCurrentScreenIndex(param);
    }
    else
    {
        _6634 = _6628;
...
float4 HSM_GetCroppedTexSample(Texture2D<float4> in_sampler, SamplerState _in_sampler_sampler, float2 in_screen_coord)
{
    ...
    float4 _6573 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler, _in_sampler_sampler, param, param_1, param_2);
    return _6573;
}
...
float2 GetSimpleImageScaledCoord(inout float2 in_viewport_coord, float2 in_viewport_unscaled_coord, float2 in_tube_coord, float2 in_tube_scale, Texture2D<float4> in_sampler, SamplerState _in_sampler_sampler, float2 in_pos, float in_inherit_pos, float2 in_scale, float in_scale_inherit_mode, float in_keep_aspect, float in_mirror_horz, float in_rotate)
{
    ...
    uint _5139_dummy_parameter;
    float2 tex_size = float2(int2(spvTextureSize(in_sampler, uint(0), _5139_dummy_parameter)));
    float _5144;
    if (in_rotate == 1.0f)
    {
        _5144 = tex_size.y / tex_size.x;
    }
    else
    {
        _5144 = tex_size.x / tex_size.y;
    }
    float tex_aspect = _5144;
    float _5161;
    if (in_keep_aspect == 1.0f)
    {
        _5161 = output_aspect / tex_aspect;
    }
    else
    {
        _5161 = 1.0f;
    ...
    if (in_rotate > 0.5f)
    {
        _5175 = global_FinalViewportSize.x / global_FinalViewportSize.y;
    }
    else
    {
        _5175 = 1.0f;
...
{
    uint _6544_dummy_parameter;
    float2 tex_size = float2(int2(spvTextureSize(in_sampler, uint(0), _6544_dummy_parameter)));
...
float4 HSM_GetNightLightingMultiplyColor(float2 in_coord, float hue, float saturation, float value, float contrast, float global_ambient_opacity, Texture2D<float4> NightLightingImage_1, SamplerState _NightLightingImage_1_sampler)
{
    ...
    float4 _6695 = lighting_image;
    float3 _6699 = lerp(1.0f.xxx, _6695.xyz, global_ambient_opacity.xxx);
    lighting_image.x = _6699.x;
    lighting_image.y = _6699.y;
    lighting_image.z = _6699.z;
...
bool HSM_Fill_Ambient_Images(float2 in_viewport_coord, float2 in_viewport_unscaled_coord, float2 in_tube_coord, float2 in_tube_scale, float in_swap_images, Texture2D<float4> in_ambient_sampler, SamplerState _in_ambient_sampler_sampler, Texture2D<float4> in_ambient2_sampler, SamplerState _in_ambient2_sampler_sampler, out float4 ambient_lighting_image, out float4 ambient2_lighting_image)
{
    ...
    if (HSM_AMBIENT1_OPACITY > 0.0f)
    {
        ...
        bool _6717 = HSM_AMBIENT1_SCALE_INHERIT_MODE == 1.0f;
        bool _6724;
        if (_6717)
        {
            _6724 = (HSM_AMBIENT1_SCALE * HSM_VIEWPORT_ZOOM) < 1.0f;
        }
        else
        {
            _6724 = _6717;
        ...
        float2 _6758 = GetSimpleImageScaledCoord(param, param_1, param_2, param_3, in_ambient_sampler, _in_ambient_sampler_sampler, param_4, param_5, param_6, param_7, param_8, param_9, param_10);
        float2 lighting_coord = _6758;
    ...
    if (HSM_AMBIENT2_OPACITY > 0.0f)
    {
        ...
        bool _6779 = HSM_AMBIENT2_SCALE_INHERIT_MODE == 1.0f;
        bool _6786;
        if (_6779)
        {
            _6786 = (HSM_AMBIENT2_SCALE * HSM_VIEWPORT_ZOOM) < 1.0f;
        }
        else
        {
            _6786 = _6779;
        ...
        float2 _6820 = GetSimpleImageScaledCoord(param_17, param_18, param_19, param_20, in_ambient2_sampler, _in_ambient2_sampler_sampler, param_21, param_22, param_23, param_24, param_25, param_26, param_27);
        float2 lighting2_coord = _6820;
...
{
    bool _7443 = HSM_TUBE_DIFFUSE_MODE == 1.0f;
    bool _7449;
    if (_7443)
    {
        float param = HSM_TUBE_DIFFUSE_IMAGE_DUALSCREEN_VIS_MODE;
        _7449 = HSM_GetUseOnCurrentScreenIndex(param);
    }
    else
    {
        _7449 = _7443;
...
{
    bool _7453 = HSM_TUBE_COLORED_GEL_IMAGE_ON > 0.5f;
    bool _7459;
    if (_7453)
    {
        float param = HSM_TUBE_COLORED_GEL_IMAGE_DUALSCREEN_VIS_MODE;
        _7459 = HSM_GetUseOnCurrentScreenIndex(param);
    }
    else
    {
        _7459 = _7453;
...
{
    bool _7433 = HSM_TUBE_STATIC_REFLECTION_IMAGE_ON > 0.5f;
    bool _7439;
    if (_7433)
    {
        float param = HSM_TUBE_STATIC_REFLECTION_IMAGE_DUALSCREEN_VIS_MODE;
        _7439 = HSM_GetUseOnCurrentScreenIndex(param);
    }
    else
    {
        _7439 = _7433;
...
float4 HSM_GetPreMultipliedColorLinear(float4 in_color, float matte_type, float encoded_gamma)
{
    ...
    {
        float4 _2870 = out_color;
        float _2873 = out_color.w;
        float3 _2879 = clamp(_2870.xyz - (1.0f - _2873).xxx, 0.0f.xxx, 1.0f.xxx);
        out_color.x = _2879.x;
        out_color.y = _2879.y;
        out_color.z = _2879.z;
    }
    ...
    {
        float _2897 = out_color.w;
        float4 _2898 = out_color;
        float3 _2900 = _2898.xyz * _2897;
        out_color.x = _2900.x;
        out_color.y = _2900.y;
        out_color.z = _2900.z;
    }
...
float4 HSM_BlendModeLayerMix(float4 color_under, inout float4 color_over, float blend_mode, float layer_opacity)
{
    ...
    {
        float _2959 = color_over.w;
        float4 _2960 = color_over;
        float3 _2962 = _2960.xyz * _2959;
        color_over.x = _2962.x;
        color_over.y = _2962.y;
        color_over.z = _2962.z;
    ...
    else
    {
        ...
        {
            float3 _2986 = color_under.xyz + color_over.xyz;
            blend_color.x = _2986.x;
            blend_color.y = _2986.y;
            blend_color.z = _2986.z;
        }
        if (blend_mode == BLEND_MODE_MULTIPLY)
        {
            float3 _3002 = color_under.xyz * color_over.xyz;
            blend_color.x = _3002.x;
            blend_color.y = _3002.y;
            blend_color.z = _3002.z;
        }
...
float4 HSM_GetStoichaicBlurredSample(Texture2D<float4> in_sampler, SamplerState _in_sampler_sampler, float2 in_coord, float num_samples, float max_blur_size, float blur_ratio)
{
    ...
    {
        float param = radius;
        float _6143 = HSM_rand(param);
        radius = param;
        radius_vector.x = _6143;
        float param_1 = radius;
        float _6148 = HSM_rand(param_1);
        radius = param_1;
        radius_vector.y = _6148;
    ...
    if (num_samples < 2.5f)
    {
        ...
        float _6170 = HSM_rand(param_2);
        radius = param_2;
        radius_vector.x = _6170;
        float param_3 = radius;
        float _6175 = HSM_rand(param_3);
        radius = param_3;
        radius_vector.y = _6175;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 2.0f.xxxx);
        float param_4 = radius;
        float _6193 = HSM_rand(param_4);
        radius = param_4;
        radius_vector.x = _6193;
        float param_5 = radius;
        float _6198 = HSM_rand(param_5);
        radius = param_5;
        radius_vector.y = _6198;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 2.0f.xxxx);
    }
    if (num_samples > 2.5f)
    {
        ...
        float _6220 = HSM_rand(param_6);
        radius = param_6;
        radius_vector.x = _6220;
        float param_7 = radius;
        float _6225 = HSM_rand(param_7);
        radius = param_7;
        radius_vector.y = _6225;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_8 = radius;
        float _6243 = HSM_rand(param_8);
        radius = param_8;
        radius_vector.x = _6243;
        float param_9 = radius;
        float _6248 = HSM_rand(param_9);
        radius = param_9;
        radius_vector.y = _6248;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_10 = radius;
        float _6266 = HSM_rand(param_10);
        radius = param_10;
        radius_vector.x = _6266;
        float param_11 = radius;
        float _6271 = HSM_rand(param_11);
        radius = param_11;
        radius_vector.y = _6271;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_12 = radius;
        float _6289 = HSM_rand(param_12);
        radius = param_12;
        radius_vector.x = _6289;
        float param_13 = radius;
        float _6294 = HSM_rand(param_13);
        radius = param_13;
        radius_vector.y = _6294;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_14 = radius;
        float _6312 = HSM_rand(param_14);
        radius = param_14;
        radius_vector.x = _6312;
        float param_15 = radius;
        float _6317 = HSM_rand(param_15);
        radius = param_15;
        radius_vector.y = _6317;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_16 = radius;
        float _6335 = HSM_rand(param_16);
        radius = param_16;
        radius_vector.x = _6335;
        float param_17 = radius;
        float _6340 = HSM_rand(param_17);
        radius = param_17;
        radius_vector.y = _6340;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_18 = radius;
        float _6358 = HSM_rand(param_18);
        radius = param_18;
        radius_vector.x = _6358;
        float param_19 = radius;
        float _6363 = HSM_rand(param_19);
        radius = param_19;
        radius_vector.y = _6363;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_20 = radius;
        float _6381 = HSM_rand(param_20);
        radius = param_20;
        radius_vector.x = _6381;
        float param_21 = radius;
        float _6386 = HSM_rand(param_21);
        radius = param_21;
        radius_vector.y = _6386;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_22 = radius;
        float _6404 = HSM_rand(param_22);
        radius = param_22;
        radius_vector.x = _6404;
        float param_23 = radius;
        float _6409 = HSM_rand(param_23);
        radius = param_23;
        radius_vector.y = _6409;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_24 = radius;
        float _6427 = HSM_rand(param_24);
        radius = param_24;
        radius_vector.x = _6427;
        float param_25 = radius;
        float _6432 = HSM_rand(param_25);
        radius = param_25;
        radius_vector.y = _6432;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_26 = radius;
        float _6450 = HSM_rand(param_26);
        radius = param_26;
        radius_vector.x = _6450;
        float param_27 = radius;
        float _6455 = HSM_rand(param_27);
        radius = param_27;
        radius_vector.y = _6455;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_28 = radius;
        float _6473 = HSM_rand(param_28);
        radius = param_28;
        radius_vector.x = _6473;
        float param_29 = radius;
        float _6478 = HSM_rand(param_29);
        radius = param_29;
        radius_vector.y = _6478;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
    }
...
float4 HSM_GetTubeHighlight(inout float2 tube_curved_coord, bool apply_to_mirror, Texture2D<float4> TubeStaticReflectionImage_1, SamplerState _TubeStaticReflectionImage_1_sampler)
{
    ...
    if (apply_to_mirror)
    {
        ...
        float2 _7473 = HSM_GetMirrorWrappedCoord(param_1);
        mirrored_tube_coord = _7473;
    }
    ...
    float4 _7574 = out_color;
    float3 _7576 = _7574.xyz * HSM_GLOBAL_GRAPHICS_BRIGHTNESS;
    out_color.x = _7576.x;
    out_color.y = _7576.y;
    out_color.z = _7576.z;
    return out_color;
...
{
    bool _7588 = global_HSM_MONOCHROME_MODE > 0.5f;
    bool _7594;
    if (_7588)
    {
        float param = HSM_MONOCHROME_DUALSCREEN_VIS_MODE;
        _7594 = HSM_GetUseOnCurrentScreenIndex(param);
    }
    else
    {
        _7594 = _7588;
    ...
    float2 _7610 = HSM_GetMirrorWrappedCoord(param_2);
    float2 mirrored_screen_coord = _7610;
    float4 out_color = 0.0f.xxxx;
    ...
    float2 _7648 = HSM_GetMirrorWrappedCoord(param_13);
    float2 mirrored_tube_coord = _7648;
    ...
    bool _7681 = HSM_AB_COMPARE_SHOW_MODE == 1.0f;
    bool _7687;
    if (_7681)
    {
        float2 param_15 = VIEWPORT_COORD;
        _7687 = HSM_GetIsInABCompareArea(param_15);
    }
    else
    {
        _7687 = _7681;
    ...
    if (_7687)
    {
        ...
        float2 _7701 = HSM_GetMirrorWrappedCoord(param_16);
        ab_screen_coord = _7701;
    ...
    float4 _7779 = crt_color;
    float3 _7781 = _7779.xyz * screen_to_tube_mask_invert;
    crt_color.x = _7781.x;
    crt_color.y = _7781.y;
    crt_color.z = _7781.z;
    ...
    bool _7804 = HSM_Fill_Ambient_Images(param_34, param_35, param_36, param_37, param_38, NightLightingImage_1, _NightLightingImage_1_sampler, NightLighting2Image_1, _NightLighting2Image_1_sampler, param_39, param_40);
    ambient_lighting_image = param_39;
    ambient2_lighting_image = param_40;
    float3 tube_shadow = 1.0f.xxx;
    bool _7809 = HSM_TUBE_SHADOW_IMAGE_ON > 0.5f;
    bool _7823;
    if (_7809)
    {
        bool _7812 = HSM_GetUseTubeDiffuseImage();
        bool _7817;
        if (!_7812)
        {
            _7817 = HSM_GetUseTubeColoredGelImage();
        }
        else
        {
            _7817 = _7812;
        ...
        if (!_7817)
        {
            _7822 = HSM_GetUseTubeStaticReflection();
        }
        else
        {
            _7822 = _7817;
    ...
    if (_7823)
    {
        ...
        float2 _7830 = HSM_GetMirrorWrappedCoord(param_41);
        shadow_coord = _7830;
    ...
    {
        bool _7923 = adjusted_tube_diffuse_amount > 0.0f;
        bool _7930;
        if (!_7923)
        {
            _7930 = global_HSM_CRT_BLEND_MODE == 2.0f;
        }
        else
        {
            _7930 = _7923;
        ...
        if (_7930)
        {
            ...
            float2 _7937 = HSM_GetMirrorWrappedCoord(param_51);
            diffuse_coord = _7937;
            ...
            float3 _7987 = HSM_GetPreMultipliedColorLinear(param_57, param_58, param_59).xyz * HSM_GLOBAL_GRAPHICS_BRIGHTNESS;
            tube_diffuse.x = _7987.x;
            tube_diffuse.y = _7987.y;
            tube_diffuse.z = _7987.z;
            ...
            float3 _8007 = HSM_ApplyHSVAdjustment(param_60, param_61, param_62, param_63, param_64, param_65);
            tube_diffuse.x = _8007.x;
            tube_diffuse.y = _8007.y;
            tube_diffuse.z = _8007.z;
            float4 _8015 = tube_diffuse;
            float3 _8017 = _8015.xyz * HSM_TUBE_DIFFUSE_IMAGE_AMOUNT;
            tube_diffuse.x = _8017.x;
            tube_diffuse.y = _8017.y;
            tube_diffuse.z = _8017.z;
            float4 _8025 = tube_diffuse;
            float3 _8027 = _8025.xyz * tube_shadow;
            tube_diffuse.x = _8027.x;
            tube_diffuse.y = _8027.y;
            tube_diffuse.z = _8027.z;
        }
        ...
        float3 _8042 = HSM_ApplyAmbientImage(param_66, param_67, param_68);
        tube_diffuse.x = _8042.x;
        tube_diffuse.y = _8042.y;
        tube_diffuse.z = _8042.z;
        ...
        float3 _8057 = HSM_ApplyAmbientImage(param_69, param_70, param_71);
        tube_diffuse.x = _8057.x;
        tube_diffuse.y = _8057.y;
        tube_diffuse.z = _8057.z;
    }
    tube_diffuse *= HSM_GetTubeOpacity();
    tube_diffuse.w *= tube_mask;
    out_color = tube_diffuse;
    float4 _8075 = out_color;
    float3 _8077 = _8075.xyz + crt_color.xyz;
    out_color.x = _8077.x;
    out_color.y = _8077.y;
    out_color.z = _8077.z;
    ...
    {
        float4 _8097 = out_color;
        float3 _8099 = _8097.xyz + tube_diffuse.xyz;
        out_color.x = _8099.x;
        out_color.y = _8099.y;
        out_color.z = _8099.z;
    }
    if (global_HSM_CRT_BLEND_MODE == 2.0f)
    {
        ...
        float4 _8126 = HSM_BlendModeLayerMix(param_72, param_73, param_74, param_75);
        out_color = _8126;
    }
    if (HSM_GetUseTubeColoredGelImage())
    {
        float2 param_76 = tube_diffuse_curved_coord;
        float2 _8133 = HSM_GetMirrorWrappedCoord(param_76);
        float2 gel_coord = _8133;
        ...
        {
            float4 _8173 = out_color;
            float4 _8175 = out_color;
            float3 _8182 = lerp(_8173.xyz, _8175.xyz * gel_image.xyz, HSM_TUBE_COLORED_GEL_IMAGE_MULTIPLY_AMOUNT.xxx);
            out_color.x = _8182.x;
            out_color.y = _8182.y;
            out_color.z = _8182.z;
        }
        ...
        {
            float4 _8193 = out_color;
            float4 _8195 = out_color;
            float3 _8202 = lerp(_8193.xyz, _8195.xyz + gel_image.xyz, HSM_TUBE_COLORED_GEL_IMAGE_ADDITIVE_AMOUNT.xxx);
            out_color.x = _8202.x;
            out_color.y = _8202.y;
            out_color.z = _8202.z;
        }
        if (HSM_TUBE_COLORED_GEL_IMAGE_NORMAL_AMOUNT > 0.0f)
        {
            ...
            float4 _8223 = gel_image;
            float3 _8225 = _8223.xyz * HSM_TUBE_COLORED_GEL_IMAGE_NORMAL_BRIGHTNESS;
            gel_image.x = _8225.x;
            gel_image.y = _8225.y;
            gel_image.z = _8225.z;
            float4 _8233 = gel_image;
            float3 _8235 = _8233.xyz * tube_shadow;
            gel_image.x = _8235.x;
            gel_image.y = _8235.y;
            gel_image.z = _8235.z;
            if (HSM_TUBE_COLORED_GEL_IMAGE_FAKE_SCANLINE_AMOUNT > 0.001000000047497451305389404296875f)
            {
                ...
                float4 _8258 = HSM_ApplyScanlineMask(param_82, param_83, param_84, param_85, param_86, param_87);
                gel_image.x = _8258.xyz.x;
                gel_image.y = _8258.xyz.y;
                gel_image.z = _8258.xyz.z;
            }
            ...
            float3 _8274 = HSM_ApplyAmbientImage(param_88, param_89, param_90);
            gel_image.x = _8274.x;
            gel_image.y = _8274.y;
            gel_image.z = _8274.z;
            ...
            float3 _8289 = HSM_ApplyAmbientImage(param_91, param_92, param_93);
            gel_image.x = _8289.x;
            gel_image.y = _8289.y;
            gel_image.z = _8289.z;
            ...
            float4 _8309 = HSM_BlendModeLayerMix(param_94, param_95, param_96, param_97);
            gel_image = _8309;
            if (HSM_SHOW_CRT_ON_TOP_OF_COLORED_GEL == 0.0f)
            {
                ...
                float4 _8322 = HSM_BlendModeLayerMix(param_98, param_99, param_100, param_101);
                out_color = _8322;
            }
            if (HSM_SHOW_CRT_ON_TOP_OF_COLORED_GEL == 1.0f)
            {
                float4 _8328 = gel_image;
                float3 _8330 = _8328.xyz * tube_shadow;
                gel_image.x = _8330.x;
                gel_image.y = _8330.y;
                gel_image.z = _8330.z;
                ...
                float4 _8346 = HSM_BlendModeLayerMix(param_102, param_103, param_104, param_105);
                float4 normal_blended = _8346;
                out_color = normal_blended + out_color;
            }
        }
    }
    float4 _8351 = out_color;
    float3 _8353 = _8351.xyz * tube_diffuse_mask;
    out_color.x = _8353.x;
    out_color.y = _8353.y;
    out_color.z = _8353.z;
    out_color *= tube_mask;
    if (HSM_GetUseTubeStaticReflection())
    {
        ...
        float4 _8370 = HSM_GetTubeHighlight(param_106, param_107, TubeStaticReflectionImage_1, _TubeStaticReflectionImage_1_sampler);
        float3 tube_highlight_image = _8370.xyz;
        ...
        float4 _8418 = out_color;
        float3 _8420 = _8418.xyz + (tube_highlight_image * HSM_TUBE_STATIC_REFLECTION_IMAGE_OPACITY);
        out_color.x = _8420.x;
        out_color.y = _8420.y;
        out_color.z = _8420.z;
    }
...
void frag_main()
{
    ...
    {
        _812 = global_HSM_SNAP_TO_CLOSEST_INT_SCALE_TOLERANCE / 100.0f;
    }
    else
    {
        _812 = 0.0f;
    }
    HSM_SNAP_TO_CLOSEST_INT_SCALE_TOLERANCE = _812;
    ...
    bool _8444 = HSM_AB_COMPARE_FREEZE_CRT_TUBE == 1.0f;
    bool _8452;
    if (_8444)
    {
        float2 param = vTexCoord;
        _8452 = HSM_GetIsInABCompareArea(param);
    }
    else
    {
        _8452 = _8444;
    ...
    float4 _8472 = HSM_UpdateGlobalScreenValuesFromCache(InfoCachePass, _InfoCachePass_sampler, param_3);
    float2 cache_bounds_coord = TUBE_COORD;
    float2 param_4 = cache_bounds_coord;
    if (HHLP_IsOutsideCoordSpace(param_4))
    {
        FragColor = 0.0f.xxxx;
        return;
    }
    float2 param_5 = TUBE_DIFFUSE_COORD;
    float2 param_6 = TUBE_DIFFUSE_SCALE;
    float2 param_7 = TUBE_SCALE;
    float param_8 = SCREEN_ASPECT;
    bool param_9 = false;
    float2 param_10 = BEZEL_OUTSIDE_SCALE;
    float2 param_11 = BEZEL_OUTSIDE_COORD;
    float2 param_12 = BEZEL_OUTSIDE_CURVED_COORD;
    float2 param_13 = FRAME_OUTSIDE_CURVED_COORD;
    float _8498 = HSM_GetBezelCoords(param_5, param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13);
    ...
    float2 param_14 = SCREEN_COORD;
    float param_15 = 1.0f;
    float param_16 = SCREEN_ASPECT;
    float2 screen_curved_coord = HSM_GetCurvedCoord(param_14, param_15, param_16);
    FragColor = Source.Sample(_Source_sampler, UNFLIPPED_VIEWPORT_COORD);
    float4 param_17 = FragColor;
    float2 param_18 = vTexCoord;
    float2 param_19 = screen_curved_coord;
    float4 _8529 = HSM_GetPostCrtPreppedColor(param_17, param_18, param_19, DerezedPass, _DerezedPass_sampler, TubeDiffuseImage, _TubeDiffuseImage_sampler, TubeColoredGelImage, _TubeColoredGelImage_sampler, TubeStaticReflectionImage, _TubeStaticReflectionImage_sampler, BackgroundImage, _BackgroundImage_sampler, BackgroundVertImage, _BackgroundVertImage_sampler, NightLightingImage, _NightLightingImage_sampler, NightLighting2Image, _NightLighting2Image_sampler);
    FragColor = _8529;
}
