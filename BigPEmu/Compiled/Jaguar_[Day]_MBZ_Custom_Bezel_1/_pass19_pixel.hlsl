...
float3 maxscale(float2 sinangle, float2 cosangle, float in_radius, float in_distance, float in_aspect)
{
    ...
    float2 _3937 = fwtrans(param_5, param_6, param_7, param_8, param_9);
    ...
    float2 _3954 = fwtrans(param_10, param_11, param_12, param_13, param_14);
    float2 lo = float2(_3937.x, _3954.y) / aspect_vec2;
    ...
    float2 _3974 = fwtrans(param_15, param_16, param_17, param_18, param_19);
    ...
    float2 _3990 = fwtrans(param_20, param_21, param_22, param_23, param_24);
    float2 hi = float2(_3974.x, _3990.y) / aspect_vec2;
...
float2 HSM_GetGeomCurvedCoord(float2 in_coord, float tilt_x, float tilt_y, float in_radius, inout float in_distance, float in_aspect)
{
    ...
    float2 _4087 = transform(param_5, param_6, param_7, param_8, param_9, param_10, param_11);
    float2 curved_coord = _4087;
    return curved_coord;
}
...
float2 HSM_GetGeomCurvedCoordRetainWidth(float2 in_coord, float tilt_x, float tilt_y, float in_radius, float in_distance, float in_aspect)
{
    ...
    float2 _4104 = HSM_GetGeomCurvedCoord(param, param_1, param_2, param_3, param_4, param_5);
    float2 ctr_curved_coord = _4104 - 0.5f.xx;
    ...
    float2 _4119 = HSM_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11);
    float2 right_edge_curved_ctr_coord = _4119 - 0.5f.xx;
...
float3 hrg_get_ideal_global_eye_pos_for_points(inout float3 eye_pos, float2 output_aspect, float3 global_coords[9], int num_points, float in_geom_radius, float in_geom_view_dist)
{
    ...
    for (int iter = 0; iter < max_centering_iters; iter++)
    {
        ...
        float3 _2667 = eye_pos;
        float2 _2669 = _2667.xy + center_offset;
        eye_pos.x = _2669.x;
        eye_pos.y = _2669.y;
...
float3 hrg_get_ideal_global_eye_pos(float3x3 local_to_global, float2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist)
{
    ...
    if (in_geom_mode < 2.5f)
    {
        _2828 = float3(0.0f, 0.0f, eye_z_spherical);
    }
    else
    {
        _2828 = float3(0.0f, 0.0f, max(in_geom_view_dist, eye_z_spherical));
    }
    float3 eye_pos = _2828;
    int num_points = 9;
    ...
    else
    {
        ...
        float3 _2987 = hrg_get_ideal_global_eye_pos_for_points(param_36, param_37, param_38, param_39, param_40, param_41);
        _2970 = _2987;
    }
    return _2970;
}
...
float2 hrg_get_curved_video_uv_coords_and_tangent_matrix(float2 flat_video_uv, float3 eye_pos_local, float2 output_pixel_size, float2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, float3x3 global_to_local, out float2x2 pixel_to_tangent_video_uv)
{
    ...
    float3 param_5;
    float2 _3212 = hrg_view_vec_to_uv(param, param_1, param_2, param_3, param_4, param_5);
    float3 pos = param_5;
    float2 centered_uv = _3212;
    ...
    if (geom_force_correct_tangent_matrix)
    {
        ...
        {
            _3224 = float3(pos.x, 0.0f, pos.z);
        }
        float3 normal_base = _3224;
...
float2 HRG_GetGeomCurvedCoord(float2 in_coord, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, float in_geom_tilt_angle_x, float in_geom_tilt_angle_y, float in_screen_aspect, float pin_inner_edge, float2 in_source_size, float2 in_output_size, out float2x2 pixel_to_video_uv)
{
    ...
    if (in_geom_mode > 0.5f)
    {
        ...
        float2x2 param_14;
        float2 _3392 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14);
        pixel_to_video_uv = param_14;
        float2 ctr_curved_coord = _3392 - 0.5f.xx;
        if ((in_geom_tilt_angle_x == 0.0f) && (in_geom_tilt_angle_y == 0.0f))
        {
            ...
            float2x2 param_23;
            float2 _3421 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_15, param_16, param_17, param_18, param_19, param_20, param_21, param_22, param_23);
            pixel_to_video_uv = param_23;
            float2 right_edge_curved_ctr_coord = _3421 - 0.5f.xx;
            ...
            float2x2 param_32;
            float2 _3443 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_24, param_25, param_26, param_27, param_28, param_29, param_30, param_31, param_32);
            pixel_to_video_uv = param_32;
            float2 bottom_edge_curved_ctr_coord = _3443 - 0.5f.xx;
            ctr_curved_coord.x = (ctr_curved_coord.x * 0.5f) / right_edge_curved_ctr_coord.x;
        ...
        if (pin_inner_edge == 1.0f)
        {
            if (in_geom_tilt_angle_y != 0.0f)
            {
                ...
                float2x2 param_41;
                float2 _3486 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_33, param_34, param_35, param_36, param_37, param_38, param_39, param_40, param_41);
                pixel_to_video_uv = param_41;
                float2 top_edge_curved_ctr_coord = _3486;
            ...
            if (in_geom_tilt_angle_x != 0.0f)
            {
                ...
                float2x2 param_50;
                float2 _3515 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_42, param_43, param_44, param_45, param_46, param_47, param_48, param_49, param_50);
                pixel_to_video_uv = param_50;
                float2 left_edge_curved_ctr_coord = _3515;
...
float2 HSM_GetCurvatureValues(float screen_aspect)
{
    ...
    {
        _3648 = float2((((2.0f * global_HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f, ((global_HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f);
    }
    else
    {
        _3648 = float2(((global_HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f, (((2.0f * global_HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f);
    }
    float2 curvature_values = _3648;
...
float2 HSM_Get2DCurvedCoord(float2 in_coord, float2 curvature_values)
{
    ...
    float2 _4183 = HSM_GetCrtPiCurvedCoord(param, param_1);
    ctr_curved_coord = _4183 - 0.5f.xx;
    float2 param_2 = float2(1.0f, 0.5f);
    float2 param_3 = curvature_values;
    float2 _4190 = HSM_GetCrtPiCurvedCoord(param_2, param_3);
    float2 right_edge_curved_ctr_coord = _4190 - 0.5f.xx;
    ctr_curved_coord.x = (ctr_curved_coord.x * 0.5f) / right_edge_curved_ctr_coord.x;
    float2 param_4 = float2(0.5f, 1.0f);
    float2 param_5 = curvature_values;
    float2 _4204 = HSM_GetCrtPiCurvedCoord(param_4, param_5);
    float2 bottom_edge_curved_ctr_coord = _4204 - 0.5f.xx;
    ctr_curved_coord.y = (ctr_curved_coord.y * 0.5f) / bottom_edge_curved_ctr_coord.y;
    return ctr_curved_coord + 0.5f.xx;
}
...
float2 HSM_GetCurvedCoord(float2 in_coord, float curvature_multiplier, float screen_aspect)
{
    ...
    if (HSM_CURVATURE_MODE > CURVATURE_MODE_2D_CYLINDER)
    {
        ...
        else
        {
            ...
            float2x2 param_16;
            float2 _4311 = HRG_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14, param_15, param_16);
...
float2 HSM_GetTubeCurvedCoord(float2 screen_coord, float curvature_scale, float2 screen_scale, float2 tube_scale, float screen_aspect, float apply_black_edge_offset)
{
    ...
    if (global_HSM_BZL_USE_INDEPENDENT_CURVATURE == 1.0f)
    {
        ...
        if (screen_aspect < 1.0f)
        {
            _4506 = float2((((2.0f * global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f, ((global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f);
        }
        else
        {
            _4506 = float2(((global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f, (((2.0f * global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f);
        }
        float2 curvature_values = _4506;
...
float HSM_GetBezelCoords(float2 tube_diffuse_coord, float2 tube_diffuse_scale, float2 tube_scale, float screen_aspect, bool curve_coords_on, out float2 bezel_outside_scale, inout float2 bezel_outside_coord, inout float2 bezel_outside_curved_coord, inout float2 frame_outside_curved_coord)
{
    ...
    if (curve_coords_on)
    {
        bool _4760 = HSM_CURVATURE_MODE > 0.0f;
        bool _4768;
        if (_4760)
        {
            _4768 = (HSM_CURVATURE_3D_TILT_ANGLE_X != 0.0f) || (HSM_CURVATURE_3D_TILT_ANGLE_Y != 0.0f);
        }
        else
        {
            _4768 = _4760;
...
{
    bool _1646 = abs(in_coord.x - 0.5f) > 0.500999987125396728515625f;
    bool _1656;
    if (!_1646)
    {
        _1656 = abs(in_coord.y - 0.5f) > 0.500999987125396728515625f;
    }
    else
    {
        _1656 = _1646;
...
float2 HSM_GetMirrorWrappedCoord(inout float2 in_coord)
{
    ...
    bool _4373 = abs(ctr_coord.x) > 0.5f;
    bool _4381;
    if (!_4373)
    {
        _4381 = abs(ctr_coord.y) > 0.5f;
    }
    else
    {
        _4381 = _4373;
...
float2 HSM_GetCrtShaderFinalCoord(float2 screen_coord, float2 screen_scale, float raster_bloom_avg_lum, inout float2 screen_curved_coord)
{
    ...
    float2 _4645 = HSM_GetMirrorWrappedCoord(param_1);
    float2 screen_curved_coord_with_overscan_and_mirror = _4645;
...
float4 HSM_GetCroppedTexSample(Texture2D<float4> in_sampler, SamplerState _in_sampler_sampler, float2 in_screen_coord)
{
    ...
    float4 _4580 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler, _in_sampler_sampler, param, param_1, param_2);
    return _4580;
}
...
float SlotMask(inout float2 pos, float m)
{
    ...
    else
    {
        ...
        {
            bool _6166 = py == global_double_slot;
            bool _6173;
            if (_6166)
            {
                _6173 = px >= global_slotwidth;
            }
            else
            {
                _6173 = _6166;
...
float3 Mask(inout float2 pos, float mx, float mb, out float mask_average_luma)
{
    ...
    bool _4955 = frac(abs(global_mshift)) > 0.25f;
    bool _4963;
    if (_4955)
    {
        _4963 = abs(global_mshift) > 1.25f;
    }
    else
    {
        _4963 = _4955;
    ...
    {
        _4977 = pos0.x + (next_line * global_mshift);
    }
    else
    {
        _4977 = pos0.x + (pos.y * global_mshift);
    }
    pos0.x = _4977;
    ...
    bool _5027 = global_shadowMask == 8.0f;
    bool _5035;
    if (!_5027)
    {
        _5035 = global_shadowMask == 11.0f;
    }
    else
    {
        _5035 = _5027;
...
void frag_main()
{
    ...
    {
        _718 = global_HSM_SNAP_TO_CLOSEST_INT_SCALE_TOLERANCE / 100.0f;
    }
    else
    {
        _718 = 0.0f;
    ...
    float4 _6610 = HSM_UpdateGlobalScreenValuesFromCache(InfoCachePass, _InfoCachePass_sampler, param_1);
    ...
    float2 param_15 = screen_curved_coord;
    float2 _6689 = HSM_GetCrtShaderFinalCoord(param_12, param_13, param_14, param_15);
    screen_curved_coord = param_15;
    float2 screen_curved_coord_with_overscan_and_mirror = _6689;
    float2 pos = screen_curved_coord_with_overscan_and_mirror;
    float3 color = Source.Sample(_Source_sampler, pos1).xyz;
    float2 param_16 = pos;
    float3 Bloom = HSM_GetCroppedTexSample(BloomPass, _BloomPass_sampler, param_16).xyz;
    float2 param_17 = ((screen_curved_coord - 0.5f.xx) * 0.999000012874603271484375f) + 0.5f.xx;
    float param_18 = SCREEN_ASPECT;
    float param_19 = global_HSM_GLOBAL_CORNER_RADIUS * (global_HSM_SCREEN_CORNER_RADIUS_SCALE / 100.0f);
    float param_20 = 0.89999997615814208984375f;
    ...
    if (is_inside_screen)
    {
        _6735 = global_GDV_DECONVERGENCE_ON > 0.5f;
    }
    else
    {
    ...
    if (_6735)
    {
        _6762 = (((((abs(global_deconrr) + abs(global_deconrry)) + abs(global_deconrg)) + abs(global_deconrgy)) + abs(global_deconrb)) + abs(global_deconrby)) > 0.20000000298023223876953125f;
    }
    else
    {
        _6762 = _6735;
    ...
    if (_6762)
    {
        float3 param_21 = color;
        float3 param_22 = Bloom;
        float2 param_23 = pos1;
        float2 param_24 = pos;
    ...
    float2 param_28 = maskcoord;
    float param_29 = mx;
    float param_30 = mb;
    float param_31 = mask_average_luma;
    float3 _6928 = Mask(param_28, param_29, param_30, param_31);
    ...
    if (interb)
    {
        ...
        {
            _7052 = pow(0.800000011920928955078125f * bb, 0.64999997615814208984375f);
        }
        else
        {
            _7052 = pow(bb, 0.699999988079071044921875f);
        }
        bb = _7052;
    }
    color *= bb;
    float2 param_34 = pos;
    float3 Ref = HSM_GetCroppedTexSample(LinearizePass, _LinearizePass_sampler, param_34).xyz;
    float2 param_35 = pos;
    float3 Glow = HSM_GetCroppedTexSample(GlowPass, _GlowPass_sampler, param_35).xyz;
    float2 param_36 = pos;
    float maxb = HSM_GetCroppedTexSample(BloomPass, _BloomPass_sampler, param_36).w;
    float3 Bloom1 = Bloom;
    if (abs(global_bloom) > 0.02500000037252902984619140625f)
    {
        if (global_bloom < (-0.00999999977648258209228515625f))
        {
            float3 param_37 = Bloom;
            float param_38 = maxb;
    ...
    if (!interb)
    {
        float3 param_39 = color;
        float param_40 = lerp(1.0f, w3, 0.60000002384185791015625f);
        float3 _7183 = declip(param_39, param_40);
        color = _7183;
    }
    else
    {
    ...
    else
    {
        if (global_halation < (-0.00999999977648258209228515625f))
        {
            ...
            float3 param_41 = ((Bloom + Ref) + orig1) + ((Bloom * Bloom) * Bloom);
            float param_42 = min(mbl * mbl, 0.75f);
    ...
    else
    {
        ...
        float3 param_43 = orig1 + (Ref * 0.001000000047497451305389404296875f);
        float param_44 = 1.0f;
        orig1 = plant(param_43, param_44);
        float3 param_45 = Glow;
        float param_46 = 1.0f;
    ...
    bool _7456 = global_glow >= 0.0f;
    bool _7462;
    if (_7456)
    {
        _7462 = global_m_glow < 0.5f;
    }
    else
    {
        _7462 = _7456;
    ...
    if (global_GDV_NOISE_ON > 0.5f)
    {
        ...
        {
            _7525 = global_noiseresd * global_noiseresd4kmult;
        }
        else
        {
            _7525 = global_noiseresd;
        }
        float noise_res = _7525;
        bool _7541 = global_noiseresd4kmult > 1.0f;
        bool _7548;
        if (_7541)
        {
            _7548 = global_FinalViewportSize.y > 4200.0f;
        }
        else
        {
            _7548 = _7541;
        ...
        float3 param_47 = float3(floor((global_OutputSize.xy * vTexCoord) / noise_res.xx), float(global_FrameCount));
        float3 _7570 = _noise(param_47);
        float3 noise0 = _7570;
    ...
    float param_48 = lerp(pos.y, pos.x, adjusted_bardir);
    float _7620 = humbar(param_48);
    color *= _7620;
    FragColor = float4(color, 1.0f);
    float4 param_49 = FragColor;
