...
{
    bool _1174 = abs(in_coord.x - 0.5f) > 0.500999987125396728515625f;
    bool _1184;
    if (!_1174)
    {
        _1184 = abs(in_coord.y - 0.5f) > 0.500999987125396728515625f;
    }
    else
    {
        _1184 = _1174;
...
float2 HSM_GetCurvatureValues(float screen_aspect)
{
    ...
    {
        _1784 = float2((((2.0f * global_HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f, ((global_HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f);
    }
    else
    {
        _1784 = float2(((global_HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f, (((2.0f * global_HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f);
    }
    float2 curvature_values = _1784;
...
float2 HSM_Get2DCurvedCoord(float2 in_coord, float2 curvature_values)
{
    ...
    float2 _1867 = HSM_GetCrtPiCurvedCoord(param, param_1);
    ctr_curved_coord = _1867 - 0.5f.xx;
    float2 param_2 = float2(1.0f, 0.5f);
    float2 param_3 = curvature_values;
    float2 _1875 = HSM_GetCrtPiCurvedCoord(param_2, param_3);
    float2 right_edge_curved_ctr_coord = _1875 - 0.5f.xx;
    ...
    float2 _1890 = HSM_GetCrtPiCurvedCoord(param_4, param_5);
    float2 bottom_edge_curved_ctr_coord = _1890 - 0.5f.xx;
...
float2 HSM_GetMirrorWrappedCoord(inout float2 in_coord)
{
    ...
    bool _1981 = abs(ctr_coord.x) > 0.5f;
    bool _1989;
    if (!_1981)
    {
        _1989 = abs(ctr_coord.y) > 0.5f;
    }
    else
    {
        _1989 = _1981;
...
float2 HSM_GetCrtShaderFinalCoord(float2 screen_coord, float2 screen_scale, float raster_bloom_avg_lum, inout float2 screen_curved_coord)
{
    ...
    float2 _2171 = HSM_GetMirrorWrappedCoord(param_1);
    float2 screen_curved_coord_with_overscan_and_mirror = _2171;
...
float4 HSM_GetCroppedTexSample(Texture2D<float4> in_sampler, SamplerState _in_sampler_sampler, float2 in_screen_coord)
{
    ...
    float4 _2106 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler, _in_sampler_sampler, param, param_1, param_2);
    return _2106;
}
...
float SlotMask(inout float2 pos, float m)
{
    ...
    else
    {
        ...
        {
            bool _3487 = py == global_double_slot;
            bool _3494;
            if (_3487)
            {
                _3494 = px >= global_slotwidth;
            }
            else
            {
                _3494 = _3487;
...
float3 Mask(inout float2 pos, float mx, float mb, out float mask_average_luma)
{
    ...
    bool _2271 = frac(abs(global_mshift)) > 0.25f;
    bool _2279;
    if (_2271)
    {
        _2279 = abs(global_mshift) > 1.25f;
    }
    else
    {
        _2279 = _2271;
    ...
    {
        _2293 = pos0.x + (next_line * global_mshift);
    }
    else
    {
        _2293 = pos0.x + (pos.y * global_mshift);
    }
    pos0.x = _2293;
    ...
    bool _2343 = global_shadowMask == 8.0f;
    bool _2351;
    if (!_2343)
    {
        _2351 = global_shadowMask == 11.0f;
    }
    else
    {
        _2351 = _2343;
...
void frag_main()
{
    ...
    {
        _481 = global_HSM_SNAP_TO_CLOSEST_INT_SCALE_TOLERANCE / 100.0f;
    }
    else
    {
        _481 = 0.0f;
    ...
    float4 _3933 = HSM_UpdateGlobalScreenValuesFromCache(InfoCachePass, _InfoCachePass_sampler, param_1);
    ...
    float2 _3985 = HSM_GetCrtShaderFinalCoord(param_3, param_4, param_5, param_6);
    screen_curved_coord = param_6;
    float2 screen_curved_coord_with_overscan_and_mirror = _3985;
    ...
    if (is_inside_screen)
    {
        _4033 = global_GDV_DECONVERGENCE_ON > 0.5f;
    }
    else
    {
    ...
    if (_4033)
    {
        _4060 = (((((abs(global_deconrr) + abs(global_deconrry)) + abs(global_deconrg)) + abs(global_deconrgy)) + abs(global_deconrb)) + abs(global_deconrby)) > 0.20000000298023223876953125f;
    }
    else
    {
        _4060 = _4033;
    ...
    float3 _4227 = Mask(param_19, param_20, param_21, param_22);
    ...
    if (interb)
    {
        ...
        {
            _4351 = pow(0.800000011920928955078125f * bb, 0.64999997615814208984375f);
        }
        else
        {
            _4351 = pow(bb, 0.699999988079071044921875f);
        }
        bb = _4351;
    }
    color *= bb;
    ...
    if (!interb)
    {
        ...
        float3 _4482 = declip(param_30, param_31);
        color = _4482;
    }
    else
    {
    ...
    bool _4755 = global_glow >= 0.0f;
    bool _4761;
    if (_4755)
    {
        _4761 = global_m_glow < 0.5f;
    }
    else
    {
        _4761 = _4755;
    ...
    if (global_GDV_NOISE_ON > 0.5f)
    {
        ...
        {
            _4825 = global_noiseresd * global_noiseresd4kmult;
        }
        else
        {
            _4825 = global_noiseresd;
        }
        float noise_res = _4825;
        bool _4841 = global_noiseresd4kmult > 1.0f;
        bool _4848;
        if (_4841)
        {
            _4848 = global_FinalViewportSize.y > 4200.0f;
        }
        else
        {
            _4848 = _4841;
