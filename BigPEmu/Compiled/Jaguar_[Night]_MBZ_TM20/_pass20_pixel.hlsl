...
bool HSM_GetIsInABCompareArea(float2 viewport_coord)
{
    ...
    {
        _6509 = viewport_coord.y;
    }
    else
    {
        _6509 = viewport_coord.x;
    }
    float test_value = _6509;
    float _6522;
    if (mod(HSM_AB_COMPARE_AREA, 2.0f) == 1.0f)
    {
        _6522 = 1.0f - HSM_AB_COMPARE_SPLIT_POSITION;
    }
    else
    {
        _6522 = HSM_AB_COMPARE_SPLIT_POSITION;
    }
    float position = _6522;
    bool _6536 = (mod(HSM_AB_COMPARE_AREA, 2.0f) == 0.0f) && (test_value < position);
    bool _6547;
    if (!_6536)
    {
        _6547 = (mod(HSM_AB_COMPARE_AREA, 2.0f) == 1.0f) && (test_value > position);
    }
    else
    {
        _6547 = _6536;
...
{
    bool _3053 = abs(in_coord.x - 0.5f) > 0.500999987125396728515625f;
    bool _3062;
    if (!_3053)
    {
        _3062 = abs(in_coord.y - 0.5f) > 0.500999987125396728515625f;
    }
    else
    {
        _3062 = _3053;
...
float3 maxscale(float2 sinangle, float2 cosangle, float in_radius, float in_distance, float in_aspect)
{
    ...
    float2 _5511 = fwtrans(param_5, param_6, param_7, param_8, param_9);
    ...
    float2 _5528 = fwtrans(param_10, param_11, param_12, param_13, param_14);
    float2 lo = float2(_5511.x, _5528.y) / aspect_vec2;
    ...
    float2 _5548 = fwtrans(param_15, param_16, param_17, param_18, param_19);
    ...
    float2 _5564 = fwtrans(param_20, param_21, param_22, param_23, param_24);
    float2 hi = float2(_5548.x, _5564.y) / aspect_vec2;
...
float2 HSM_GetGeomCurvedCoord(float2 in_coord, float tilt_x, float tilt_y, float in_radius, inout float in_distance, float in_aspect)
{
    ...
    float2 _5661 = transform(param_5, param_6, param_7, param_8, param_9, param_10, param_11);
    float2 curved_coord = _5661;
    return curved_coord;
}
...
float2 HSM_GetGeomCurvedCoordRetainWidth(float2 in_coord, float tilt_x, float tilt_y, float in_radius, float in_distance, float in_aspect)
{
    ...
    float2 _5678 = HSM_GetGeomCurvedCoord(param, param_1, param_2, param_3, param_4, param_5);
    float2 ctr_curved_coord = _5678 - 0.5f.xx;
    ...
    float2 _5693 = HSM_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11);
    float2 right_edge_curved_ctr_coord = _5693 - 0.5f.xx;
...
float3 hrg_uv_to_xyz(float2 uv, float2 output_aspect, float in_geom_mode, float in_geom_radius)
{
    ...
    if (in_geom_mode < 1.5f)
    {
        ...
        _3892 = hrg_sphere_uv_to_xyz(param, param_1, param_2);
    }
    else
    {
        float3 _3905;
        if (in_geom_mode < 2.5f)
        {
            ...
            _3905 = hrg_sphere_alt_uv_to_xyz(param_3, param_4, param_5);
        }
        else
        {
            float2 param_6 = uv;
            ...
            _3905 = hrg_cylinder_uv_to_xyz(param_6, param_7, param_8);
        }
        _3892 = _3905;
    }
    return _3892;
...
float3 hrg_get_ideal_global_eye_pos_for_points(inout float3 eye_pos, float2 output_aspect, float3 global_coords[9], int num_points, float in_geom_radius, float in_geom_view_dist)
{
    ...
    for (int iter = 0; iter < max_centering_iters; iter++)
    {
        ...
        float3 _4071 = eye_pos;
        float2 _4073 = _4071.xy + center_offset;
        eye_pos.x = _4073.x;
        eye_pos.y = _4073.y;
        ...
        for (int i_3 = 0; i_3 < num_points; i_3++)
        {
            ...
            {
                _4137 = max(offset_z_max, offset_zzzz.x);
            }
            else
            {
                _4137 = offset_z_max;
            ...
            {
                _4150 = max(offset_z_max, offset_zzzz.y);
            }
            else
            {
                _4150 = offset_z_max;
            ...
            if (eyespace_xyz_flipy.x > 0.0f)
            {
                _4163 = max(offset_z_max, offset_zzzz.z);
            }
            else
            {
                _4163 = offset_z_max;
            ...
            if (eyespace_xyz_flipy.y > 0.0f)
            {
                _4176 = max(offset_z_max, offset_zzzz.w);
            }
            else
            {
                _4176 = offset_z_max;
...
float3 hrg_get_ideal_global_eye_pos(float3x3 local_to_global, float2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist)
{
    ...
    if (in_geom_mode < 2.5f)
    {
        _4232 = float3(0.0f, 0.0f, eye_z_spherical);
    }
    else
    {
        _4232 = float3(0.0f, 0.0f, max(in_geom_view_dist, eye_z_spherical));
    }
    float3 eye_pos = _4232;
    int num_points = 9;
    ...
    else
    {
        ...
        float3 _4391 = hrg_get_ideal_global_eye_pos_for_points(param_36, param_37, param_38, param_39, param_40, param_41);
        _4374 = _4391;
    }
    return _4374;
}
...
float2 hrg_intersect(float3 view_vec_local, float3 eye_pos_local, float in_geom_mode, float in_geom_radius)
{
    ...
    if (in_geom_mode < 2.5f)
    {
        ...
        float param_2 = in_geom_radius;
        _3831 = hrg_intersect_sphere(param, param_1, param_2);
    }
    else
    {
        ...
        float param_5 = in_geom_radius;
        _3831 = hrg_intersect_cylinder(param_3, param_4, param_5);
    }
    return _3831;
}
...
float2 hrg_xyz_to_uv(float3 intersection_pos_local, float2 output_aspect, float in_geom_mode, float in_geom_radius)
{
    ...
    if (in_geom_mode < 1.5f)
    {
        ...
        float2 param_1 = output_aspect;
        float param_2 = in_geom_radius;
        _3855 = hrg_sphere_xyz_to_uv(param, param_1, param_2);
    }
    else
    {
        float2 _3868;
        if (in_geom_mode < 2.5f)
        {
            ...
            float2 param_4 = output_aspect;
            float param_5 = in_geom_radius;
            _3868 = hrg_sphere_alt_xyz_to_uv(param_3, param_4, param_5);
        }
        else
        {
            ...
            float2 param_7 = output_aspect;
            float param_8 = in_geom_radius;
            _3868 = hrg_cylinder_xyz_to_uv(param_6, param_7, param_8);
        }
        _3855 = _3868;
    }
    return _3855;
...
float2 hrg_view_vec_to_uv(float3 view_vec_local, float3 eye_pos_local, float2 output_aspect, float in_geom_mode, float in_geom_radius, out float3 intersection_pos)
{
    ...
    if (intersect_dist_and_discriminant.y > 0.004999999888241291046142578125f)
    {
        ...
        float param_6 = in_geom_mode;
        float param_7 = in_geom_radius;
        _3949 = hrg_xyz_to_uv(param_4, param_5, param_6, param_7);
    }
    else
    {
        _3949 = 1.0f.xx;
...
float2 hrg_get_curved_video_uv_coords_and_tangent_matrix(float2 flat_video_uv, float3 eye_pos_local, float2 output_pixel_size, float2 output_aspect, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, float3x3 global_to_local, out float2x2 pixel_to_tangent_video_uv)
{
    ...
    float3 param_5;
    float2 _4616 = hrg_view_vec_to_uv(param, param_1, param_2, param_3, param_4, param_5);
    float3 pos = param_5;
    float2 centered_uv = _4616;
    ...
    if (geom_force_correct_tangent_matrix)
    {
        ...
        {
            _4628 = float3(pos.x, 0.0f, pos.z);
        }
        float3 normal_base = _4628;
...
float2 HRG_GetGeomCurvedCoord(float2 in_coord, float in_geom_mode, float in_geom_radius, float in_geom_view_dist, float in_geom_tilt_angle_x, float in_geom_tilt_angle_y, float in_screen_aspect, float pin_inner_edge, float2 in_source_size, float2 in_output_size, out float2x2 pixel_to_video_uv)
{
    ...
    if (in_geom_mode > 0.5f)
    {
        ...
        float2x2 param_14;
        float2 _4796 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14);
        pixel_to_video_uv = param_14;
        float2 ctr_curved_coord = _4796 - 0.5f.xx;
        if ((in_geom_tilt_angle_x == 0.0f) && (in_geom_tilt_angle_y == 0.0f))
        {
            ...
            float2x2 param_23;
            float2 _4825 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_15, param_16, param_17, param_18, param_19, param_20, param_21, param_22, param_23);
            pixel_to_video_uv = param_23;
            float2 right_edge_curved_ctr_coord = _4825 - 0.5f.xx;
            ...
            float2x2 param_32;
            float2 _4847 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_24, param_25, param_26, param_27, param_28, param_29, param_30, param_31, param_32);
            pixel_to_video_uv = param_32;
            float2 bottom_edge_curved_ctr_coord = _4847 - 0.5f.xx;
            ctr_curved_coord.x = (ctr_curved_coord.x * 0.5f) / right_edge_curved_ctr_coord.x;
        ...
        if (pin_inner_edge == 1.0f)
        {
            if (in_geom_tilt_angle_y != 0.0f)
            {
                ...
                float2x2 param_41;
                float2 _4890 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_33, param_34, param_35, param_36, param_37, param_38, param_39, param_40, param_41);
                pixel_to_video_uv = param_41;
                float2 top_edge_curved_ctr_coord = _4890;
            ...
            if (in_geom_tilt_angle_x != 0.0f)
            {
                ...
                float2x2 param_50;
                float2 _4919 = hrg_get_curved_video_uv_coords_and_tangent_matrix(param_42, param_43, param_44, param_45, param_46, param_47, param_48, param_49, param_50);
                pixel_to_video_uv = param_50;
                float2 left_edge_curved_ctr_coord = _4919;
...
float2 HSM_GetCurvatureValues(float screen_aspect)
{
    ...
    {
        _5222 = float2((((2.0f * global_HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f, ((global_HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f);
    }
    else
    {
        _5222 = float2(((global_HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f, (((2.0f * global_HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f);
    }
    float2 curvature_values = _5222;
...
float2 HSM_GetCrtPiCurvedCoord(inout float2 in_coord, inout float2 in_curvature)
{
    ...
    {
        _5726 = in_coord.y * in_coord.y;
    }
    float rsq = (in_coord.x * in_coord.x) + _5726;
...
float2 HSM_Get2DCurvedCoord(float2 in_coord, float2 curvature_values)
{
    ...
    float2 _5757 = HSM_GetCrtPiCurvedCoord(param, param_1);
    ctr_curved_coord = _5757 - 0.5f.xx;
    float2 param_2 = float2(1.0f, 0.5f);
    float2 param_3 = curvature_values;
    float2 _5764 = HSM_GetCrtPiCurvedCoord(param_2, param_3);
    float2 right_edge_curved_ctr_coord = _5764 - 0.5f.xx;
    ctr_curved_coord.x = (ctr_curved_coord.x * 0.5f) / right_edge_curved_ctr_coord.x;
    float2 param_4 = float2(0.5f, 1.0f);
    float2 param_5 = curvature_values;
    float2 _5778 = HSM_GetCrtPiCurvedCoord(param_4, param_5);
    float2 bottom_edge_curved_ctr_coord = _5778 - 0.5f.xx;
    ctr_curved_coord.y = (ctr_curved_coord.y * 0.5f) / bottom_edge_curved_ctr_coord.y;
    return ctr_curved_coord + 0.5f.xx;
}
...
float2 HSM_GetCurvedCoord(float2 in_coord, float curvature_multiplier, float screen_aspect)
{
    ...
    if (HSM_CURVATURE_MODE > CURVATURE_MODE_2D_CYLINDER)
    {
        ...
        else
        {
            ...
            float2x2 param_16;
            float2 _5885 = HRG_GetGeomCurvedCoord(param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14, param_15, param_16);
...
float2 HSM_GetTubeCurvedCoord(float2 screen_coord, float curvature_scale, float2 screen_scale, float2 tube_scale, float screen_aspect, float apply_black_edge_offset)
{
    ...
    if (global_HSM_BZL_USE_INDEPENDENT_CURVATURE == 1.0f)
    {
        ...
        if (screen_aspect < 1.0f)
        {
            _6042 = float2((((2.0f * global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f, ((global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f);
        }
        else
        {
            _6042 = float2(((global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f, (((2.0f * global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f);
        }
        float2 curvature_values = _6042;
...
float HSM_GetBezelCoords(float2 tube_diffuse_coord, float2 tube_diffuse_scale, float2 tube_scale, float screen_aspect, bool curve_coords_on, out float2 bezel_outside_scale, inout float2 bezel_outside_coord, inout float2 bezel_outside_curved_coord, inout float2 frame_outside_curved_coord)
{
    ...
    if (curve_coords_on)
    {
        bool _7359 = HSM_CURVATURE_MODE > 0.0f;
        bool _7367;
        if (_7359)
        {
            _7367 = (HSM_CURVATURE_3D_TILT_ANGLE_X != 0.0f) || (HSM_CURVATURE_3D_TILT_ANGLE_Y != 0.0f);
        }
        else
        {
            _7367 = _7359;
...
float3 HSM_RGBtoHSV(float3 c)
{
    ...
    if (c.y < c.z)
    {
        _2673 = float4(c.zy, K.wz);
    }
    else
    {
        _2673 = float4(c.yz, K.xy);
    }
    float4 p = _2673;
    float4 _2703;
    if (c.x < p.x)
    {
        _2703 = float4(p.xyw, c.x);
    }
    else
    {
        _2703 = float4(c.x, p.yzx);
    }
    float4 q = _2703;
...
float4 HSM_ApplyMonochrome(float4 in_color)
{
    ...
    float4 _6847 = out_color;
    float3 _6853 = pow(_6847.xyz, global_HSM_MONOCHROME_GAMMA.xxx);
    out_color.x = _6853.x;
    out_color.y = _6853.y;
    out_color.z = _6853.z;
    ...
    bool _6893 = (global_HSM_MONOCHROME_HUE_OFFSET / 360.0f) != 0.0f;
    bool _6902;
    if (!_6893)
    {
        _6902 = (global_HSM_MONOCHROME_SATURATION / 100.0f) != 0.0f;
    }
    else
    {
        _6902 = _6893;
    ...
    float3 _6932 = mcolor * pow(luma, 1.0f / global_HSM_MONOCHROME_GAMMA);
    out_color.x = _6932.x;
    out_color.y = _6932.y;
    out_color.z = _6932.z;
    return out_color;
...
float2 HSM_GetMirrorWrappedCoord(inout float2 in_coord)
{
    ...
    bool _5909 = abs(ctr_coord.x) > 0.5f;
    bool _5917;
    if (!_5909)
    {
        _5917 = abs(ctr_coord.y) > 0.5f;
    }
    else
    {
        _5917 = _5909;
...
{
    float2 param = in_coord;
    float2 _6588 = HSM_GetMirrorWrappedCoord(param);
    float2 vpos = _6588;
...
{
    bool _6637 = global_HSM_SCREEN_VIGNETTE_ON > 0.5f;
    bool _6643;
    if (_6637)
    {
        float param = HSM_SCREEN_VIGNETTE_DUALSCREEN_VIS_MODE;
        _6643 = HSM_GetUseOnCurrentScreenIndex(param);
    }
    else
    {
        _6643 = _6637;
...
float4 HSM_GetCroppedTexSample(Texture2D<float4> in_sampler, SamplerState _in_sampler_sampler, float2 in_screen_coord)
{
    ...
    float4 _6582 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler, _in_sampler_sampler, param, param_1, param_2);
    return _6582;
}
...
float2 GetSimpleImageScaledCoord(inout float2 in_viewport_coord, float2 in_viewport_unscaled_coord, float2 in_tube_coord, float2 in_tube_scale, Texture2D<float4> in_sampler, SamplerState _in_sampler_sampler, float2 in_pos, float in_inherit_pos, float2 in_scale, float in_scale_inherit_mode, float in_keep_aspect, float in_mirror_horz, float in_rotate)
{
    ...
    uint _5148_dummy_parameter;
    float2 tex_size = float2(int2(spvTextureSize(in_sampler, uint(0), _5148_dummy_parameter)));
    float _5153;
    if (in_rotate == 1.0f)
    {
        _5153 = tex_size.y / tex_size.x;
    }
    else
    {
        _5153 = tex_size.x / tex_size.y;
    }
    float tex_aspect = _5153;
    float _5170;
    if (in_keep_aspect == 1.0f)
    {
        _5170 = output_aspect / tex_aspect;
    }
    else
    {
        _5170 = 1.0f;
    ...
    if (in_rotate > 0.5f)
    {
        _5184 = global_FinalViewportSize.x / global_FinalViewportSize.y;
    }
    else
    {
        _5184 = 1.0f;
...
{
    uint _6553_dummy_parameter;
    float2 tex_size = float2(int2(spvTextureSize(in_sampler, uint(0), _6553_dummy_parameter)));
...
float4 HSM_GetNightLightingMultiplyColor(float2 in_coord, float hue, float saturation, float value, float contrast, float global_ambient_opacity, Texture2D<float4> NightLightingImage_1, SamplerState _NightLightingImage_1_sampler)
{
    ...
    float4 _6704 = lighting_image;
    float3 _6708 = lerp(1.0f.xxx, _6704.xyz, global_ambient_opacity.xxx);
    lighting_image.x = _6708.x;
    lighting_image.y = _6708.y;
    lighting_image.z = _6708.z;
...
bool HSM_Fill_Ambient_Images(float2 in_viewport_coord, float2 in_viewport_unscaled_coord, float2 in_tube_coord, float2 in_tube_scale, float in_swap_images, Texture2D<float4> in_ambient_sampler, SamplerState _in_ambient_sampler_sampler, Texture2D<float4> in_ambient2_sampler, SamplerState _in_ambient2_sampler_sampler, out float4 ambient_lighting_image, out float4 ambient2_lighting_image)
{
    ...
    if (HSM_AMBIENT1_OPACITY > 0.0f)
    {
        ...
        bool _6726 = HSM_AMBIENT1_SCALE_INHERIT_MODE == 1.0f;
        bool _6733;
        if (_6726)
        {
            _6733 = (HSM_AMBIENT1_SCALE * HSM_VIEWPORT_ZOOM) < 1.0f;
        }
        else
        {
            _6733 = _6726;
        ...
        float2 _6767 = GetSimpleImageScaledCoord(param, param_1, param_2, param_3, in_ambient_sampler, _in_ambient_sampler_sampler, param_4, param_5, param_6, param_7, param_8, param_9, param_10);
        float2 lighting_coord = _6767;
    ...
    if (HSM_AMBIENT2_OPACITY > 0.0f)
    {
        ...
        bool _6788 = HSM_AMBIENT2_SCALE_INHERIT_MODE == 1.0f;
        bool _6795;
        if (_6788)
        {
            _6795 = (HSM_AMBIENT2_SCALE * HSM_VIEWPORT_ZOOM) < 1.0f;
        }
        else
        {
            _6795 = _6788;
        ...
        float2 _6829 = GetSimpleImageScaledCoord(param_17, param_18, param_19, param_20, in_ambient2_sampler, _in_ambient2_sampler_sampler, param_21, param_22, param_23, param_24, param_25, param_26, param_27);
        float2 lighting2_coord = _6829;
...
{
    bool _7510 = HSM_TUBE_DIFFUSE_MODE == 1.0f;
    bool _7516;
    if (_7510)
    {
        float param = HSM_TUBE_DIFFUSE_IMAGE_DUALSCREEN_VIS_MODE;
        _7516 = HSM_GetUseOnCurrentScreenIndex(param);
    }
    else
    {
        _7516 = _7510;
...
{
    bool _7520 = HSM_TUBE_COLORED_GEL_IMAGE_ON > 0.5f;
    bool _7526;
    if (_7520)
    {
        float param = HSM_TUBE_COLORED_GEL_IMAGE_DUALSCREEN_VIS_MODE;
        _7526 = HSM_GetUseOnCurrentScreenIndex(param);
    }
    else
    {
        _7526 = _7520;
...
{
    bool _7500 = HSM_TUBE_STATIC_REFLECTION_IMAGE_ON > 0.5f;
    bool _7506;
    if (_7500)
    {
        float param = HSM_TUBE_STATIC_REFLECTION_IMAGE_DUALSCREEN_VIS_MODE;
        _7506 = HSM_GetUseOnCurrentScreenIndex(param);
    }
    else
    {
        _7506 = _7500;
...
float4 HSM_GetPreMultipliedColorLinear(float4 in_color, float matte_type, float encoded_gamma)
{
    ...
    {
        float4 _2879 = out_color;
        float _2882 = out_color.w;
        float3 _2888 = clamp(_2879.xyz - (1.0f - _2882).xxx, 0.0f.xxx, 1.0f.xxx);
        out_color.x = _2888.x;
        out_color.y = _2888.y;
        out_color.z = _2888.z;
    }
    ...
    {
        float _2906 = out_color.w;
        float4 _2907 = out_color;
        float3 _2909 = _2907.xyz * _2906;
        out_color.x = _2909.x;
        out_color.y = _2909.y;
        out_color.z = _2909.z;
    }
...
float4 HSM_BlendModeLayerMix(float4 color_under, inout float4 color_over, float blend_mode, float layer_opacity)
{
    ...
    {
        float _2968 = color_over.w;
        float4 _2969 = color_over;
        float3 _2971 = _2969.xyz * _2968;
        color_over.x = _2971.x;
        color_over.y = _2971.y;
        color_over.z = _2971.z;
    ...
    else
    {
        ...
        {
            float3 _2995 = color_under.xyz + color_over.xyz;
            blend_color.x = _2995.x;
            blend_color.y = _2995.y;
            blend_color.z = _2995.z;
        }
        if (blend_mode == BLEND_MODE_MULTIPLY)
        {
            float3 _3011 = color_under.xyz * color_over.xyz;
            blend_color.x = _3011.x;
            blend_color.y = _3011.y;
            blend_color.z = _3011.z;
        }
...
float4 HSM_ApplyScanlineMask(float4 in_color, float2 screen_scale, inout float2 in_coord, float2 in_screen_curved_coord, float2 in_tube_curved_coord, float in_scanline_opacity)
{
    ...
    float scan_axis_screen_scale_res = _7020;
    float _7032;
    if (USE_VERTICAL_SCANLINES > 0.5f)
    {
        _7032 = CROPPED_ROTATED_SIZE.x;
    }
    else
    {
        _7032 = CROPPED_ROTATED_SIZE.y;
    }
    float cropped_rotated_scan_res = _7032;
    float _7046;
    if (global_HSM_FAKE_SCANLINE_RES_MODE > 0.5f)
    {
        _7046 = global_HSM_FAKE_SCANLINE_RES;
    }
    else
    {
        _7046 = cropped_rotated_scan_res;
    }
    float simulated_scanline_res = _7046;
    ...
    float param_1 = 2.0f;
    float _7092 = HHLP_EasePowerOut(param, param_1);
    float color_brightness_modulation = _7092;
    ...
    float param_3 = 2.0f;
    float _7119 = HHLP_EasePowerOut(param_2, param_3);
    color_brightness_modulation = _7119;
...
float4 HSM_GetStoichaicBlurredSample(Texture2D<float4> in_sampler, SamplerState _in_sampler_sampler, float2 in_coord, float num_samples, float max_blur_size, float blur_ratio)
{
    ...
    {
        float param = radius;
        float _6152 = HSM_rand(param);
        radius = param;
        radius_vector.x = _6152;
        float param_1 = radius;
        float _6157 = HSM_rand(param_1);
        radius = param_1;
        radius_vector.y = _6157;
    ...
    if (num_samples < 2.5f)
    {
        ...
        float _6179 = HSM_rand(param_2);
        radius = param_2;
        radius_vector.x = _6179;
        float param_3 = radius;
        float _6184 = HSM_rand(param_3);
        radius = param_3;
        radius_vector.y = _6184;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 2.0f.xxxx);
        float param_4 = radius;
        float _6202 = HSM_rand(param_4);
        radius = param_4;
        radius_vector.x = _6202;
        float param_5 = radius;
        float _6207 = HSM_rand(param_5);
        radius = param_5;
        radius_vector.y = _6207;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 2.0f.xxxx);
    }
    if (num_samples > 2.5f)
    {
        ...
        float _6229 = HSM_rand(param_6);
        radius = param_6;
        radius_vector.x = _6229;
        float param_7 = radius;
        float _6234 = HSM_rand(param_7);
        radius = param_7;
        radius_vector.y = _6234;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_8 = radius;
        float _6252 = HSM_rand(param_8);
        radius = param_8;
        radius_vector.x = _6252;
        float param_9 = radius;
        float _6257 = HSM_rand(param_9);
        radius = param_9;
        radius_vector.y = _6257;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_10 = radius;
        float _6275 = HSM_rand(param_10);
        radius = param_10;
        radius_vector.x = _6275;
        float param_11 = radius;
        float _6280 = HSM_rand(param_11);
        radius = param_11;
        radius_vector.y = _6280;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_12 = radius;
        float _6298 = HSM_rand(param_12);
        radius = param_12;
        radius_vector.x = _6298;
        float param_13 = radius;
        float _6303 = HSM_rand(param_13);
        radius = param_13;
        radius_vector.y = _6303;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_14 = radius;
        float _6321 = HSM_rand(param_14);
        radius = param_14;
        radius_vector.x = _6321;
        float param_15 = radius;
        float _6326 = HSM_rand(param_15);
        radius = param_15;
        radius_vector.y = _6326;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_16 = radius;
        float _6344 = HSM_rand(param_16);
        radius = param_16;
        radius_vector.x = _6344;
        float param_17 = radius;
        float _6349 = HSM_rand(param_17);
        radius = param_17;
        radius_vector.y = _6349;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_18 = radius;
        float _6367 = HSM_rand(param_18);
        radius = param_18;
        radius_vector.x = _6367;
        float param_19 = radius;
        float _6372 = HSM_rand(param_19);
        radius = param_19;
        radius_vector.y = _6372;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_20 = radius;
        float _6390 = HSM_rand(param_20);
        radius = param_20;
        radius_vector.x = _6390;
        float param_21 = radius;
        float _6395 = HSM_rand(param_21);
        radius = param_21;
        radius_vector.y = _6395;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_22 = radius;
        float _6413 = HSM_rand(param_22);
        radius = param_22;
        radius_vector.x = _6413;
        float param_23 = radius;
        float _6418 = HSM_rand(param_23);
        radius = param_23;
        radius_vector.y = _6418;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_24 = radius;
        float _6436 = HSM_rand(param_24);
        radius = param_24;
        radius_vector.x = _6436;
        float param_25 = radius;
        float _6441 = HSM_rand(param_25);
        radius = param_25;
        radius_vector.y = _6441;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_26 = radius;
        float _6459 = HSM_rand(param_26);
        radius = param_26;
        radius_vector.x = _6459;
        float param_27 = radius;
        float _6464 = HSM_rand(param_27);
        radius = param_27;
        radius_vector.y = _6464;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
        float param_28 = radius;
        float _6482 = HSM_rand(param_28);
        radius = param_28;
        radius_vector.x = _6482;
        float param_29 = radius;
        float _6487 = HSM_rand(param_29);
        radius = param_29;
        radius_vector.y = _6487;
        sample_coord = in_coord + (radius_vector * p);
        blurred_color += (in_sampler.Sample(_in_sampler_sampler, abs(sample_coord)) / 12.0f.xxxx);
    }
...
float4 HSM_GetTubeHighlight(inout float2 tube_curved_coord, bool apply_to_mirror, Texture2D<float4> TubeStaticReflectionImage_1, SamplerState _TubeStaticReflectionImage_1_sampler)
{
    ...
    if (apply_to_mirror)
    {
        ...
        float2 _7540 = HSM_GetMirrorWrappedCoord(param_1);
        mirrored_tube_coord = _7540;
    }
    ...
    float4 _7641 = out_color;
    float3 _7643 = _7641.xyz * HSM_GLOBAL_GRAPHICS_BRIGHTNESS;
    out_color.x = _7643.x;
    out_color.y = _7643.y;
    out_color.z = _7643.z;
    return out_color;
...
float4 HSM_Apply_Sinden_Lightgun_Border(float4 in_rgba, float2 in_tube_diffuse_curved_coord, float in_tube_diffuse_mask, float in_black_edge_corner_radius)
{
    ...
    float4 _7218 = out_rgba;
    float3 _7221 = ((base_rgb * HSM_SINDEN_BORDER_BRIGHTNESS) * sinden_gun_mask) + (_7218.xyz * (1.0f - sinden_gun_mask));
    out_rgba.x = _7221.x;
    out_rgba.y = _7221.y;
    out_rgba.z = _7221.z;
    return out_rgba;
...
{
    bool _7655 = global_HSM_MONOCHROME_MODE > 0.5f;
    bool _7661;
    if (_7655)
    {
        float param = HSM_MONOCHROME_DUALSCREEN_VIS_MODE;
        _7661 = HSM_GetUseOnCurrentScreenIndex(param);
    }
    else
    {
        _7661 = _7655;
    ...
    float2 _7677 = HSM_GetMirrorWrappedCoord(param_2);
    float2 mirrored_screen_coord = _7677;
    float4 out_color = 0.0f.xxxx;
    ...
    float2 _7715 = HSM_GetMirrorWrappedCoord(param_13);
    float2 mirrored_tube_coord = _7715;
    ...
    bool _7748 = HSM_AB_COMPARE_SHOW_MODE == 1.0f;
    bool _7754;
    if (_7748)
    {
        float2 param_15 = VIEWPORT_COORD;
        _7754 = HSM_GetIsInABCompareArea(param_15);
    }
    else
    {
        _7754 = _7748;
    ...
    if (_7754)
    {
        ...
        float2 _7768 = HSM_GetMirrorWrappedCoord(param_16);
        ab_screen_coord = _7768;
    ...
    float4 _7846 = crt_color;
    float3 _7848 = _7846.xyz * screen_to_tube_mask_invert;
    crt_color.x = _7848.x;
    crt_color.y = _7848.y;
    crt_color.z = _7848.z;
    ...
    bool _7871 = HSM_Fill_Ambient_Images(param_34, param_35, param_36, param_37, param_38, NightLightingImage_1, _NightLightingImage_1_sampler, NightLighting2Image_1, _NightLighting2Image_1_sampler, param_39, param_40);
    ambient_lighting_image = param_39;
    ambient2_lighting_image = param_40;
    float3 tube_shadow = 1.0f.xxx;
    bool _7876 = HSM_TUBE_SHADOW_IMAGE_ON > 0.5f;
    bool _7890;
    if (_7876)
    {
        bool _7879 = HSM_GetUseTubeDiffuseImage();
        bool _7884;
        if (!_7879)
        {
            _7884 = HSM_GetUseTubeColoredGelImage();
        }
        else
        {
            _7884 = _7879;
        ...
        if (!_7884)
        {
            _7889 = HSM_GetUseTubeStaticReflection();
        }
        else
        {
            _7889 = _7884;
    ...
    if (_7890)
    {
        ...
        float2 _7897 = HSM_GetMirrorWrappedCoord(param_41);
        shadow_coord = _7897;
    ...
    {
        bool _7990 = adjusted_tube_diffuse_amount > 0.0f;
        bool _7997;
        if (!_7990)
        {
            _7997 = global_HSM_CRT_BLEND_MODE == 2.0f;
        }
        else
        {
            _7997 = _7990;
        ...
        if (_7997)
        {
            ...
            float2 _8004 = HSM_GetMirrorWrappedCoord(param_51);
            diffuse_coord = _8004;
            ...
            float3 _8054 = HSM_GetPreMultipliedColorLinear(param_57, param_58, param_59).xyz * HSM_GLOBAL_GRAPHICS_BRIGHTNESS;
            tube_diffuse.x = _8054.x;
            tube_diffuse.y = _8054.y;
            tube_diffuse.z = _8054.z;
            ...
            float3 _8074 = HSM_ApplyHSVAdjustment(param_60, param_61, param_62, param_63, param_64, param_65);
            tube_diffuse.x = _8074.x;
            tube_diffuse.y = _8074.y;
            tube_diffuse.z = _8074.z;
            float4 _8082 = tube_diffuse;
            float3 _8084 = _8082.xyz * HSM_TUBE_DIFFUSE_IMAGE_AMOUNT;
            tube_diffuse.x = _8084.x;
            tube_diffuse.y = _8084.y;
            tube_diffuse.z = _8084.z;
            float4 _8092 = tube_diffuse;
            float3 _8094 = _8092.xyz * tube_shadow;
            tube_diffuse.x = _8094.x;
            tube_diffuse.y = _8094.y;
            tube_diffuse.z = _8094.z;
        }
        ...
        float3 _8109 = HSM_ApplyAmbientImage(param_66, param_67, param_68);
        tube_diffuse.x = _8109.x;
        tube_diffuse.y = _8109.y;
        tube_diffuse.z = _8109.z;
        ...
        float3 _8124 = HSM_ApplyAmbientImage(param_69, param_70, param_71);
        tube_diffuse.x = _8124.x;
        tube_diffuse.y = _8124.y;
        tube_diffuse.z = _8124.z;
    }
    tube_diffuse *= HSM_GetTubeOpacity();
    tube_diffuse.w *= tube_mask;
    out_color = tube_diffuse;
    float4 _8142 = out_color;
    float3 _8144 = _8142.xyz + crt_color.xyz;
    out_color.x = _8144.x;
    out_color.y = _8144.y;
    out_color.z = _8144.z;
    ...
    {
        float4 _8164 = out_color;
        float3 _8166 = _8164.xyz + tube_diffuse.xyz;
        out_color.x = _8166.x;
        out_color.y = _8166.y;
        out_color.z = _8166.z;
    }
    if (global_HSM_CRT_BLEND_MODE == 2.0f)
    {
        ...
        float4 _8193 = HSM_BlendModeLayerMix(param_72, param_73, param_74, param_75);
        out_color = _8193;
    }
    if (HSM_GetUseTubeColoredGelImage())
    {
        float2 param_76 = tube_diffuse_curved_coord;
        float2 _8200 = HSM_GetMirrorWrappedCoord(param_76);
        float2 gel_coord = _8200;
        ...
        {
            float4 _8240 = out_color;
            float4 _8242 = out_color;
            float3 _8249 = lerp(_8240.xyz, _8242.xyz * gel_image.xyz, HSM_TUBE_COLORED_GEL_IMAGE_MULTIPLY_AMOUNT.xxx);
            out_color.x = _8249.x;
            out_color.y = _8249.y;
            out_color.z = _8249.z;
        }
        ...
        {
            float4 _8260 = out_color;
            float4 _8262 = out_color;
            float3 _8269 = lerp(_8260.xyz, _8262.xyz + gel_image.xyz, HSM_TUBE_COLORED_GEL_IMAGE_ADDITIVE_AMOUNT.xxx);
            out_color.x = _8269.x;
            out_color.y = _8269.y;
            out_color.z = _8269.z;
        }
        if (HSM_TUBE_COLORED_GEL_IMAGE_NORMAL_AMOUNT > 0.0f)
        {
            ...
            float4 _8290 = gel_image;
            float3 _8292 = _8290.xyz * HSM_TUBE_COLORED_GEL_IMAGE_NORMAL_BRIGHTNESS;
            gel_image.x = _8292.x;
            gel_image.y = _8292.y;
            gel_image.z = _8292.z;
            float4 _8300 = gel_image;
            float3 _8302 = _8300.xyz * tube_shadow;
            gel_image.x = _8302.x;
            gel_image.y = _8302.y;
            gel_image.z = _8302.z;
            if (HSM_TUBE_COLORED_GEL_IMAGE_FAKE_SCANLINE_AMOUNT > 0.001000000047497451305389404296875f)
            {
                ...
                float4 _8325 = HSM_ApplyScanlineMask(param_82, param_83, param_84, param_85, param_86, param_87);
                gel_image.x = _8325.xyz.x;
                gel_image.y = _8325.xyz.y;
                gel_image.z = _8325.xyz.z;
            }
            ...
            float3 _8341 = HSM_ApplyAmbientImage(param_88, param_89, param_90);
            gel_image.x = _8341.x;
            gel_image.y = _8341.y;
            gel_image.z = _8341.z;
            ...
            float3 _8356 = HSM_ApplyAmbientImage(param_91, param_92, param_93);
            gel_image.x = _8356.x;
            gel_image.y = _8356.y;
            gel_image.z = _8356.z;
            ...
            float4 _8376 = HSM_BlendModeLayerMix(param_94, param_95, param_96, param_97);
            gel_image = _8376;
            if (HSM_SHOW_CRT_ON_TOP_OF_COLORED_GEL == 0.0f)
            {
                ...
                float4 _8389 = HSM_BlendModeLayerMix(param_98, param_99, param_100, param_101);
                out_color = _8389;
            }
            if (HSM_SHOW_CRT_ON_TOP_OF_COLORED_GEL == 1.0f)
            {
                float4 _8395 = gel_image;
                float3 _8397 = _8395.xyz * tube_shadow;
                gel_image.x = _8397.x;
                gel_image.y = _8397.y;
                gel_image.z = _8397.z;
                ...
                float4 _8413 = HSM_BlendModeLayerMix(param_102, param_103, param_104, param_105);
                float4 normal_blended = _8413;
                out_color = normal_blended + out_color;
            }
        }
    }
    float4 _8418 = out_color;
    float3 _8420 = _8418.xyz * black_edge_mask_invert;
    out_color.x = _8420.x;
    out_color.y = _8420.y;
    out_color.z = _8420.z;
    if (HSM_GetUseTubeStaticReflection())
    {
        ...
        float4 _8434 = HSM_GetTubeHighlight(param_106, param_107, TubeStaticReflectionImage_1, _TubeStaticReflectionImage_1_sampler);
        float3 tube_highlight_image = _8434.xyz;
        ...
        float4 _8482 = out_color;
        float3 _8484 = _8482.xyz + (tube_highlight_image * HSM_TUBE_STATIC_REFLECTION_IMAGE_OPACITY);
        out_color.x = _8484.x;
        out_color.y = _8484.y;
        out_color.z = _8484.z;
    }
...
void frag_main()
{
    ...
    bool _8508 = HSM_AB_COMPARE_FREEZE_CRT_TUBE == 1.0f;
    bool _8516;
    if (_8508)
    {
        float2 param = vTexCoord;
        _8516 = HSM_GetIsInABCompareArea(param);
    }
    else
    {
        _8516 = _8508;
    ...
    float4 _8536 = HSM_UpdateGlobalScreenValuesFromCache(InfoCachePass, _InfoCachePass_sampler, param_3);
    ...
    float _8553 = HSM_GetSimpleBezelCoords(param_4, param_5, param_6, param_7, param_8, param_9);
    ...
    float _8585 = HSM_GetBezelCoords(param_11, param_12, param_13, param_14, param_15, param_16, param_17, param_18, param_19);
    ...
    float4 _8616 = HSM_GetPostCrtPreppedColor(param_23, param_24, param_25, DerezedPass, _DerezedPass_sampler, TubeDiffuseImage, _TubeDiffuseImage_sampler, TubeColoredGelImage, _TubeColoredGelImage_sampler, TubeStaticReflectionImage, _TubeStaticReflectionImage_sampler, BackgroundImage, _BackgroundImage_sampler, BackgroundVertImage, _BackgroundVertImage_sampler, NightLightingImage, _NightLightingImage_sampler, NightLighting2Image, _NightLighting2Image_sampler);
    FragColor = _8616;
}
