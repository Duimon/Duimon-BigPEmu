...
float4 HSM_BlendModeLayerMix(float4 color_under, inout float4 color_over, float blend_mode, float layer_opacity)
{
    ...
    {
        float _1164 = color_over.w;
        float4 _1165 = color_over;
        float3 _1167 = _1165.xyz * _1164;
        color_over.x = _1167.x;
        color_over.y = _1167.y;
        color_over.z = _1167.z;
    ...
    else
    {
        ...
        {
            float3 _1192 = color_under.xyz + color_over.xyz;
            blend_color.x = _1192.x;
            blend_color.y = _1192.y;
            blend_color.z = _1192.z;
        }
        if (blend_mode == BLEND_MODE_MULTIPLY)
        {
            float3 _1208 = color_under.xyz * color_over.xyz;
            blend_color.x = _1208.x;
            blend_color.y = _1208.y;
            blend_color.z = _1208.z;
        }
...
{
    uint _1240_dummy_parameter;
    float2 tex_size = float2(int2(spvTextureSize(in_sampler_2D, uint(0), _1240_dummy_parameter)));
...
float4 GetColorWithIntro(float4 in_color, inout float2 in_coord, Texture2D<float4> in_logo_image_sampler2d, SamplerState _in_logo_image_sampler2d_sampler)
{
    ...
    {
        _1894 = CURRENT_FRAME_FROM_CACHE_INFO;
    }
    else
    {
        _1894 = float(global_FrameCount);
    }
    float frame_count = _1894;
    ...
    if (frame_count <= full_seq_length)
    {
        ...
        float param_2 = 1.5f;
        float _2120 = HHLP_EasePowerOut(param_1, param_2);
        mask *= float((_2120 > height) ? 0 : 1);
        float area = width * height;
        float param_3 = clamp(1.0f - ((frame_count - ((power_on_seq_length + noise_hold) + black_hold)) / black_fade_out), 0.0f, 1.0f);
        float param_4 = 1.5f;
        float _2145 = HHLP_EasePowerIn(param_3, param_4);
        float black_opacity = _2145;
        ...
        float param_6 = 1.5f;
        float _2168 = HHLP_EasePowerIn(param_5, param_6);
        float solid_color_opacity = _2168;
        ...
        float param_8 = 4.0f;
        float _2182 = HHLP_EasePowerInOut(param_7, param_8);
        noise_opacity = _2182;
        ...
        noise_color.w = 1.0f;
        float4 _2214 = noise_color;
        float3 _2216 = _2214.xyz * 0.60000002384185791015625f;
        noise_color.x = _2216.x;
        noise_color.y = _2216.y;
        noise_color.z = _2216.z;
        ...
        float4 _2233 = HSM_BlendModeLayerMix(param_9, param_10, param_11, param_12);
        out_color = _2233;
        uint _2237_dummy_parameter;
        float2 intro_image_res = float2(int2(spvTextureSize(in_logo_image_sampler2d, uint(0), _2237_dummy_parameter)));
        ...
        if (intro_logo_height == 0.0f)
        {
            ...
            if (abs(HSM_ROTATE_CORE_IMAGE) > 0.5f)
            {
                _2257 = intro_image_res.y / global_OriginalSize.y;
            }
            else
            {
                _2257 = intro_image_res.y / global_OriginalSize.x;
            }
            intro_logo_height = _2257;
        }
        ...
        float4 _2394 = HSM_TextureQuilez(IntroImage, _IntroImage_sampler, param_13, param_14);
        float4 intro_image = _2394;
        float param_15 = clamp(1.0f - (((frame_count - mask_wait) - (mask_fade_horizontal / 4.0f)) / intro_logo_fade_in), 0.0f, 1.0f);
        float param_16 = 1.5f;
        float _2408 = HHLP_EasePowerOut(param_15, param_16);
        float logo_opacity = 1.0f - _2408;
        ...
        if (global_HSM_INTRO_LOGO_OVER_SOLID_COLOR == 0.0f)
        {
            ...
            float4 _2446 = HSM_BlendModeLayerMix(param_17, param_18, param_19, param_20);
            out_color = _2446;
        }
        ...
        float4 _2457 = HSM_BlendModeLayerMix(param_21, param_22, param_23, param_24);
        out_color = _2457;
        if (global_HSM_INTRO_LOGO_OVER_SOLID_COLOR == 1.0f)
        {
            ...
            float4 _2472 = HSM_BlendModeLayerMix(param_25, param_26, param_27, param_28);
            out_color = _2472;
        }
        out_color *= mask;
...
void frag_main()
{
    ...
    {
        _400 = global_HSM_SNAP_TO_CLOSEST_INT_SCALE_TOLERANCE / 100.0f;
    }
    else
    {
        _400 = 0.0f;
    }
    HSM_SNAP_TO_CLOSEST_INT_SCALE_TOLERANCE = _400;
    ...
    float4 _2489 = HSM_UpdateGlobalScreenValuesFromCache(InfoCachePass, _InfoCachePass_sampler, param);
    ...
    FragColor = Source.Sample(_Source_sampler, viewport_coord_adjusted);
    if (global_HSM_INTRO_WHEN_TO_SHOW > 0.0f)
    {
        float4 param_1 = FragColor;
        float2 param_2 = viewport_coord_adjusted;
        float4 _2525 = GetColorWithIntro(param_1, param_2, IntroImage, _IntroImage_sampler);
        FragColor = _2525;
    }
    ...
    {
        bool _2534 = HSM_CACHE_UPDATE_INDICATOR_MODE == 1.0f;
        bool _2543;
        if (!_2534)
        {
            _2543 = (HSM_CACHE_GRAPHICS_ON < 0.5f) && (HSM_CACHE_UPDATE_INDICATOR_MODE == 2.0f);
        }
        else
        {
            _2543 = _2534;
    ...
    {
        _2544 = cache_info_changed;
    }
    bool show_update_indicator = _2544;
    if (show_update_indicator)
    {
        ...
        if (_distance < 0.039999999105930328369140625f)
        {
            ...
            float param_3 = modulation;
            float param_4 = 2.0f;
            float _2599 = HHLP_EasePowerInOut(param_3, param_4);
            modulation = _2599;
    ...
    if (HSM_RESOLUTION_DEBUG_ON == 1.0f)
    {
        ...
        float4 _2623 = text_rgba;
        float3 _2625 = _2623.xyz * float3(1.0f, 1.0f, 0.0f);
        text_rgba.x = _2625.x;
        text_rgba.y = _2625.y;
        text_rgba.z = _2625.z;
        ...
        float4 param_5 = FragColor;
        float4 param_6 = text_rgba;
        FragColor = HSM_PreMultAlphaBlend(param_5, param_6);
    }
}
