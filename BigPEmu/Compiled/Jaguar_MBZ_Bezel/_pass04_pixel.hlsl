...
float2 GetIntegerScaleMult(float2 screen_size, float2 cropped_size)
{
    ...
    integer_scale = float2(_5617, _5636);
...
float sprite(float2 spr, float2 size, inout float2 uv)
{
    ...
    if (bounds)
    {
        _3183 = all(bool2(uv.x < size.x, uv.y < size.y));
    }
    else
    {
        _3183 = bounds;
...
float print_integer(float number, int zeros, float2 uv)
{
    ...
    for (int i = 4; i >= 0; i--)
    {
        ...
        {
            float param = digit;
            float2 _3405 = get_digit(param);
            float2 param_1 = _3405;
            float2 param_2 = uv;
...
float print_number(float number, float2 pos, float2 uv)
{
    ...
    for (int i = 3; i >= (-2); i--)
    {
        ...
        float param_3 = digit;
        float2 _3349 = get_digit(param_3);
        float2 param_4 = _3349;
...
void frag_main()
{
    ...
    float4 _5830 = HSM_UpdateGlobalScreenValuesFromCache(InfoCachePass, _InfoCachePass_sampler, param);
