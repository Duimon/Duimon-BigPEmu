...
float4 HSM_GetBlackOnlyCropInPixels(Texture2D<float4> in_sampler_2D, SamplerState _in_sampler_2D_sampler, float2 sample_start_pixel_coord, float2 window_size, float num_samples, float4 max_crop)
{
    ...
    for (int i = 0; float(i) < crop_left_px; i++)
    {
        ...
        float4 _2708 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler_2D, _in_sampler_2D_sampler, param, param_1, param_2);
        tex_sample = _2708;
    ...
    for (int i_1 = 0; float(i_1) < crop_top_px; i_1++)
    {
        ...
        float4 _2755 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler_2D, _in_sampler_2D_sampler, param_3, param_4, param_5);
        tex_sample = _2755;
        brightness = (tex_sample.x + tex_sample.y) + tex_sample.z;
        if (brightness > HSM_CROP_BLACK_THRESHOLD)
        {
    ...
    for (int i_2 = 0; float(i_2) < crop_right_px; i_2++)
    {
        ...
        float4 _2803 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler_2D, _in_sampler_2D_sampler, param_6, param_7, param_8);
        tex_sample = _2803;
        brightness = (tex_sample.x + tex_sample.y) + tex_sample.z;
        if (brightness > HSM_CROP_BLACK_THRESHOLD)
        {
    ...
    for (int i_3 = 0; float(i_3) < crop_bottom_px; i_3++)
    {
        ...
        float4 _2851 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler_2D, _in_sampler_2D_sampler, param_9, param_10, param_11);
        tex_sample = _2851;
        brightness = (tex_sample.x + tex_sample.y) + tex_sample.z;
        if (brightness > 0.0f)
        {
...
float2 HSM_GetResMult(float use_vert_scanlines)
{
    ...
    bool _2265 = (use_vert_scanlines == 1.0f) && (HSM_ROTATE_CORE_IMAGE == 0.0f);
    bool _2277;
    if (!_2265)
    {
        bool _2270 = use_vert_scanlines == 0.0f;
        bool _2276;
        if (_2270)
        {
            _2276 = abs(HSM_ROTATE_CORE_IMAGE) == 1.0f;
        }
        else
        {
            _2276 = _2270;
    ...
    else
    {
        ...
        if (global_HSM_INT_SCALE_MODE == 0.0f)
        {
            if (HSM_USE_PHYSICAL_SIZE_FOR_NON_INTEGER == 1.0f)
            {
                ...
                {
                    _1937 = 1.0f / HSM_PHYSICAL_MONITOR_ASPECT_RATIO;
                }
                else
                {
            ...
            if (HSM_SNAP_TO_CLOSEST_INT_SCALE_TOLERANCE > 0.0f)
            {
                ...
                {
                    _1992 = 1.0f - int_scale_remainder;
                }
        ...
        if (global_HSM_INT_SCALE_MODE > 0.5f)
        {
            ...
            bool _2146 = HSM_GetUseVerticalScanlines(param) == 1.0f;
            bool _2153;
            if (!_2146)
            {
                _2153 = global_HSM_INT_SCALE_MODE == 2.0f;
            }
            else
            {
                _2153 = _2146;
...
float2 HSM_GetScreenPositionOffset(inout float2 placement_image_pos, float2 screen_scale, float screen_index)
{
    ...
    if (screen_index == 1.0f)
    {
        _1377 = float2(HSM_SCREEN_POSITION_X / output_aspect, HSM_SCREEN_POSITION_Y) + (placement_image_pos - 0.5f.xx);
    }
    else
    {
        _1377 = float2(HSM_2ND_SCREEN_POS_X / output_aspect, HSM_2ND_SCREEN_POS_Y);
    }
    float2 pos_offset = _1377;
...
float4 HSM_GetColorForScreenInfoCache(float2 viewport_coord, Texture2D<float4> feedback_pass, SamplerState _feedback_pass_sampler, Texture2D<float4> original_pass, SamplerState _original_pass_sampler, Texture2D<float4> screen_placement_image, SamplerState _screen_placement_image_sampler)
{
    ...
    float2 _3651 = HSM_GetCroppedRotatedSizeWithResMult(param_7, param_8);
    float2 cropped_rotated_size_with_res_mult = _3651;
    float param_9 = 60.0f;
    ...
    {
        _3676 = (0.60000002384185791015625f * HSM_SINDEN_BORDER_THICKNESS) * 60.0f;
    }
    else
    {
        _3676 = 0.0f;
    ...
    float2 _3710 = HSM_GetTubeScale(param_14, param_15, param_16);
    float2 tube_diffuse_scale = _3710;
    ...
    if (global_HSM_TUBE_DIFFUSE_FORCE_ASPECT > 0.0f)
    {
        ...
        if (global_HSM_TUBE_DIFFUSE_FORCE_ASPECT == 2.0f)
        {
            ...
            {
                _3737 = global_HSM_TUBE_EXPLICIT_ASPECT;
            }
            else
            {
                _3737 = 1.0f / global_HSM_TUBE_EXPLICIT_ASPECT;
            }
    ...
    float2 _3776 = HSM_GetTubeScale(param_17, param_18, param_19);
    float2 tube_scale = _3776;
    float2 param_20 = screen_pos_and_height.xy;
    float2 param_21 = screen_scale;
    float param_22 = 1.0f;
    float2 _3784 = HSM_GetScreenPositionOffset(param_20, param_21, param_22);
    float2 pos_offset = _3784;
    float param_23 = 1.0f;
    ...
    float2 _3827 = HSM_GetCroppedRotatedSizeWithResMult(param_30, param_31);
    float2 cropped_size_with_res_mult_2nd_screen = _3827;
    ...
    float2 _3846 = HSM_GetTubeScale(param_32, param_33, param_34);
    float2 tube_diffuse_scale_2nd_screen = _3846;
    ...
    float2 _3865 = HSM_GetTubeScale(param_35, param_36, param_37);
    float2 tube_scale_2nd_screen = _3865;
    ...
    float param_40 = 2.0f;
    float2 _3879 = HSM_GetScreenPositionOffset(param_38, param_39, param_40);
    float2 pos_offset_2nd_screen = _3879;
    ...
    if (HSM_IsCoordIn2DRange(param_56, param_57) == 1)
    {
        ...
        float2 _3960 = HSM_GetRotatedDerezedSize();
        out_color.z = _3960.x;
        out_color.w = _3960.y;
    }
    float param_58 = 1.0f;
    ...
    if (HSM_IsCoordIn2DRange(param_80, param_81) == 1)
    {
        ...
        float2 _4080 = HSM_GetRotatedCoreOriginalSize();
        out_color.z = _4080.x;
        out_color.w = _4080.y;
    }
    float param_82 = 3.0f;
    ...
    {
        float2 _4098 = float2(HSM_VIEWPORT_ZOOM, HSM_VIEWPORT_ZOOM);
        out_color.x = _4098.x;
        out_color.y = _4098.y;
        float2 _4105 = float2(HSM_VIEWPORT_POSITION_X, HSM_VIEWPORT_POSITION_Y);
        out_color.z = _4105.x;
        out_color.w = _4105.y;
    }
    float param_86 = 4.0f;
    ...
    if (!_4287)
    {
        _4297 = abs(parameter_sum - parameter_sum_feedback) > 1.0000000116860974230803549289703e-07f;
    }
    else
    {
        _4297 = _4287;
    ...
    {
        _4298 = false || (HSM_CACHE_GRAPHICS_ON < 0.5f);
    }
...
void frag_main()
{
    ...
    float4 _4334 = HSM_GetColorForScreenInfoCache(param, InfoCachePassFeedback, _InfoCachePassFeedback_sampler, Source, _Source_sampler, ScreenPlacementImage, _ScreenPlacementImage_sampler);
    FragColor = _4334;
}
