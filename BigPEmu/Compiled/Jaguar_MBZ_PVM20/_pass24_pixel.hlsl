...
Texture2D<float4> PostCRTPass : register(t3);
SamplerState _PostCRTPass_sampler : register(s3);
...
void frag_main()
{
    ...
    {
        _305 = global_HSM_SNAP_TO_CLOSEST_INT_SCALE_TOLERANCE / 100.0f;
    }
    else
    {
        _305 = 0.0f;
    }
    HSM_SNAP_TO_CLOSEST_INT_SCALE_TOLERANCE = _305;
    ...
    float4 crt_rgba = CRTPass.SampleLevel(_CRTPass_sampler, vTexCoord, 2.0f);
    float4 post_crt_rgba = PostCRTPass.SampleLevel(_PostCRTPass_sampler, vTexCoord, 2.0f);
    FragColor = lerp(crt_rgba, post_crt_rgba, HSM_REFLECT_SHOW_TUBE_FX_AMOUNT.xxxx);
    float4 param = FragColor;
    float param_1 = 0.86666667461395263671875f;
    FragColor = HSM_ApplyGamma(param, param_1);
    FragColor = clamp(FragColor, 0.0f.xxxx, 1.0f.xxxx);
}
