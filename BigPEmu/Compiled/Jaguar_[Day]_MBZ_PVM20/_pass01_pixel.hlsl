...
float4 HSM_GetBlackOnlyCropInPixels(Texture2D<float4> in_sampler_2D, SamplerState _in_sampler_2D_sampler, float2 sample_start_pixel_coord, float2 window_size, float num_samples, float4 max_crop)
{
    ...
    for (int i = 0; float(i) < crop_left_px; i++)
    {
        ...
        float4 _3954 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler_2D, _in_sampler_2D_sampler, param, param_1, param_2);
        tex_sample = _3954;
    ...
    for (int i_1 = 0; float(i_1) < crop_top_px; i_1++)
    {
        ...
        float4 _4001 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler_2D, _in_sampler_2D_sampler, param_3, param_4, param_5);
        tex_sample = _4001;
        brightness = (tex_sample.x + tex_sample.y) + tex_sample.z;
        if (brightness > HSM_CROP_BLACK_THRESHOLD)
        {
    ...
    for (int i_2 = 0; float(i_2) < crop_right_px; i_2++)
    {
        ...
        float4 _4049 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler_2D, _in_sampler_2D_sampler, param_6, param_7, param_8);
        tex_sample = _4049;
        brightness = (tex_sample.x + tex_sample.y) + tex_sample.z;
        if (brightness > HSM_CROP_BLACK_THRESHOLD)
        {
    ...
    for (int i_3 = 0; float(i_3) < crop_bottom_px; i_3++)
    {
        ...
        float4 _4097 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler_2D, _in_sampler_2D_sampler, param_9, param_10, param_11);
        tex_sample = _4097;
        brightness = (tex_sample.x + tex_sample.y) + tex_sample.z;
        if (brightness > 0.0f)
        {
...
float HSM_GetScreenAspect(float screen_index, float2 cropped_size)
{
    ...
    {
        _3551 = 1.0f / core_aspect_ratio;
    }
    else
    {
        _3551 = core_aspect_ratio;
    }
    float core_aspect_horizontal = _3551;
    ...
    if (HSM_ASPECT_RATIO_ORIENTATION < 0.5f)
    {
        ...
        bool _3820 = HSM_ResolutionIsEqual(param_30, param_31);
        bool _3829;
        if (!_3820)
        {
            float2 param_32 = atari_2600_crop_res;
            float2 param_33 = original_size;
            _3829 = HSM_ResolutionIsEqual(param_32, param_33);
        }
        else
        {
            _3829 = _3820;
        ...
        if (!_3829)
        {
            ...
            _3838 = HSM_ResolutionIsEqual(param_34, param_35);
        }
        else
        {
            _3838 = _3829;
        ...
        if (!_3838)
        {
            ...
            _3847 = HSM_ResolutionIsEqual(param_36, param_37);
        }
        else
        {
            _3847 = _3838;
        ...
        if (!_3847)
        {
            ...
            _3856 = HSM_ResolutionIsEqual(param_38, param_39);
        }
        else
        {
            _3856 = _3847;
        ...
        if (!_3856)
        {
            _3864 = original_size.y == sony_ps_height;
        }
        else
        {
            _3864 = _3856;
...
float2 HSM_GetResMult(float use_vert_scanlines)
{
    ...
    bool _3511 = (use_vert_scanlines == 1.0f) && (HSM_ROTATE_CORE_IMAGE == 0.0f);
    bool _3523;
    if (!_3511)
    {
        bool _3516 = use_vert_scanlines == 0.0f;
        bool _3522;
        if (_3516)
        {
            _3522 = abs(HSM_ROTATE_CORE_IMAGE) == 1.0f;
        }
        else
        {
            _3522 = _3516;
    ...
    else
    {
        ...
        if (global_HSM_INT_SCALE_MODE == 0.0f)
        {
            if (HSM_USE_PHYSICAL_SIZE_FOR_NON_INTEGER == 1.0f)
            {
                ...
                {
                    _3183 = 1.0f / HSM_PHYSICAL_MONITOR_ASPECT_RATIO;
                }
                else
                {
                    _3183 = HSM_PHYSICAL_MONITOR_ASPECT_RATIO;
                }
                float monitor_aspect_with_rotation = _3183;
            ...
            if (HSM_SNAP_TO_CLOSEST_INT_SCALE_TOLERANCE > 0.0f)
            {
                ...
                {
                    _3238 = int_scale_remainder;
                }
                else
                {
                    _3238 = 1.0f - int_scale_remainder;
                }
                int_scale_remainder = _3238;
        ...
        if (global_HSM_INT_SCALE_MODE > 0.5f)
        {
            ...
            bool _3392 = HSM_GetUseVerticalScanlines(param) == 1.0f;
            bool _3399;
            if (!_3392)
            {
                _3399 = global_HSM_INT_SCALE_MODE == 2.0f;
            }
            else
            {
                _3399 = _3392;
...
float2 HSM_GetScreenPositionOffset(inout float2 placement_image_pos, float2 screen_scale, float screen_index)
{
    ...
    if (screen_index == 1.0f)
    {
        _2623 = float2(HSM_SCREEN_POSITION_X / output_aspect, HSM_SCREEN_POSITION_Y) + (placement_image_pos - 0.5f.xx);
    }
    else
    {
        _2623 = float2(HSM_2ND_SCREEN_POS_X / output_aspect, HSM_2ND_SCREEN_POS_Y);
    }
    float2 pos_offset = _2623;
...
{
    bool _3887 = in_coord.x > in_2d_range.x;
    bool _3895;
    if (_3887)
    {
        _3895 = in_coord.y > in_2d_range.y;
    }
    else
    {
        _3895 = _3887;
    ...
    if (_3895)
    {
        _3903 = in_coord.x < in_2d_range.z;
    }
    else
    {
        _3903 = _3895;
    ...
    if (_3903)
    {
        _3911 = in_coord.y < in_2d_range.w;
    }
    else
    {
        _3911 = _3903;
...
float4 HSM_GetColorForScreenInfoCache(float2 viewport_coord, Texture2D<float4> feedback_pass, SamplerState _feedback_pass_sampler, Texture2D<float4> original_pass, SamplerState _original_pass_sampler, Texture2D<float4> screen_placement_image, SamplerState _screen_placement_image_sampler)
{
    ...
    float2 _5558 = HSM_GetCroppedRotatedSizeWithResMult(param_7, param_8);
    float2 cropped_rotated_size_with_res_mult = _5558;
    float param_9 = 60.0f;
    ...
    {
        _5583 = (0.60000002384185791015625f * HSM_SINDEN_BORDER_THICKNESS) * 60.0f;
    }
    else
    {
        _5583 = 0.0f;
    }
    float tube_sinden_border_thickness = _5583;
    ...
    float2 _5617 = HSM_GetTubeScale(param_14, param_15, param_16);
    float2 tube_diffuse_scale = _5617;
    ...
    if (global_HSM_TUBE_DIFFUSE_FORCE_ASPECT > 0.0f)
    {
        ...
        if (global_HSM_TUBE_DIFFUSE_FORCE_ASPECT == 2.0f)
        {
            ...
            {
                _5644 = global_HSM_TUBE_EXPLICIT_ASPECT;
            }
            else
            {
                _5644 = 1.0f / global_HSM_TUBE_EXPLICIT_ASPECT;
            }
            tube_diffuse_aspect = _5644;
        }
    ...
    float2 _5683 = HSM_GetTubeScale(param_17, param_18, param_19);
    float2 tube_scale = _5683;
    float2 param_20 = screen_pos_and_height.xy;
    float2 param_21 = screen_scale;
    float param_22 = 1.0f;
    float2 _5691 = HSM_GetScreenPositionOffset(param_20, param_21, param_22);
    float2 pos_offset = _5691;
    float param_23 = 1.0f;
    ...
    {
        _5708 = cropped_size_2nd_screen.x / cropped_size_2nd_screen.y;
    }
    else
    {
        _5708 = screen_aspect;
    }
    float screen_aspect_2nd_screen = _5708;
    ...
    float2 _5734 = HSM_GetCroppedRotatedSizeWithResMult(param_30, param_31);
    float2 cropped_size_with_res_mult_2nd_screen = _5734;
    ...
    float2 _5753 = HSM_GetTubeScale(param_32, param_33, param_34);
    float2 tube_diffuse_scale_2nd_screen = _5753;
    ...
    float2 _5772 = HSM_GetTubeScale(param_35, param_36, param_37);
    float2 tube_scale_2nd_screen = _5772;
    ...
    float param_40 = 2.0f;
    float2 _5786 = HSM_GetScreenPositionOffset(param_38, param_39, param_40);
    float2 pos_offset_2nd_screen = _5786;
    ...
    if (HSM_IsCoordIn2DRange(param_56, param_57) == 1)
    {
        ...
        float2 _5867 = HSM_GetRotatedDerezedSize();
        out_color.z = _5867.x;
        out_color.w = _5867.y;
    }
    float param_58 = 1.0f;
    ...
    if (HSM_IsCoordIn2DRange(param_80, param_81) == 1)
    {
        ...
        float2 _5987 = HSM_GetRotatedCoreOriginalSize();
        out_color.z = _5987.x;
        out_color.w = _5987.y;
    }
    float param_82 = 3.0f;
    ...
    {
        float2 _6005 = float2(HSM_VIEWPORT_ZOOM, HSM_VIEWPORT_ZOOM);
        out_color.x = _6005.x;
        out_color.y = _6005.y;
        float2 _6012 = float2(HSM_VIEWPORT_POSITION_X, HSM_VIEWPORT_POSITION_Y);
        out_color.z = _6012.x;
        out_color.w = _6012.y;
    }
    float param_86 = 4.0f;
    ...
    bool _6194 = (res_mult_size_sum != res_mult_size_sum_feedback) || (res_mult_size2_sum != res_mult_size2_sum_feedback);
    bool _6204;
    if (!_6194)
    {
        _6204 = abs(parameter_sum - parameter_sum_feedback) > 1.0000000116860974230803549289703e-07f;
    }
    else
    {
        _6204 = _6194;
    ...
    {
        _6205 = false || (HSM_CACHE_GRAPHICS_ON < 0.5f);
    }
    bool cache_changed = _6205;
...
void frag_main()
{
    ...
    float4 _6241 = HSM_GetColorForScreenInfoCache(param, InfoCachePassFeedback, _InfoCachePassFeedback_sampler, Source, _Source_sampler, ScreenPlacementImage, _ScreenPlacementImage_sampler);
    FragColor = _6241;
}
