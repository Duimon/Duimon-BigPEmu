...
{
    bool _1191 = abs(in_coord.x - 0.5f) > 0.500999987125396728515625f;
    bool _1201;
    if (!_1191)
    {
        _1201 = abs(in_coord.y - 0.5f) > 0.500999987125396728515625f;
    }
    else
    {
        _1201 = _1191;
...
float4 HSM_GetCroppedTexSample(Texture2D<float4> in_sampler, SamplerState _in_sampler_sampler, float2 in_screen_coord)
{
    ...
    float4 _2206 = HSM_GetTexSampleFromSampleStartAndSize(in_sampler, _in_sampler_sampler, param, param_1, param_2);
    return _2206;
}
...
float2 HSM_GetCurvatureValues(float screen_aspect)
{
    ...
    {
        _1802 = float2((((2.0f * global_HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f, ((global_HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f);
    }
    else
    {
        _1802 = float2(((global_HSM_CURVATURE_2D_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f, (((2.0f * global_HSM_CURVATURE_2D_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f);
    }
    float2 curvature_values = _1802;
...
float2 HSM_Get2DCurvedCoord(float2 in_coord, float2 curvature_values)
{
    ...
    float2 _1885 = HSM_GetCrtPiCurvedCoord(param, param_1);
    ctr_curved_coord = _1885 - 0.5f.xx;
    float2 param_2 = float2(1.0f, 0.5f);
    float2 param_3 = curvature_values;
    float2 _1893 = HSM_GetCrtPiCurvedCoord(param_2, param_3);
    float2 right_edge_curved_ctr_coord = _1893 - 0.5f.xx;
    ...
    float2 _1908 = HSM_GetCrtPiCurvedCoord(param_4, param_5);
    float2 bottom_edge_curved_ctr_coord = _1908 - 0.5f.xx;
...
{
    float2 ctr_coord = in_coord - 0.5f.xx;
    bool _1999 = abs(ctr_coord.x) > 0.5f;
    bool _2007;
    if (!_1999)
    {
        _2007 = abs(ctr_coord.y) > 0.5f;
    }
    else
    {
        _2007 = _1999;
...
float2 HSM_GetCrtShaderFinalCoordExtraVariables(float2 screen_coord, float2 screen_scale, float raster_bloom_avg_lum, inout float2 screen_curved_coord, inout float2 screen_curved_coord_with_overscan, inout float2 screen_coord_with_overscan, inout float2 screen_scale_with_overscan)
{
    ...
    float2 _2523 = HSM_GetMirrorWrappedCoord(param_1);
    float2 screen_curved_coord_with_overscan_and_mirror = _2523;
...
float sw0(float x, float color, float scanline)
{
    ...
    {
        _2697 = lerp(ex * ex, (ex * ex) * ex, 0.4000000059604644775390625f);
    }
    ex = _2697;
...
bool HSM_GetUseFakeScanlines()
{
    ...
    bool _2225 = (global_HSM_FAKE_SCANLINE_OPACITY / 100.0f) > 0.001000000047497451305389404296875f;
    bool _2247;
    if (_2225)
    {
        bool _2231 = global_HSM_FAKE_SCANLINE_MODE == 1.0f;
        bool _2246;
        if (!_2231)
        {
            bool _2237 = global_HSM_FAKE_SCANLINE_MODE == 2.0f;
            bool _2245;
            if (_2237)
            {
                _2245 = scane_axis_core_res > global_HSM_INTERLACE_TRIGGER_RES;
            }
            else
            {
                _2245 = _2237;
...
float2 HSM_GetTubeCurvedCoord(float2 screen_coord, float curvature_scale, float2 screen_scale, float2 tube_scale, float screen_aspect, float apply_black_edge_offset)
{
    ...
    if (global_HSM_BZL_USE_INDEPENDENT_CURVATURE == 1.0f)
    {
        ...
        if (screen_aspect < 1.0f)
        {
            _2132 = float2((((2.0f * global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f, ((global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f);
        }
        else
        {
            _2132 = float2(((global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_LONG_AXIS / 100.0f) * 3.0f) / 100.0f, (((2.0f * global_HSM_BZL_INDEPENDENT_CURVATURE_SCALE_SHORT_AXIS) / 100.0f) * 2.0f) / 100.0f);
        }
        float2 curvature_values = _2132;
...
float4 HSM_ApplyScanlineMask(float4 in_color, float2 screen_scale, inout float2 in_coord, float2 in_screen_curved_coord, float2 in_tube_curved_coord, float in_scanline_opacity)
{
    ...
    {
        _2322 = global_HSM_FAKE_SCANLINE_RES;
    }
    else
    {
...
void frag_main()
{
    ...
    float4 _2823 = HSM_UpdateGlobalScreenValuesFromCache(InfoCachePass, _InfoCachePass_sampler, param_1);
    ...
    float2 _2897 = HSM_GetCrtShaderFinalCoordExtraVariables(param_5, param_6, param_7, param_8, param_9, param_10, param_11);
    ...
    float2 screen_curved_coord_with_overscan_and_mirror = _2897;
    ...
    if (!interb)
    {
        ...
        bool _4150 = global_HSM_INTERLACE_MODE < 0.0f;
        bool _4157;
        if (_4150)
        {
            _4157 = global_HSM_INTERLACE_TRIGGER_RES <= scan_res;
        }
        else
        {
            _4157 = _4150;
    ...
    if (HSM_GetUseFakeScanlines())
    {
        ...
        float4 _4292 = HSM_ApplyScanlineMask(param_58, param_59, param_60, param_61, param_62, param_63);
        FragColor = _4292;
    }
}
